...
unsigned long loginlog_failedattempts(uint32 ip, uint32 minutes) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT count(*) FROM `%s` WHERE `ip` = '%s' AND (`rcode` = '0' OR `rcode` = '1') AND `time` > NOW() - INTERVAL %d MINUTE", log_login_db.c_str(), ip2str(ip, nullptr), minutes)) // how many times failed account? in one ip.
