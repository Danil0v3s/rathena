...
bool pc_can_sell_item(const map_session_data* sd, const item* item, enum npc_subtype shoptype) {
	...
	switch (shoptype) {
		case NPCTYPE_SHOP:
			if (item->bound && battle_config.allow_bound_sell & ISR_BOUND_SELLABLE && (item->bound != BOUND_GUILD || (sd->guild && sd->status.char_id == sd->guild->guild.member[0].char_id) || (item->bound == BOUND_GUILD && !(battle_config.allow_bound_sell & ISR_BOUND_GUILDLEADER_ONLY))))
				return true;
			break;
		case NPCTYPE_ITEMSHOP:
			if (item->bound && battle_config.allow_bound_sell & ISR_BOUND && (item->bound != BOUND_GUILD || (sd->guild && sd->status.char_id == sd->guild->guild.member[0].char_id) || (item->bound == BOUND_GUILD && !(battle_config.allow_bound_sell & ISR_BOUND_GUILDLEADER_ONLY))))
				return true;
			else if (!item->bound) {
...
bool pc_adoption(map_session_data* p1_sd, map_session_data* p2_sd, map_session_data* b_sd) {
	...
	if (job != -1 && pc_jobchange(b_sd, job, 0)) { // Success, proceed to configure parents and baby skills
...
void pc_reg_received(map_session_data* sd) {
	...
	if ((sd->class_ & MAPID_FIRSTMASK) == MAPID_TAEKWON) { // Better check for class rather than skill to prevent "skill resets" from unsetting this
...
void pc_calc_skilltree(map_session_data* sd) {
	...
	if (class_ == -1) { //Unable to normalize job??
	...
	do {
		...
		for (const auto& skillsit : tree->skills) {
			...
			if (!fail) {
				...
				if (!sd->status.skill[sk_idx].lv && ((skill->inf2[INF2_ISQUEST] && !battle_config.quest_skill_learn) || skill->inf2[INF2_ISWEDDING] || (skill->inf2[INF2_ISSPIRIT] && !sd->sc.getSCE(SC_SPIRIT))))
...
void pc_updateweightstatus(map_session_data& sd) {
	uint8 old_overweight = (sd.sc.getSCE(SC_WEIGHT90) != nullptr) ? 2 : (sd.sc.getSCE(SC_WEIGHT50) != nullptr) ? 1
	                                                                                                           : 0;
	uint16 overweight_percent = pc_getpercentweight(sd);
	uint8 new_overweight = (overweight_percent >= battle_config.major_overweight_rate) ? 2 : (overweight_percent >= battle_config.natural_heal_weight_rate) ? 1
	                                                                                                                                                        : 0;
...
/// Show error message
#define PC_BONUS_SHOW_ERROR(type, type2, val)                                      \
	{                                                                              \
		ShowError("%s: %s: Invalid %s %d.\n", __FUNCTION__, #type, #type2, (val)); \
		break;                                                                     \
	}
/// Check for valid Element, break & show error message if invalid Element
#define PC_BONUS_CHK_ELEMENT(ele, bonus)                  \
	{                                                     \
		if (!CHK_ELEMENT((ele))) {                        \
			PC_BONUS_SHOW_ERROR((bonus), Element, (ele)); \
		}                                                 \
	}
/// Check for valid Race, break & show error message if invalid Race
#define PC_BONUS_CHK_RACE(rc, bonus)                  \
	{                                                 \
		if (!CHK_RACE((rc))) {                        \
			PC_BONUS_SHOW_ERROR((bonus), Race, (rc)); \
		}                                             \
	}
/// Check for valid Race2, break & show error message if invalid Race2
#define PC_BONUS_CHK_RACE2(rc2, bonus)                  \
	{                                                   \
		if ((rc2) <= RC2_NONE || (rc2) >= RC2_MAX) {    \
			PC_BONUS_SHOW_ERROR((bonus), Race2, (rc2)); \
		}                                               \
	}
/// Check for valid Class, break & show error message if invalid Class
#define PC_BONUS_CHK_CLASS(cl, bonus)                  \
	{                                                  \
		if (!CHK_CLASS((cl))) {                        \
			PC_BONUS_SHOW_ERROR((bonus), Class, (cl)); \
		}                                              \
	}
/// Check for valid Size, break & show error message if invalid Size
#define PC_BONUS_CHK_SIZE(sz, bonus)                  \
	{                                                 \
		if (!CHK_MOBSIZE((sz))) {                     \
			PC_BONUS_SHOW_ERROR((bonus), Size, (sz)); \
		}                                             \
	}
/// Check for valid SC, break & show error message if invalid SC
#define PC_BONUS_CHK_SC(sc, bonus)                      \
	{                                                   \
		if ((sc) <= SC_NONE || (sc) >= SC_MAX) {        \
			PC_BONUS_SHOW_ERROR((bonus), Effect, (sc)); \
		}                                               \
	}
...
enum e_additem_result pc_cart_additem(map_session_data* sd, struct item* item, int32 amount, e_log_pick_type log_type) {
	...
	if (!itemdb_cancartstore(item, pc_get_group_level(sd)) || (item->bound > BOUND_ACCOUNT && !pc_can_give_bounded_items(sd))) { // Check item trade restrictions	[Skotlex]
	...
	if (itemdb_isstackable2(data) && !item->expire_time) {
		for (i = 0; i < MAX_CART; i++) {
			if (sd->cart.u.items_cart[i].nameid == item->nameid && sd->cart.u.items_cart[i].bound == item->bound && sd->cart.u.items_cart[i].unique_id == item->unique_id && memcmp(sd->cart.u.items_cart[i].card, item->card, sizeof(item->card)) == 0)
				break;
		}
	}
...
bool pc_steal_item(map_session_data* sd, block_list* bl, uint16 skill_lv) {
	...
	if (md->master_id || status_has_mode(md_status, MD_STATUSIMMUNE) || util::vector_exists(status_get_race2(md), RC2_TREASURE) ||
	    ...
	    (battle_config.skill_steal_max_tries &&                               //Reached limit of steal attempts. [Lupus]
	        md->state.steal_flag++ >= battle_config.skill_steal_max_tries)) { //Can't steal from
...
enum e_setpos pc_setpos(map_session_data* sd, uint16 mapindex, int32 x, int32 y, clr_type clrtype) {
	...
	if (sd->status.guild_id > 0 && mapdata->getMapFlag(MF_GVG_CASTLE)) { // Increased guild castle regen [Valaris]
...
uint64 pc_jobid2mapid(uint16 b_class) {
	switch (b_class) {
		...
		case JOB_BABY_OBORO:
			return MAPID_BABY_KAGEROUOBORO;
			//Baby 2-2 Jobs
		...
		case JOB_BABY_STAR_EMPEROR:
			return MAPID_BABY_STAR_EMPEROR;
			//Baby 3-2 Jobs
...
int32 pc_mapid2jobid(uint64 class_, int32 sex) {
	switch (class_) {
		...
		case MAPID_BABY_KAGEROUOBORO:
			return sex ? JOB_BABY_KAGEROU : JOB_BABY_OBORO;
			//Baby 2-2 Jobs
		...
		case MAPID_BABY_SOUL_LINKER:
			return JOB_BABY_SOUL_LINKER;
			//3-1 Jobs
		...
		case MAPID_SHINKIROSHIRANUI:
			return sex ? JOB_SHINKIRO : JOB_SHIRANUI;
			//3-2 Jobs
		...
		case MAPID_SOUL_REAPER:
			return JOB_SOUL_REAPER;
			//Trans 3-1 Jobs
		...
		case MAPID_BABY_STAR_EMPEROR:
			return JOB_BABY_STAR_EMPEROR;
			//Baby 3-2 Jobs
		...
		case MAPID_BABY_SOUL_REAPER:
			return JOB_BABY_SOUL_REAPER;
			//4-1 Jobs
		...
		case MAPID_SKY_EMPEROR:
			return JOB_SKY_EMPEROR;
			//4-2 Jobs
		...
		case MAPID_SOUL_ASCETIC:
			return JOB_SOUL_ASCETIC;
			//None
		default:
			return -1;
	}
...
int32 pc_resetskill(map_session_data* sd, int32 flag) {
	...
	for (const auto& skill : skill_db) {
		...
		if (skill.second->inf2[INF2_ISQUEST] && !battle_config.quest_skill_learn) { //Only handle quest skills in a special way when you can't learn them manually
	...
	if (bonus) {
		switch (skill_id) {
			...
			case AB_HIGHNESSHEAL:
				if (!(battle_config.skill_add_heal_rate & 64))
					bonus = 0;
				break;
			case CD_MEDIALE_VOTUM:
				if (!(battle_config.skill_add_heal_rate & 128))
					bonus = 0;
				break;
			case CD_DILECTIO_HEAL:
				if (!(battle_config.skill_add_heal_rate & 256))
					bonus = 0;
				break;
		}
	}
...
int32 pc_dead(map_session_data* sd, block_list* src) {
	...
	if (battle_config.bone_drop == 2 || (battle_config.bone_drop == 1 && mapdata->getMapFlag(MF_PVP))) {
	...
	if (battle_config.death_penalty_type && (sd->class_ & MAPID_SECONDMASK) != MAPID_NOVICE // only novices will receive no penalty
	    && !sd->sc.cant.deathpenalty && !mapdata->getMapFlag(MF_NOEXPPENALTY) && !mapdata_flag_gvg2(mapdata)) {
		...
		for (const auto& it : mapdata->drop_list) {
			...
			if (id == -1) {
				...
				for (i = 0; i < MAX_INVENTORY; i++) {
					...
					if ((type & NMDT_INVENTORY && !sd->inventory.u.items_inventory[i].equip) || (type & NMDT_EQUIP && sd->inventory.u.items_inventory[i].equip) || type == NMDT_ALL) {
						eq_n[eq_num++] = i;
					}
				}
				if (eq_num > 0) {
				...
				for (i = 0; i < MAX_INVENTORY; i++) {
					if (sd->inventory.u.items_inventory[i].nameid == id && pc_candrop(sd, &sd->inventory.u.items_inventory[i]) && rnd() % 10000 < per && ((type & NMDT_INVENTORY && !sd->inventory.u.items_inventory[i].equip) || (type & NMDT_EQUIP && sd->inventory.u.items_inventory[i].equip) || type == NMDT_ALL)) {
						if (sd->inventory.u.items_inventory[i].equip)
 ...
 *------------------------------------------*/
void pc_heal(map_session_data* sd, uint32 hp, uint32 sp, uint32 ap, int32 type) { // Is there going to be a effect for gaining AP soon??? [Rytech]
	nullpo_retv(sd);
...
void pc_setoption(map_session_data* sd, int32 type, int32 subtype) {
	...
	if ((type & OPTION_RIDING && !(p_type & OPTION_RIDING)) || (type & OPTION_DRAGON && !(p_type & OPTION_DRAGON) && pc_checkskill(sd, RK_DRAGONTRAINING) > 0)) { // Mounting
		clif_status_load(sd, EFST_RIDING, 1);
		status_calc_pc(sd, SCO_NONE);
	} else if ((!(type & OPTION_RIDING) && p_type & OPTION_RIDING) || (!(type & OPTION_DRAGON) && p_type & OPTION_DRAGON && pc_checkskill(sd, RK_DRAGONTRAINING) > 0)) { // Dismount
...
TIMER_FUNC(pc_calc_pvprank_timer) {
	...
	if (pc_isinvisible(sd)) { // do not calculate the pvp rank for a hidden GM
		return 0;
	}
...
bool pc_divorce(map_session_data* sd) {
	...
	if ((p_sd = map_charid2sd(sd->status.partner_id)) == nullptr) { // Lets char server do the divorce
...
static int32 pc_daynight_timer_sub(map_session_data* sd, va_list ap) {
	if (sd->state.night != night_flag && map_getmapflag(sd->m, MF_NIGHTENABLED)) { //Night/day state does not match.
...
void pc_bonus_script_clear(map_session_data* sd, uint32 flag) {
	...
	while (node) {
		...
		if (entry && ((flag == BSF_PERMANENT) ||                      // Remove all with permanent bonus
...
int16 pc_maxaspd(const map_session_data* sd) {
	...
	return ((sd->class_ & JOBL_THIRD) ? battle_config.max_third_aspd : (((sd->class_ & MAPID_SECONDMASK) == MAPID_KAGEROUOBORO || (sd->class_ & MAPID_SECONDMASK) == MAPID_REBELLION) ? battle_config.max_extended_aspd : ((sd->class_ & MAPID_FIRSTMASK) == MAPID_SUMMONER) ? battle_config.max_summoner_aspd
