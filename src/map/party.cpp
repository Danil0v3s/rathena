...
int32 party_skill_check(map_session_data* sd, int32 party_id, uint16 skill_id, uint16 skill_lv) {
	...
	for (i = 0; i < MAX_PARTY; i++) {
		...
		switch (skill_id) {
			case TK_COUNTER: //Increase Triple Attack rate of Monks.
				if ((p_sd->class_ & MAPID_SECONDMASK) == MAPID_MONK && pc_checkskill(p_sd, MO_TRIPLEATTACK)) {
				...
				break;
			case MO_COMBOFINISH: //Increase Counter rate of Star Gladiators
				if ((p_sd->class_ & MAPID_SECONDMASK) == MAPID_STAR_GLADIATOR && p_sd->sc.getSCE(SC_READYCOUNTER) && pc_checkskill(p_sd, SG_FRIEND)) {
