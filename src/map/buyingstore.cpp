...
int8 buyingstore_setup(map_session_data* sd, unsigned char slots) {
	...
	if (map_getmapflag(sd->m, MF_NOBUYINGSTORE)) {     // custom: no buyingstore maps
		clif_displaymessage(sd->fd, msg_txt(sd, 276)); // "You can't open a shop on this map"
		return 3;
	}

	if (map_getcell(sd->m, sd->x, sd->y, CELL_CHKNOBUYINGSTORE)) { // custom: no buyingstore cells
...
int8 buyingstore_create(map_session_data* sd, int32 zenylimit, unsigned char result, const char* storename, const struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub* itemlist, uint32 count, struct s_autotrader* at) {
	...
	if (pc_getpercentweight(*sd, weight) >= battle_config.major_overweight_rate) { // not able to carry all wanted items without getting overweight (90%)
		sd->buyingstore.slots = 0;
...
void buyingstore_open(map_session_data* sd, uint32 account_id) {
	...
	if ((pl_sd = map_id2sd(account_id)) == nullptr || !pl_sd->state.buyingstore) { // not online or not buying
		return;
	}

	if (!searchstore_queryremote(*sd, account_id) && (sd->m != pl_sd->m || !check_distance_bl(sd, pl_sd, AREA_SIZE))) { // out of view range
		return;
	}
...
void buyingstore_trade(map_session_data* sd, uint32 account_id, uint32 buyer_id, const struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub* itemlist, uint32 count) {
	...
	if ((pl_sd = map_id2sd(account_id)) == nullptr || !pl_sd->state.buyingstore || pl_sd->buyer_id != buyer_id) { // not online, not buying or not same store
		clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, 0);
		return;
	}
...
bool buyingstore_searchall(const map_session_data* sd, const struct s_search_store_search* s) {
	...
	for (idx = 0; idx < s->item_count; idx++) {
		...
		if (s->min_price && s->min_price > (uint32)it->price) { // too low price
			continue;
		}

		if (s->max_price && s->max_price < (uint32)it->price) { // too high price
			continue;
		}
	...
	if (battle_config.feature_autotrade) {
		...
		if (Sql_NumRows(mmysql_handle) > 0) {
			...
			for (at = (struct s_autotrader*)dbi_first(iter); dbi_exists(iter); at = (struct s_autotrader*)dbi_next(iter)) {
				...
				if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `item_id`, `amount`, `price` "
				                                          "FROM `%s` "
				...
				while (SQL_SUCCESS == Sql_NextRow(mmysql_handle) && j < at->count) {
					...
					Sql_GetData(mmysql_handle, 0, &data, nullptr);
					at->entries[j]->item_id = strtoul(data, nullptr, 10);
					Sql_GetData(mmysql_handle, 1, &data, nullptr);
					at->entries[j]->amount = atoi(data);
					Sql_GetData(mmysql_handle, 2, &data, nullptr);
					at->entries[j]->price = atoi(data);
					j++;
				}
				items += j;
				Sql_FreeResult(mmysql_handle);
