...
#define APPLY_CARDFIX(damage, fix) \
	{ (damage) = (damage) - (int64)(((damage) * (1000 - max(0, fix))) / 1000); }
...
#define APPLY_CARDFIX_RE(damage, fix) \
	{ (damage) = (damage) - (int64)(((damage) * (100 - max(0, 100 + (fix)))) / 100); }
				...
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					APPLY_CARDFIX_RE(damage, sd->indexed_bonus.magic_addele[tstatus->def_ele] + sd->indexed_bonus.magic_addele[ELE_ALL] + sd->indexed_bonus.magic_addele_script[tstatus->def_ele] + sd->indexed_bonus.magic_addele_script[ELE_ALL]);
				}
			}
...
// Pre-renewal / old renewal behaviour
				...
				if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					cardfix = cardfix * (100 + sd->indexed_bonus.magic_addele[tstatus->def_ele] + sd->indexed_bonus.magic_addele[ELE_ALL] + sd->indexed_bonus.magic_addele_script[tstatus->def_ele] + sd->indexed_bonus.magic_addele_script[ELE_ALL]) / 100;
		...
		case BF_WEAPON:
			...
			if (sd && !nk[NK_IGNOREATKCARD] && (left & 2)) {
				...
				if (sd->state.arrow_atk) { // Ranged attack
					cardfix = cardfix * (100 + sd->right_weapon.addrace[tstatus->race] + sd->indexed_bonus.arrow_addrace[tstatus->race] + sd->right_weapon.addrace[RC_ALL] + sd->indexed_bonus.arrow_addrace[RC_ALL]) / 100;
					if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
					...
					cardfix = cardfix * (100 + sd->right_weapon.addsize[tstatus->size] + sd->indexed_bonus.arrow_addsize[tstatus->size] + sd->right_weapon.addsize[SZ_ALL] + sd->indexed_bonus.arrow_addsize[SZ_ALL]) / 100;
					...
					cardfix = cardfix * (100 + race_fix) / 100;
					cardfix = cardfix * (100 + sd->right_weapon.addclass[tstatus->class_] + sd->indexed_bonus.arrow_addclass[tstatus->class_] + sd->right_weapon.addclass[CLASS_ALL] + sd->indexed_bonus.arrow_addclass[CLASS_ALL]) / 100;
				} else { // Melee attack
					int32 skill = 0;
					...
					// Calculates right & left hand weapon as unity
						...
						int32 ele_fix = sd->right_weapon.addele[tstatus->def_ele] + sd->left_weapon.addele[tstatus->def_ele] + sd->right_weapon.addele[ELE_ALL] + sd->left_weapon.addele[ELE_ALL];
						...
						cardfix = cardfix * (100 + ele_fix) / 100;
						//}
						cardfix = cardfix * (100 + sd->right_weapon.addrace[tstatus->race] + sd->left_weapon.addrace[tstatus->race] + sd->right_weapon.addrace[RC_ALL] + sd->left_weapon.addrace[RC_ALL]) / 100;
						cardfix = cardfix * (100 + sd->right_weapon.addsize[tstatus->size] + sd->left_weapon.addsize[tstatus->size] + sd->right_weapon.addsize[SZ_ALL] + sd->left_weapon.addsize[SZ_ALL]) / 100;
						for (const auto& raceit : t_race2)
							cardfix = cardfix * (100 + sd->right_weapon.addrace2[raceit] + sd->left_weapon.addrace2[raceit]) / 100;
						cardfix = cardfix * (100 + sd->right_weapon.addclass[tstatus->class_] + sd->left_weapon.addclass[tstatus->class_] + sd->right_weapon.addclass[CLASS_ALL] + sd->left_weapon.addclass[CLASS_ALL]) / 100;
					}
#ifndef RENEWAL
...
bool battle_status_block_damage(block_list* src, block_list* target, status_change* sc, struct Damage* d, int64& damage, uint16 skill_id, uint16 skill_lv) {
	...
	if ((sce = sc->getSCE(SC_AUTOGUARD)) && flag & BF_WEAPON && rnd() % 100 < sce->val2 && !skill_get_inf2(skill_id, INF2_IGNOREAUTOGUARD)) {
		...
		if (sce_d && (d_bl = map_id2bl(sce_d->val1)) &&
		    ...
		    check_distance_bl(target, d_bl, sce_d->val3)) { //If player is target of devotion, show guard effect on the devotion caster rather than the target
...
int64 battle_calc_damage(block_list* src, block_list* bl, struct Damage* d, int64 damage, uint16 skill_id, uint16 skill_lv) {
	...
	if (map_getcell(bl->m, bl->x, bl->y, CELL_CHKMAELSTROM) && skill_id && skill_get_type(skill_id) != BF_MISC && skill_get_casttype(skill_id) == CAST_GROUND)
		return 0;
	...
	if (battle_config.skill_min_damage && damage > 0 && damage < div_) {
		if ((flag & BF_WEAPON && battle_config.skill_min_damage & 1) || (flag & BF_MAGIC && battle_config.skill_min_damage & 2) || (flag & BF_MISC && battle_config.skill_min_damage & 4))
			damage = div_;
	}
...
static int32 battle_range_type(const block_list* src, const block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	if (battle_config.skillrange_by_distance &&
	    (src->type & battle_config.skillrange_by_distance)) { //based on distance between src/target [Skotlex]
...
static int32 is_attack_piercing(struct Damage* wd, block_list* src, const block_list* target, int32 skill_id, int32 skill_lv, int16 weapon_position) {
	...
	if (src != nullptr) {
		...
		if (skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN && skill_id != KO_HAPPOKUNAI
			...
			if (sd && (sd->right_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) || sd->right_weapon.def_ratio_atk_ele & (1 << ELE_ALL) || sd->right_weapon.def_ratio_atk_race & (1 << tstatus->race) || sd->right_weapon.def_ratio_atk_race & (1 << RC_ALL) || sd->right_weapon.def_ratio_atk_class & (1 << tstatus->class_) || sd->right_weapon.def_ratio_atk_class & (1 << CLASS_ALL)))
				if (weapon_position == EQI_HAND_R)
					return 1;

			if (sd && (sd->left_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) || sd->left_weapon.def_ratio_atk_ele & (1 << ELE_ALL) || sd->left_weapon.def_ratio_atk_race & (1 << tstatus->race) || sd->left_weapon.def_ratio_atk_race & (1 << RC_ALL) || sd->left_weapon.def_ratio_atk_class & (1 << tstatus->class_) || sd->left_weapon.def_ratio_atk_class & (1 << CLASS_ALL))) { //Pass effect onto right hand if configured so. [Skotlex]
...
static bool attack_ignores_def(Damage* wd, block_list* src, const block_list* target, uint16 skill_id, uint16 skill_lv, int16 weapon_position) {
	...
	if (skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS) { //Ignore Defense?
		if (sd && (sd->right_weapon.ignore_def_ele & (1 << tstatus->def_ele) || sd->right_weapon.ignore_def_ele & (1 << ELE_ALL) || sd->right_weapon.ignore_def_race & (1 << tstatus->race) || sd->right_weapon.ignore_def_race & (1 << RC_ALL) || sd->right_weapon.ignore_def_class & (1 << tstatus->class_) || sd->right_weapon.ignore_def_class & (1 << CLASS_ALL)))
			if (weapon_position == EQI_HAND_R)
				return true;

		if (sd && (sd->left_weapon.ignore_def_ele & (1 << tstatus->def_ele) || sd->left_weapon.ignore_def_ele & (1 << ELE_ALL) || sd->left_weapon.ignore_def_race & (1 << tstatus->race) || sd->left_weapon.ignore_def_race & (1 << RC_ALL) || sd->left_weapon.ignore_def_class & (1 << tstatus->class_) || sd->left_weapon.ignore_def_class & (1 << CLASS_ALL))) {
...
#define ATK_RATE(damage, damage2, a)              \
	do {                                          \
		int64 rate_ = (a);                        \
		(damage) = (damage) * rate_ / 100;        \
		if (is_attack_left_handed(src, skill_id)) \
			(damage2) = (damage2) * rate_ / 100;  \
	} while (0);
#define ATK_RATE2(damage, damage2, a, b)          \
	do {                                          \
		int64 rate_ = (a), rate2_ = (b);          \
		(damage) = (damage) * rate_ / 100;        \
		if (is_attack_left_handed(src, skill_id)) \
			(damage2) = (damage2) * rate2_ / 100; \
	} while (0);
#define ATK_RATER(damage, a) \
	{ (damage) = (damage) * (a) / 100; }
#define ATK_RATEL(damage2, a) \
	{ (damage2) = (damage2) * (a) / 100; }
//Adds dmg%. 100 = +100% (double) damage. 10 = +10% damage
#define ATK_ADDRATE(damage, damage2, a)           \
	do {                                          \
		int64 rate_ = (a);                        \
		(damage) += (damage) * rate_ / 100;       \
		if (is_attack_left_handed(src, skill_id)) \
...
#define ATK_ADDRATE2(damage, damage2, a, b)        \
	do {                                           \
		int64 rate_ = (a), rate2_ = (b);           \
		(damage) += (damage) * rate_ / 100;        \
		if (is_attack_left_handed(src, skill_id))  \
...
#define ATK_ADD(damage, damage2, a)               \
	do {                                          \
		int64 rate_ = (a);                        \
		(damage) += rate_;                        \
		if (is_attack_left_handed(src, skill_id)) \
...
#define ATK_ADD2(damage, damage2, a, b)           \
	do {                                          \
		int64 rate_ = (a), rate2_ = (b);          \
		(damage) += rate_;                        \
		if (is_attack_left_handed(src, skill_id)) \
...
#define RE_ALLATK_ADD(wd, a)                              \
	do {                                                  \
		int64 a_ = (a);                                   \
		ATK_ADD((wd)->statusAtk, (wd)->statusAtk2, a_);   \
		ATK_ADD((wd)->weaponAtk, (wd)->weaponAtk2, a_);   \
		ATK_ADD((wd)->equipAtk, (wd)->equipAtk2, a_);     \
		ATK_ADD((wd)->masteryAtk, (wd)->masteryAtk2, a_); \
	} while (0);
#define RE_ALLATK_RATE(wd, a)                              \
	do {                                                   \
		int64 a_ = (a);                                    \
		ATK_RATE((wd)->statusAtk, (wd)->statusAtk2, a_);   \
		ATK_RATE((wd)->weaponAtk, (wd)->weaponAtk2, a_);   \
		ATK_RATE((wd)->equipAtk, (wd)->equipAtk2, a_);     \
		ATK_RATE((wd)->masteryAtk, (wd)->masteryAtk2, a_); \
	} while (0);
#define RE_ALLATK_ADDRATE(wd, a)                              \
	do {                                                      \
		int64 a_ = (a);                                       \
		ATK_ADDRATE((wd)->statusAtk, (wd)->statusAtk2, a_);   \
		ATK_ADDRATE((wd)->weaponAtk, (wd)->weaponAtk2, a_);   \
		ATK_ADDRATE((wd)->equipAtk, (wd)->equipAtk2, a_);     \
		ATK_ADDRATE((wd)->masteryAtk, (wd)->masteryAtk2, a_); \
	} while (0);
#else
#define RE_ALLATK_ADD(wd, a) \
	{ ; }
...
static void battle_calc_attack_masteries(struct Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	if (sd && battle_skill_stacks_masteries_vvs(skill_id, BCHK_ALL)) { //Add mastery damage
		uint16 skill;
...
static void battle_calc_skill_base_damage(struct Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	switch (skill_id) { //Calc base damage according to skill
...
#endif
				                                sstatus->batk +
				                            sstatus->rhw.atk + (index >= 0 && sd->inventory_data[index] ? sd->inventory_data[index]->atk : 0)) *
				    (skill_lv + 5) / 5;
		...
		default:
			...
			if (is_attack_critical(wd, src, target, skill_id, skill_lv, false))
				bflag |= BDMG_CRIT;
			if (!skill_id && sc && sc->getSCE(SC_CHANGE))
				bflag |= BDMG_MAGIC;
#ifdef RENEWAL
			if (sd)
				battle_calc_damage_parts(wd, src, target, skill_id, skill_lv);
...
#define DAMAGE_DIV_FIX2(dmg, div) \
	{                             \
		if ((div) > 1)            \
			(dmg) *= div;         \
	}
...
static void battle_calc_multi_attack(struct Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	if (sd && !skill_id) { // if no skill_id passed, check for double attack [helvetica]
		int16 i;
		if (sc && sc->getSCE(SC_FEARBREEZE) && sd->weapontype1 == W_BOW && (i = sd->equip_index[EQI_AMMO]) >= 0 && sd->inventory_data[i] && sd->inventory.u.items_inventory[i].amount > 1) {
		...
		if (wd->div_ == 1 && ((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER) || (pc_checkskill_flag(*sd, TF_DOUBLE) > SKILL_FLAG_PERMANENT && sd->weapontype1 != W_FIST) || (sd->bonus.double_rate > 0 && sd->weapontype1 != W_FIST) // Will fail bare-handed
...
static void battle_calc_weapon_final_atk_modifiers(struct Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	if (wd->damage && tsc && tsc->getSCE(SC_REJECTSWORD) &&
	    (src->type != BL_PC || (((TBL_PC*)src)->weapontype1 == W_DAGGER || ((TBL_PC*)src)->weapontype1 == W_1HSWORD || ((TBL_PC*)src)->status.weapon == W_2HSWORD)) &&
...
void battle_do_reflect(int32 attack_type, struct Damage* wd, block_list* src, block_list* target, uint16 skill_id, uint16 skill_lv) {
	...
	if ((wd->damage + wd->damage2) && src && target && src != target && (src->type != BL_SKILL || (src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM)))) {
		int64 damage = wd->damage + wd->damage2, rdamage = 0;
		map_session_data* tsd = BL_CAST(BL_PC, target);
		status_change* tsc = status_get_sc(target);
		status_data* sstatus = status_get_status_data(*src);
...
#define MATK_RATE(a) \
	{ ad.damage = ad.damage * (a) / 100; }
//Adds dmg%. 100 = +100% (double) damage. 10 = +10% damage
#define MATK_ADDRATE(a) \
	{ ad.damage += ad.damage * (a) / 100; }
//Adds an absolute value to damage. 100 = +100 damage
#define MATK_ADD(a) \
	{ ad.damage += a; }
...
#ifdef RENEWAL
				i = 10 * skill_lv + sstatus->luk + sstatus->int_ + status_get_lv(src) + 300 - 300 * tstatus->hp / tstatus->max_hp;
#else
				i = 20 * skill_lv + sstatus->luk + sstatus->int_ + status_get_lv(src) + 200 - 200 * tstatus->hp / tstatus->max_hp;
#endif
				if (i > 700)
					i = 700;
		...
		if (sd) {
			...
			//Ignore Defense?
			if (!flag.imdef && (sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) || sd->bonus.ignore_mdef_ele & (1 << ELE_ALL) || sd->bonus.ignore_mdef_race & (1 << tstatus->race) || sd->bonus.ignore_mdef_race & (1 << RC_ALL) || sd->bonus.ignore_mdef_class & (1 << tstatus->class_) || sd->bonus.ignore_mdef_class & (1 << CLASS_ALL)))
				flag.imdef = 1;
		}
			...
			if (tsc->getSCE(SC_REFLECTSHIELD)) {
				...
				if ((sce_d = tsc->getSCE(SC_DEVOTION)) && (d_bl = map_id2bl(sce_d->val1)) &&
				    ((d_bl->type == BL_MER && ((TBL_MER*)d_bl)->master && ((TBL_MER*)d_bl)->master->id == tbl->id) ||
				        (d_bl->type == BL_PC && ((TBL_PC*)d_bl)->devotion[sce_d->val2] == tbl->id))) { //Don't reflect non-skill attack if has SC_REFLECTSHIELD from Devotion bonus inheritance
...
enum damage_lv battle_weapon_attack(block_list* src, block_list* target, t_tick tick, int32 flag) {
	...
	if (tsc && tsc->getSCE(SC_BLADESTOP_WAIT) &&
		...
		if (sc_start4(src, src, SC_BLADESTOP, 100, sd ? pc_checkskill(sd, MO_BLADESTOP) : 5, 0, 0, target->id, duration)) { //Target locked.
			clif_damage(*src, *target, tick, sstatus->amotion, 1, 0, 1, DMG_NORMAL, 0, false);                              //Display MISS.
	...
	if (tsc) {
		if (tsc->getSCE(SC_DEVOTION)) {
			...
			if (d_bl && ((d_bl->type == BL_MER && ((TBL_MER*)d_bl)->master && ((TBL_MER*)d_bl)->master->id == target->id) || (d_bl->type == BL_PC && ((TBL_PC*)d_bl)->devotion[sce->val2] == target->id)) && check_distance_bl(target, d_bl, sce->val3)) {
...
int32 battle_check_target(const block_list* src, const block_list* target, int32 flag) {
	...
	switch (target->type) { // Checks on actual target
		...
		case BL_MOB: {
			...
			if ((((md->special_state.ai == AI_SPHERE && battle_config.alchemist_summon_setting & 2) || //Marine Spheres
			         (md->special_state.ai == AI_FLORA && battle_config.alchemist_summon_setting & 1)) &&
			        s_bl->type == BL_PC && src->type != BL_MOB) ||                                                            //Floras
			    (md->special_state.ai == AI_ZANZOU && t_bl->id != s_bl->id) ||                                                //Zanzou
			    (md->special_state.ai == AI_FAW && (t_bl->id != s_bl->id || (s_bl->type == BL_PC && src->type != BL_MOB)))) { //Targettable by players
				state |= BCT_ENEMY;
				strip_enemy = 0;
			}
			break;
	...
	if (mapdata_flag_vs(mapdata)) { //Check rivalry settings.
		...
		if (state & BCT_ENEMY && battle_config.pk_mode && !mapdata_flag_gvg(mapdata) && s_bl->type == BL_PC && t_bl->type == BL_PC) { // Prevent novice engagement on pk_mode (feature by Valaris)
			const map_session_data* sd = static_cast<const map_session_data*>(s_bl);
...
static const struct _battle_data {
    ...
    {
        "warp_point_debug",
        &battle_config.warp_point_debug,
        0,
        0,
        1,
    },
    ...
    {
        "mob_critical_rate",
        &battle_config.mob_critical_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "enable_baseatk",
        &battle_config.enable_baseatk,
        BL_CHAR | BL_HOM,
        BL_NUL,
        BL_ALL,
    },
    {
        "enable_baseatk_renewal",
        &battle_config.enable_baseatk_renewal,
        BL_ALL,
        BL_NUL,
        BL_ALL,
    },
    ...
    {
        "delay_dependon_dex",
        &battle_config.delay_dependon_dex,
        0,
        0,
        1,
    },
    {
        "delay_dependon_agi",
        &battle_config.delay_dependon_agi,
        0,
        0,
        1,
    },
    ...
    {
        "cardfix_monster_physical",
        &battle_config.cardfix_monster_physical,
        1,
        0,
        1,
    },
    ...
    {
        "skillrange_by_distance",
        &battle_config.skillrange_by_distance,
        ~BL_PC,
        BL_NUL,
        BL_ALL,
    },
    {
        ...
        &battle_config.use_weapon_skill_range,
        BL_NUL,
        BL_NUL,
        BL_ALL,
    },
    ...
    },
    {
        ...
        &battle_config.clear_unit_onwarp,
        BL_ALL,
        BL_NUL,
        BL_ALL,
    },
    ...
    },
    {
        ...
        &battle_config.first_attack_loot_bonus,
        30,
        0,
        100,
    },
    {
        "mvp_to_loot_priority",
        &battle_config.mvp_to_loot_priority,
        0,
        0,
        1,
    },
    {
        "item_first_get_time",
        &battle_config.item_first_get_time,
        3000,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.item_second_get_time,
        2000,
        0,
        INT_MAX,
    },
    ...
    {
        "death_penalty_base",
        &battle_config.death_penalty_base,
        0,
        0,
        INT_MAX,
    },
    {
        "death_penalty_job",
        &battle_config.death_penalty_job,
        0,
        0,
        INT_MAX,
    },
    ...
    {
        "chase_range_rate",
        &battle_config.chase_range_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.guild_skill_relog_delay,
        300000,
        0,
        INT_MAX,
    },
#ifdef RENEWAL
    {
        ...
        &battle_config.guild_skill_relog_type,
        0,
        0,
        1,
    },
    ...
    {
        "atcommand_slave_clone_limit",
        &battle_config.atc_slave_clone_limit,
        25,
        0,
        INT_MAX,
    },
    {
        "partial_name_scan",
        &battle_config.partial_name_scan,
        0,
        0,
        1,
    },
    ...
    {
        "monster_loot_type",
        &battle_config.monster_loot_type,
        0,
        0,
        1,
    },
    ...
    {
        "plant_spawn_delay",
        &battle_config.plant_spawn_delay,
        100,
        0,
        INT_MAX,
    },
    {
        "boss_spawn_delay",
        &battle_config.boss_spawn_delay,
        100,
        0,
        INT_MAX,
    },
    {
        "no_spawn_on_player",
        &battle_config.no_spawn_on_player,
        0,
        0,
        100,
    },
    {
        "force_random_spawn",
        &battle_config.force_random_spawn,
        0,
        0,
        1,
    },
    {
        "slaves_inherit_mode",
        &battle_config.slaves_inherit_mode,
        4,
        0,
        4,
    },
    {
        "slaves_inherit_speed",
        &battle_config.slaves_inherit_speed,
        3,
        0,
        3,
    },
    {
        "summons_trigger_autospells",
        &battle_config.summons_trigger_autospells,
        1,
        0,
        1,
    },
    {
        "pc_damage_walk_delay_rate",
        &battle_config.pc_walk_delay_rate,
        20,
        0,
        INT_MAX,
    },
    {
        "damage_walk_delay_rate",
        &battle_config.walk_delay_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "quest_skill_learn",
        &battle_config.quest_skill_learn,
        0,
        0,
        1,
    },
    {
        "quest_skill_reset",
        &battle_config.quest_skill_reset,
        0,
        0,
        1,
    },
    ...
    {
        "pet_friendly_rate",
        &battle_config.pet_friendly_rate,
        100,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.pet_hungry_delay_rate,
        100,
        10,
        INT_MAX,
    },
    {
        ...
        &battle_config.pet_hungry_friendly_decrease,
        5,
        0,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.pet_support_min_friendly,
        900,
        0,
        950,
    },
    {
        "pet_support_rate",
        &battle_config.pet_support_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "pet_attack_exp_rate",
        &battle_config.pet_attack_exp_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.finger_offensive_type,
        0,
        0,
        1,
    },
    ...
    {
        "resurrection_exp",
        &battle_config.resurrection_exp,
        0,
        0,
        INT_MAX,
    },
    ...
    {
        "combo_delay_rate",
        &battle_config.combo_delay_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "item_use_interval",
        &battle_config.item_use_interval,
        100,
        0,
        INT_MAX,
    },
    {
        "cashfood_use_interval",
        &battle_config.cashfood_use_interval,
        60000,
        0,
        INT_MAX,
    },
    ...
    {
        "xmas_ignorepalette",
        &battle_config.xmas_ignorepalette,
        0,
        0,
        1,
    },
    {
        "summer_ignorepalette",
        &battle_config.summer_ignorepalette,
        0,
        0,
        1,
    },
    {
        "hanbok_ignorepalette",
        &battle_config.hanbok_ignorepalette,
        0,
        0,
        1,
    },
    {
        "oktoberfest_ignorepalette",
        &battle_config.oktoberfest_ignorepalette,
        0,
        0,
        1,
    },
    ...
    {
        "ammo_check_weapon",
        &battle_config.ammo_check_weapon,
        1,
        0,
        1,
    },
    ...
    {
        "max_summoner_aspd",
        &battle_config.max_summoner_aspd,
        193,
        100,
        199,
    },
    {
        "max_walk_speed",
        &battle_config.max_walk_speed,
        300,
        100,
        100 * DEFAULT_WALK_SPEED,
    },
    {
        "max_lv",
    ...
    {
        "undead_detect_type",
        &battle_config.undead_detect_type,
        0,
        0,
        3,
    },
    {
        "auto_counter_type",
        &battle_config.auto_counter_type,
        BL_ALL,
        BL_NUL,
        BL_ALL,
    },
    ...
    {
        "agi_penalty_target",
        &battle_config.agi_penalty_target,
        BL_PC,
        BL_NUL,
        BL_ALL,
    },
    ...
    {
        "agi_penalty_count",
        &battle_config.agi_penalty_count,
        3,
        2,
        INT_MAX,
    },
    ...
    {
        "vit_penalty_target",
        &battle_config.vit_penalty_target,
        BL_PC,
        BL_NUL,
        BL_ALL,
    },
    {
        "vit_penalty_type",
        &battle_config.vit_penalty_type,
        1,
        0,
        2,
    },
    {
        "vit_penalty_count",
        &battle_config.vit_penalty_count,
        3,
        2,
        INT_MAX,
    },
    ...
    {
        "skill_reiteration",
        &battle_config.skill_reiteration,
        BL_NUL,
        BL_NUL,
        BL_ALL,
    },
    ...
    },
    {
        ...
        &battle_config.pc_cloak_check_type,
        1,
        0,
        1 | 2 | 4,
    },
    ...
    {
        "gvg_weapon_attack_damage_rate",
        &battle_config.gvg_weapon_damage_rate,
        60,
        0,
        INT_MAX,
    },
    ...
    {
        "gvg_flee_penalty",
        &battle_config.gvg_flee_penalty,
        20,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.pk_short_damage_rate,
        80,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.pk_long_damage_rate,
        70,
        0,
        INT_MAX,
    },
    {
        "pk_weapon_attack_damage_rate",
        &battle_config.pk_weapon_damage_rate,
        60,
        0,
        INT_MAX,
    },
    {
        "pk_magic_attack_damage_rate",
        &battle_config.pk_magic_damage_rate,
        60,
        0,
        INT_MAX,
    },
    {
        "pk_misc_attack_damage_rate",
        &battle_config.pk_misc_damage_rate,
        60,
        0,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.attack_direction_change,
        BL_ALL,
        BL_NUL,
        BL_ALL,
    },
    {
        "land_skill_limit",
        &battle_config.land_skill_limit,
        BL_ALL,
        BL_NUL,
        BL_ALL,
    },
    ...
    {
        "display_skill_fail",
        &battle_config.display_skill_fail,
        2,
        0,
        1 | 2 | 4 | 8,
    },
    ...
    {
        "dead_branch_active",
        &battle_config.dead_branch_active,
        1,
        0,
        1,
    },
    ...
    {
        "vending_over_max",
        &battle_config.vending_over_max,
        1,
        0,
        1,
    },
    ...
    {
        "party_hp_mode",
        &battle_config.party_hp_mode,
        0,
        0,
        1,
    },
    {
        "show_party_share_picker",
        &battle_config.party_show_share_picker,
        1,
        0,
        1,
    },
    {
        "show_picker.item_type",
        &battle_config.show_picker_item_type,
        112,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.party_update_interval,
        1000,
        100,
        INT_MAX,
    },
    {
        "party_item_share_type",
        &battle_config.party_share_type,
        0,
        0,
        1 | 2 | 3,
    },
    ...
    },
    {
        ...
        &battle_config.invite_request_check,
        1,
        0,
        1,
    },
    ...
    {
        "skill_wall_check",
        &battle_config.skill_wall_check,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.custom_cell_stack_limit,
        1,
        1,
        255,
    },
    {
        "dancing_weaponswitch_fix",
        &battle_config.dancing_weaponswitch_fix,
        1,
        0,
        1,
    },
    ...
    // eAthena additions
    {
        "item_logarithmic_drops",
        &battle_config.logarithmic_drops,
        0,
        0,
        1,
    },
    {
        ...
        &battle_config.item_drop_common_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_common_max",
        &battle_config.item_drop_common_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_equip_min",
        &battle_config.item_drop_equip_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_equip_max",
        &battle_config.item_drop_equip_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_card_min",
        &battle_config.item_drop_card_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_card_max",
        &battle_config.item_drop_card_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_mvp_min",
        &battle_config.item_drop_mvp_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_mvp_max",
        &battle_config.item_drop_mvp_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_mvp_mode",
        &battle_config.item_drop_mvp_mode,
        0,
        0,
        2,
    },
    {
        "item_drop_heal_min",
        &battle_config.item_drop_heal_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_heal_max",
        &battle_config.item_drop_heal_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_use_min",
        &battle_config.item_drop_use_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_use_max",
        &battle_config.item_drop_use_max,
        10000,
        1,
        10000,
    },
    {
        "item_drop_add_min",
        &battle_config.item_drop_adddrop_min,
        1,
        0,
        10000,
    },
    ...
    {
        "item_group_drop_min",
        &battle_config.item_group_drop_min,
        1,
        0,
        10000,
    },
    ...
    {
        "item_drop_treasure_min",
        &battle_config.item_drop_treasure_min,
        1,
        0,
        10000,
    },
    {
        "item_drop_treasure_max",
    ...
    },
    {
        ...
        &battle_config.item_rate_common_boss,
        100,
        0,
        1000000,
    },
    ...
    },
    {
        ...
        &battle_config.item_rate_equip_boss,
        100,
        0,
        1000000,
    },
    {
        "item_rate_equip_mvp",
        &battle_config.item_rate_equip_mvp,
        100,
        0,
        1000000,
    },
    ...
    {
        "item_rate_card_boss",
        &battle_config.item_rate_card_boss,
        100,
        0,
        1000000,
    },
    {
        "item_rate_card_mvp",
        &battle_config.item_rate_card_mvp,
        100,
        0,
        1000000,
    },
    ...
    {
        "item_rate_heal_mvp",
        &battle_config.item_rate_heal_mvp,
        100,
        0,
        1000000,
    },
    ...
    {
        "item_rate_use_mvp",
        &battle_config.item_rate_use_mvp,
        100,
        0,
        1000000,
    },
    {
        "item_rate_adddrop",
        &battle_config.item_rate_adddrop,
        100,
        0,
        1000000,
    },
    ...
    {
        "item_rate_treasure",
        &battle_config.item_rate_treasure,
        100,
        0,
        1000000,
    },
    ...
    {
        "prevent_logout_trigger",
        &battle_config.prevent_logout_trigger,
        0xE,
        0,
        0xF,
    },
    {
        "alchemist_summon_reward",
        &battle_config.alchemist_summon_reward,
        1,
        0,
        2,
    },
    ...
    },
    {
        ...
        &battle_config.equip_natural_break_rate,
        0,
        0,
        INT_MAX,
    },
    {
        "equip_self_break_rate",
        &battle_config.equip_self_break_rate,
        100,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.equip_skill_break_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "pet_unequip_destroy",
        &battle_config.pet_unequip_destroy,
        1,
        0,
        1,
    },
    ...
    {
        "multi_level_up_base",
        &battle_config.multi_level_up_base,
        0,
        0,
        MAX_LEVEL,
    },
    {
        "multi_level_up_job",
        &battle_config.multi_level_up_job,
        0,
        0,
        MAX_LEVEL,
    },
    {
        "max_exp_gain_rate",
        &battle_config.max_exp_gain_rate,
        0,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.backstab_bow_penalty,
        0,
        0,
        1,
    },
    ...
    {
        "vcast_stat_scale",
        &battle_config.vcast_stat_scale,
        530,
        1,
        INT_MAX,
    },
    ...
    {
        "mobs_level_up",
        &battle_config.mobs_level_up,
        0,
        0,
        1,
    },
    {
        "mobs_level_up_exp_rate",
        &battle_config.mobs_level_up_exp_rate,
        1,
        1,
        INT_MAX,
    },
    ...
    {
        "skill_steal_random_options",
        &battle_config.skill_steal_random_options,
        0,
        0,
        1,
    },
    ...
    {
        "exp_bonus_attacker",
        &battle_config.exp_bonus_attacker,
        25,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.exp_bonus_max_attacker,
        12,
        2,
        INT_MAX,
    },
    {
        "exp_bonus_nodamage_attacker",
        &battle_config.exp_bonus_nodamage_attacker,
        0,
        0,
        1,
    },
    ...
    {
        "amotion_min_skill_delay",
        &battle_config.amotion_min_skill_delay,
        0,
        0,
        1,
    },
    {
        "default_walk_delay",
        &battle_config.default_walk_delay,
        300,
        0,
        INT_MAX,
    },
    ...
    {
        "attack_walk_delay",
        &battle_config.attack_walk_delay,
        BL_NUL,
        BL_NUL,
        BL_ALL,
    },
    {
        "damage_walk_delay",
        &battle_config.damage_walk_delay,
        BL_NUL,
        BL_NUL,
        BL_ALL,
    },
    ...
    {
        "idle_no_share",
        &battle_config.idle_no_share,
        0,
        0,
        INT_MAX,
    },
    {
        "party_even_share_bonus",
        &battle_config.party_even_share_bonus,
        0,
        0,
        INT_MAX,
    },
    {
        "delay_battle_damage",
        &battle_config.delay_battle_damage,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.display_hallucination,
        1,
        0,
        1,
    },
    {
        "use_statpoint_table",
        &battle_config.use_statpoint_table,
        1,
        0,
        1,
    },
    {
        "debuff_on_logout",
        &battle_config.debuff_on_logout,
        0,
        0,
        1 | 2,
    },
    ...
    {
        "mob_remove_damaged",
        &battle_config.mob_remove_damaged,
        1,
        0,
        1,
    },
    {
        "show_hp_sp_drain",
        &battle_config.show_hp_sp_drain,
        0,
        0,
        1,
    },
    ...
    {
        "mob_npc_event_type",
        &battle_config.mob_npc_event_type,
        1,
        0,
        1,
    },
    ...
    {
        "mob_max_skilllvl",
        &battle_config.mob_max_skilllvl,
        MAX_MOBSKILL_LEVEL,
        1,
        MAX_MOBSKILL_LEVEL,
    },
    {
        "retaliate_to_master",
        &battle_config.retaliate_to_master,
        1,
        0,
        1,
    },
    ...
    {
        "drop_rate_cap_vip",
        &battle_config.drop_rate_cap_vip,
        9000,
        0,
        10000,
    },
    ...
    {
        "duel_allow_teleport",
        &battle_config.duel_allow_teleport,
        0,
        0,
        1,
    },
    {
        "duel_autoleave_when_die",
        &battle_config.duel_autoleave_when_die,
        1,
        0,
        1,
    },
    {
        "duel_time_interval",
        &battle_config.duel_time_interval,
        60,
        0,
        INT_MAX,
    },
    {
        "duel_only_on_same_map",
        &battle_config.duel_only_on_same_map,
        0,
        0,
        1,
    },
    {
        ...
        &battle_config.skip_teleport_lv1_menu,
        0,
        0,
        1,
    },
    {
        "allow_skill_without_day",
    ...
    {
        "skill_caster_check",
        &battle_config.skill_caster_check,
        1,
        0,
        1,
    },
    ...
    },
    {
        ...
        &battle_config.sg_angel_skill_ratio,
        10,
        0,
        10000,
    },
    ...
    {
        "override_mob_names",
        &battle_config.override_mob_names,
        0,
        0,
        2,
    },
    ...
    {
        "hom_rename",
        &battle_config.hom_rename,
        0,
        0,
        1,
    },
    {
        "homunculus_show_growth",
        &battle_config.homunculus_show_growth,
        0,
        0,
        1,
    },
    {
        "homunculus_friendly_rate",
        &battle_config.homunculus_friendly_rate,
        100,
        0,
        INT_MAX,
    },
    ...
    {
        "mob_remove_delay",
        &battle_config.mob_remove_delay,
        60000,
        1000,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.sg_miracle_skill_duration,
        3600000,
        0,
        INT_MAX,
    },
    {
        "hvan_explosion_intimate",
        &battle_config.hvan_explosion_intimate,
        45000,
        0,
        100000,
    },
    ...
    {
        "at_mapflag",
        &battle_config.autotrade_mapflag,
        0,
        0,
        1,
    },
    ...
    {
        "homunculus_autoloot",
        &battle_config.homunculus_autoloot,
        0,
        0,
        1,
    },
    {
        "idle_no_autoloot",
        &battle_config.idle_no_autoloot,
        0,
        0,
        INT_MAX,
    },
    {
        "max_guild_alliance",
        &battle_config.max_guild_alliance,
        3,
        0,
        3,
    },
    ...
    {
        "auction_feeperhour",
        &battle_config.auction_feeperhour,
        12000,
        0,
        INT_MAX,
    },
    {
        "auction_maximumprice",
        &battle_config.auction_maximumprice,
        500000000,
        0,
        MAX_ZENY,
    },
    {
        "homunculus_auto_vapor",
        &battle_config.homunculus_auto_vapor,
        80,
        0,
        100,
    },
    {
        "display_status_timers",
        &battle_config.display_status_timers,
        1,
        0,
        1,
    },
    {
        "skill_add_heal_rate",
        &battle_config.skill_add_heal_rate,
        487,
        0,
        INT_MAX,
    },
    {
        "eq_single_target_reflectable",
        &battle_config.eq_single_target_reflectable,
        1,
        0,
        1,
    },
    {
        "invincible.nodamage",
        &battle_config.invincible_nodamage,
        0,
        0,
        1,
    },
    {
        "mob_slave_keep_target",
        &battle_config.mob_slave_keep_target,
        0,
        0,
        1,
    },
    {
        "autospell_check_range",
    ...
    {
        "knockback_left",
        &battle_config.knockback_left,
        1,
        0,
        1,
    },
    {
        "client_reshuffle_dice",
    ...
    {
        "client_sort_storage",
        &battle_config.client_sort_storage,
        0,
        0,
        1,
    },
    {
        "feature.buying_store",
        &battle_config.feature_buying_store,
        1,
        0,
        1,
    },
    {
        "feature.search_stores",
        &battle_config.feature_search_stores,
        1,
        0,
        1,
    },
    {
        "searchstore_querydelay",
        &battle_config.searchstore_querydelay,
        10,
        0,
        INT_MAX,
    },
    {
        "searchstore_maxresults",
        &battle_config.searchstore_maxresults,
        30,
        1,
        INT_MAX,
    },
    {
        "display_party_name",
        &battle_config.display_party_name,
        0,
        0,
        1,
    },
    {
        "cashshop_show_points",
        &battle_config.cashshop_show_points,
        0,
        0,
        1,
    },
    {
        "mail_show_status",
        &battle_config.mail_show_status,
        0,
        0,
        2,
    },
    {
        "client_limit_unit_lv",
        &battle_config.client_limit_unit_lv,
        0,
        0,
        BL_ALL,
    },
    {
        "land_protector_behavior",
    ...
    {
        "npc_emotion_behavior",
        &battle_config.npc_emotion_behavior,
        0,
        0,
        1,
    },
    ...
    {
        "bg_update_interval",
        &battle_config.bg_update_interval,
        1000,
        100,
        INT_MAX,
    },
    {
        "bg_short_attack_damage_rate",
        &battle_config.bg_short_damage_rate,
        80,
        0,
        INT_MAX,
    },
    {
        "bg_long_attack_damage_rate",
        &battle_config.bg_long_damage_rate,
        80,
        0,
        INT_MAX,
    },
    {
        "bg_weapon_attack_damage_rate",
        &battle_config.bg_weapon_damage_rate,
        60,
        0,
        INT_MAX,
    },
    {
        "bg_magic_attack_damage_rate",
    ...
    {
        "bg_misc_attack_damage_rate",
        &battle_config.bg_misc_damage_rate,
        60,
        0,
        INT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.max_baby_third_parameter,
        108,
        10,
        SHRT_MAX,
    },
    {
        "max_trans_parameter",
        &battle_config.max_trans_parameter,
        99,
        10,
        SHRT_MAX,
    },
    {
        ...
        &battle_config.max_third_trans_parameter,
        135,
        10,
        SHRT_MAX,
    },
    {
        ...
        &battle_config.max_extended_parameter,
        125,
        10,
        SHRT_MAX,
    },
    ...
    },
    {
        ...
        &battle_config.max_fourth_parameter,
        135,
        10,
        SHRT_MAX,
    },
    ...
    {
        "homunculus_max_level",
        &battle_config.hom_max_level,
        99,
        0,
        MAX_LEVEL,
    },
    {
        "homunculus_S_max_level",
        &battle_config.hom_S_max_level,
        150,
        0,
        MAX_LEVEL,
    },
    ...
    {
        "multi_trigger_trap",
        &battle_config.multi_trigger_trap,
        0,
        0,
        1,
    },
    {
        "allow_consume_restricted_item",
        &battle_config.allow_consume_restricted_item,
        1,
        0,
        1,
    },
    {
        "allow_equip_restricted_item",
    ...
    {
        "item_enabled_npc",
        &battle_config.item_enabled_npc,
        0,
        0,
        1,
    },
...
#endif
    {
        "item_flooritem_check",
        &battle_config.item_onfloor,
        1,
        0,
        1,
    },
    {
        "bowling_bash_area",
        &battle_config.bowling_bash_area,
        0,
        0,
        20,
    },
    {
        "drop_rateincrease",
        &battle_config.drop_rateincrease,
        0,
        0,
        1,
    },
...
#endif
    {
        "vip_base_exp_increase",
        &battle_config.vip_base_exp_increase,
        50,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.vip_job_exp_increase,
        50,
        0,
        INT_MAX,
    },
    {
        "vip_exp_penalty_base",
        &battle_config.vip_exp_penalty_base,
        100,
        0,
        INT_MAX,
    },
    {
        "vip_exp_penalty_job",
        &battle_config.vip_exp_penalty_job,
        100,
        0,
        INT_MAX,
    },
    {
        "vip_zeny_penalty",
        &battle_config.vip_zeny_penalty,
        0,
        0,
        INT_MAX,
    },
    ...
    {
        "vip_drop_increase",
        &battle_config.vip_drop_increase,
        50,
        0,
        INT_MAX,
    },
    ...
    {
        "vip_disp_rate",
        &battle_config.vip_disp_rate,
        1,
        0,
        1,
    },
    {
        "mon_trans_disable_in_gvg",
        &battle_config.mon_trans_disable_in_gvg,
        0,
        0,
        1,
    },
    {
        "homunculus_S_growth_level",
        &battle_config.hom_S_growth_level,
        99,
        0,
        MAX_LEVEL,
    },
    {
        "discount_item_point_shop",
        &battle_config.discount_item_point_shop,
        0,
        0,
        3,
    },
    {
        ...
        &battle_config.update_enemy_position,
        0,
        0,
        1,
    },
    {
        "devotion_rdamage",
        &battle_config.devotion_rdamage,
        0,
        0,
        100,
    },
    {
        "feature.autotrade",
        &battle_config.feature_autotrade,
        1,
        0,
        1,
    },
    {
        "feature.autotrade_direction",
        &battle_config.feature_autotrade_direction,
        4,
        -1,
        7,
    },
    {
        "feature.autotrade_head_direction",
        &battle_config.feature_autotrade_head_direction,
        0,
        -1,
        2,
    },
    {
        "feature.autotrade_sit",
        &battle_config.feature_autotrade_sit,
        1,
        -1,
        1,
    },
    {
        "feature.autotrade_open_delay",
        &battle_config.feature_autotrade_open_delay,
        5000,
        1000,
        INT_MAX,
    },
    {
        "disp_servervip_msg",
        &battle_config.disp_servervip_msg,
        0,
        0,
        1,
    },
    ...
    {
        "path_blown_halt",
        &battle_config.path_blown_halt,
        1,
        0,
        1,
    },
    {
        "rental_mount_speed_boost",
        &battle_config.rental_mount_speed_boost,
        25,
        0,
        100,
    },
    {
        "feature.warp_suggestions",
        &battle_config.warp_suggestions_enabled,
        0,
        0,
        1,
    },
    {
        "taekwon_mission_mobname",
        &battle_config.taekwon_mission_mobname,
        0,
        0,
        2,
    },
    {
        "teleport_on_portal",
        &battle_config.teleport_on_portal,
        0,
        0,
        1,
    },
    {
        "cart_revo_knockback",
        &battle_config.cart_revo_knockback,
        1,
        0,
        1,
    },
    {
        ...
        &battle_config.guild_notice_changemap,
        2,
        0,
        2,
    },
    {
        "transcendent_status_points",
        &battle_config.transcendent_status_points,
        52,
        1,
        INT_MAX,
    },
    {
        ...
        &battle_config.taekwon_ranker_min_lv,
        90,
        1,
        MAX_LEVEL,
    },
    {
        "revive_onwarp",
        &battle_config.revive_onwarp,
        1,
        0,
        1,
    },
    {
        "fame_taekwon_mission",
        &battle_config.fame_taekwon_mission,
        1,
        0,
        INT_MAX,
    },
    ...
    {
        "fame_pharmacy_10",
        &battle_config.fame_pharmacy_10,
        50,
        0,
        INT_MAX,
    },
    ...
    {
        "at_monsterignore",
        &battle_config.autotrade_monsterignore,
        0,
        0,
        1,
    },
    ...
    {
        "spawn_direction",
        &battle_config.spawn_direction,
        0,
        0,
        1,
    },
    {
        "arrow_shower_knockback",
        &battle_config.arrow_shower_knockback,
        1,
        0,
        1,
    },
    {
        "devotion_rdamage_skill_only",
    ...
    {
        "max_extended_aspd",
        &battle_config.max_extended_aspd,
        193,
        100,
        199,
    },
    {
        "monster_chase_refresh",
        &battle_config.mob_chase_refresh,
        32,
        0,
        MAX_WALKPATH,
    },
    {
        "mob_icewall_walk_block",
        &battle_config.mob_icewall_walk_block,
        75,
        0,
        255,
    },
    ...
    {
        "stormgust_knockback",
        &battle_config.stormgust_knockback,
        1,
        0,
        1,
    },
    ...
    {
        "default_bind_on_equip",
        &battle_config.default_bind_on_equip,
        BOUND_CHAR,
        BOUND_NONE,
        BOUND_MAX - 1,
    },
    {
        "pet_ignore_infinite_def",
        &battle_config.pet_ignore_infinite_def,
        0,
        0,
        1,
    },
    {
        "homunculus_evo_intimacy_need",
        &battle_config.homunculus_evo_intimacy_need,
        91100,
        0,
        INT_MAX,
    },
    {
        "homunculus_evo_intimacy_reset",
        &battle_config.homunculus_evo_intimacy_reset,
        1000,
        0,
        INT_MAX,
    },
    {
        "monster_loot_search_type",
        &battle_config.monster_loot_search_type,
        1,
        0,
        1,
    },
    {
        "feature.roulette",
        &battle_config.feature_roulette,
        1,
        0,
        1,
    },
    {
        "feature.roulette_bonus_reward",
        &battle_config.feature_roulette_bonus_reward,
        1,
        0,
        1,
    },
    {
        "monster_hp_bars_info",
        &battle_config.monster_hp_bars_info,
        1,
        0,
        1,
    },
    {
        "save_body_style",
        &battle_config.save_body_style,
        1,
        0,
        1,
    },
    {
        "monster_eye_range_bonus",
        &battle_config.mob_eye_range_bonus,
        0,
        0,
        10,
    },
    {
        "monster_stuck_warning",
        &battle_config.mob_stuck_warning,
        0,
        0,
        1,
    },
    {
        ...
        &battle_config.skill_eightpath_algorithm,
        1,
        0,
        1,
    },
    {
        "skill_eightpath_same_cell",
        &battle_config.skill_eightpath_same_cell,
        1,
        0,
        1,
    },
    {
        "death_penalty_maxlv",
        &battle_config.death_penalty_maxlv,
        0,
        0,
        3,
    },
    {
        "exp_cost_redemptio",
        &battle_config.exp_cost_redemptio,
        1,
        0,
        100,
    },
    {
        ...
        &battle_config.exp_cost_redemptio_limit,
        5,
        0,
        MAX_PARTY,
    },
    {
        "mvp_exp_reward_message",
        &battle_config.mvp_exp_reward_message,
        0,
        0,
        1,
    },
    {
        "can_damage_skill",
        &battle_config.can_damage_skill,
        1,
        0,
        BL_ALL,
    },
    ...
    },
    {
        ...
        &battle_config.atcommand_disable_npc,
        1,
        0,
        1,
    },
    {
        "block_account_in_same_party",
        &battle_config.block_account_in_same_party,
        1,
        0,
        1,
    },
    {
        "tarotcard_equal_chance",
        &battle_config.tarotcard_equal_chance,
        0,
        0,
        1,
    },
    {
        "change_party_leader_samemap",
        &battle_config.change_party_leader_samemap,
        1,
        0,
        1,
    },
    ...
    {
        "refresh_song_icon",
        &battle_config.refresh_song_icon,
        0,
        0,
        1,
    },
    {
        "guild_maprespawn_clones",
        &battle_config.guild_maprespawn_clones,
        0,
        0,
        1,
    },
    ...
    {
        "mail_daily_count",
        &battle_config.mail_daily_count,
        100,
        0,
        INT32_MAX,
    },
    {
        "mail_zeny_fee",
        &battle_config.mail_zeny_fee,
        2,
        0,
        100,
    },
    {
        "mail_attachment_price",
        &battle_config.mail_attachment_price,
        2500,
        0,
        INT32_MAX,
    },
    {
        ...
        &battle_config.mail_attachment_weight,
        2000,
        0,
        INT32_MAX,
    },
    {
        "banana_bomb_duration",
    ...
    {
        "guild_leaderchange_woe",
        &battle_config.guild_leaderchange_woe,
        0,
        0,
        1,
    },
    {
        "guild_alliance_onlygm",
        &battle_config.guild_alliance_onlygm,
        0,
        0,
        1,
    },
    {
        "feature.achievement",
        &battle_config.feature_achievement,
        1,
        0,
        1,
    },
    {
        "allow_bound_sell",
        &battle_config.allow_bound_sell,
        0,
        0,
        0xF,
    },
    {
        "autoloot_adjust",
        &battle_config.autoloot_adjust,
        0,
        0,
        1,
    },
    {
        "feature.petevolution",
        &battle_config.feature_petevolution,
        1,
        0,
        1,
    },
    ...
    {
        "feature.pet_autofeed_rate",
        &battle_config.feature_pet_autofeed_rate,
        89,
        0,
        100,
    },
    {
        "pet_autofeed_always",
        &battle_config.pet_autofeed_always,
        1,
        0,
        1,
    },
    {
        "broadcast_hide_name",
        &battle_config.broadcast_hide_name,
        2,
        0,
        NAME_LENGTH,
    },
    {
        "skill_drop_items_full",
        &battle_config.skill_drop_items_full,
        0,
        0,
        1,
    },
    {
        "switch_remove_edp",
        &battle_config.switch_remove_edp,
        2,
        0,
        3,
    },
    {
        "feature.homunculus_autofeed",
        &battle_config.feature_homunculus_autofeed,
        1,
        0,
        1,
    },
    {
        "feature.homunculus_autofeed_rate",
        &battle_config.feature_homunculus_autofeed_rate,
        30,
        0,
        100,
    },
    ...
    {
        "summoner_size",
        &battle_config.summoner_size,
        SZ_SMALL,
        SZ_SMALL,
        SZ_BIG,
    },
    {
        "homunculus_autofeed_always",
        &battle_config.homunculus_autofeed_always,
        1,
        0,
        1,
    },
    {
        "feature.attendance",
        &battle_config.feature_attendance,
        1,
        0,
        1,
    },
    {
        "feature.privateairship",
        &battle_config.feature_privateairship,
        1,
        0,
        1,
    },
    {
        "rental_transaction",
        &battle_config.rental_transaction,
        1,
        0,
        1,
    },
    ...
    {
        "feature.equipswitch",
        &battle_config.feature_equipswitch,
        1,
        0,
        1,
    },
    {
        "pet_walk_speed",
        &battle_config.pet_walk_speed,
        1,
        1,
        3,
    },
    {
        "blacksmith_fame_refine_threshold",
        &battle_config.blacksmith_fame_refine_threshold,
        10,
        1,
        MAX_REFINE,
    },
    {
        "mob_nopc_idleskill_rate",
        &battle_config.mob_nopc_idleskill_rate,
        100,
        0,
        100,
    },
    {
        "mob_nopc_move_rate",
        &battle_config.mob_nopc_move_rate,
        100,
        0,
        100,
    },
    {
        "boss_nopc_idleskill_rate",
        &battle_config.boss_nopc_idleskill_rate,
        100,
        0,
        100,
    },
    {
        "boss_nopc_move_rate",
        &battle_config.boss_nopc_move_rate,
        100,
        0,
        100,
    },
    {
        "hom_idle_no_share",
        &battle_config.hom_idle_no_share,
        0,
        0,
        INT_MAX,
    },
    {
        "idletime_hom_option",
        &battle_config.idletime_hom_option,
        0x1F,
        0x1,
        0xFFF,
    },
    {
        "devotion_standup_fix",
        &battle_config.devotion_standup_fix,
        1,
        0,
        1,
    },
    {
        "feature.bgqueue",
        &battle_config.feature_bgqueue,
        1,
        0,
        1,
    },
    {
        "bgqueue_nowarp_mapflag",
        &battle_config.bgqueue_nowarp_mapflag,
        0,
        0,
        1,
    },
    {
        "homunculus_exp_gain",
        &battle_config.homunculus_exp_gain,
        10,
        0,
        100,
    },
    {
        "rental_item_novalue",
        &battle_config.rental_item_novalue,
        1,
        0,
        1,
    },
    {
        "ping_timer_inverval",
        &battle_config.ping_timer_interval,
        30,
        0,
        99999999,
    },
    ...
    {
        "show_skill_scale",
        &battle_config.show_skill_scale,
        1,
        0,
        1,
    },
    {
        "achievement_mob_share",
        &battle_config.achievement_mob_share,
        0,
        0,
        1,
    },
    {
        "slave_stick_with_master",
        &battle_config.slave_stick_with_master,
        0,
        0,
        1,
    },
    {
        "slave_active_with_master",
        &battle_config.slave_active_with_master,
        0,
        0,
        1,
    },
    {
        "at_logout_event",
        &battle_config.at_logout_event,
        1,
        0,
        1,
    },
    {
        "homunculus_starving_rate",
        &battle_config.homunculus_starving_rate,
        10,
        0,
        100,
    },
    {
        "homunculus_starving_delay",
        &battle_config.homunculus_starving_delay,
        20000,
        0,
        INT_MAX,
    },
    {
        ...
        &battle_config.drop_connection_on_quit,
        0,
        0,
        1,
    },
    {
        "mob_spawn_variance",
        &battle_config.mob_spawn_variance,
        1,
        0,
        3,
    },
    {
        "mercenary_autoloot",
        &battle_config.mercenary_autoloot,
        0,
        0,
        1,
    },
    {
        "mer_idle_no_share",
        &battle_config.mer_idle_no_share,
        0,
        0,
        INT_MAX,
    },
    {
        "idletime_mer_option",
        &battle_config.idletime_mer_option,
        0x1F,
        0x1,
        0xFFF,
    },
    ...
    {
        "rndopt_drop_pillar",
        &battle_config.rndopt_drop_pillar,
        1,
        0,
        1,
    },
    {
        "pet_legacy_formula",
        &battle_config.pet_legacy_formula,
        0,
        0,
        1,
    },
    {
        "pet_distance_check",
        &battle_config.pet_distance_check,
        5,
        0,
        50,
    },
    {
        "pet_hide_check",
        &battle_config.pet_hide_check,
        1,
        0,
        1,
    },
    {
        "instance_block_leave",
        &battle_config.instance_block_leave,
        1,
        0,
        1,
    },
    {
        "instance_block_leaderchange",
        &battle_config.instance_block_leaderchange,
        1,
        0,
        1,
    },
    {
        "instance_block_invite",
        &battle_config.instance_block_invite,
        1,
        0,
        1,
    },
    {
        "instance_block_expulsion",
        &battle_config.instance_block_expulsion,
        1,
        0,
        1,
    },
    ...
    // 4th Job Stuff
    {
        "use_traitpoint_table",
        &battle_config.use_traitpoint_table,
        1,
        0,
        1,
    },
    {
        "trait_points_job_change",
    ...
    {
        "max_trait_parameter",
        &battle_config.max_trait_parameter,
        100,
        10,
        SHRT_MAX,
    },
    {
        "max_res_mres_ignored",
        &battle_config.max_res_mres_ignored,
        50,
        0,
        100,
    },
    ...
    {
        "loose_ap_on_death",
        &battle_config.loose_ap_on_death,
        1,
        0,
        1,
    },
    ...
    {
        "keep_ap_on_logout",
        &battle_config.keep_ap_on_logout,
        1,
        0,
        1,
    },
    {
        "attack_machine_level_difference",
        &battle_config.attack_machine_level_difference,
        15,
        0,
        INT_MAX,
    },
    ...
    {
        "feature.barter",
        &battle_config.feature_barter,
        1,
        0,
        1,
    },
    {
        "feature.barter_extended",
        &battle_config.feature_barter_extended,
        1,
        0,
        1,
    },
    {
        "feature.itemlink",
        &battle_config.feature_itemlink,
        1,
        0,
        1,
    },
    {
        "feature.mesitemlink",
        &battle_config.feature_mesitemlink,
        1,
        0,
        1,
    },
    {
        "feature.mesitemlink_brackets",
    ...
    {
        "feature.mesitemlink_dbname",
        &battle_config.feature_mesitemlink_dbname,
        0,
        0,
        1,
    },
    {
        "feature.mesitemicon",
        &battle_config.feature_mesitemicon,
        1,
        0,
        1,
    },
    {
        "feature.mesitemicon_dbname",
        &battle_config.feature_mesitemicon_dbname,
        0,
        0,
        1,
    },
    {
        "break_mob_equip",
        &battle_config.break_mob_equip,
        0,
        0,
        1,
    },
    {
        "macro_detection_retry",
        &battle_config.macro_detection_retry,
        3,
        1,
        INT_MAX,
    },
    {
        "macro_detection_timeout",
        &battle_config.macro_detection_timeout,
        60000,
        0,
        INT_MAX,
    },
    {
        "macro_detection_punishment",
        &battle_config.macro_detection_punishment,
        0,
        0,
        1,
    },
    {
        "macro_detection_punishment_time",
        &battle_config.macro_detection_punishment_time,
        0,
        0,
        INT_MAX,
    },
    {
        "macrochecker_delay",
        &battle_config.macrochecker_delay,
        600000,
        0,
        INT_MAX,
    },
    ...
    {
        "feature.dynamicnpc_timeout",
        &battle_config.feature_dynamicnpc_timeout,
        1000,
        60000,
        INT_MAX,
    },
    {
        "feature.dynamicnpc_rangex",
        &battle_config.feature_dynamicnpc_rangex,
        2,
        0,
        INT_MAX,
    },
    {
        "feature.dynamicnpc_rangey",
        &battle_config.feature_dynamicnpc_rangey,
        2,
        0,
        INT_MAX,
    },
    {
        "feature.dynamicnpc_direction",
        &battle_config.feature_dynamicnpc_direction,
        0,
        0,
        1,
    },
    ...
    {
        "mob_respawn_time",
        &battle_config.mob_respawn_time,
        1000,
        1000,
        INT_MAX,
    },
    ...
    {
        "map_edge_size",
        &battle_config.map_edge_size,
        15,
        1,
        40,
    },
    {
        "randomize_center_cell",
        &battle_config.randomize_center_cell,
        1,
        0,
        1,
    },
    ...
    {
        "feature.stylist",
        &battle_config.feature_stylist,
        1,
        0,
        1,
    },
    {
        "feature.banking_state_enforce",
        &battle_config.feature_banking_state_enforce,
        0,
        0,
        1,
    },
#ifdef RENEWAL
    {
        "feature.instance_allow_reconnect",
        &battle_config.instance_allow_reconnect,
        1,
        0,
        1,
    },
    ...
    {
        "synchronize_damage",
        &battle_config.synchronize_damage,
        0,
        0,
        1,
    },
...
#ifdef RENEWAL
    {
        "hom_delay_reset_vaporize",
        &battle_config.hom_delay_reset_vaporize,
        0,
        0,
        1,
    },
    {
        "hom_delay_reset_warp",
        &battle_config.hom_delay_reset_warp,
        0,
        0,
        1,
    },
    {
        "alchemist_summon_setting",
        &battle_config.alchemist_summon_setting,
        12,
        0,
        15,
    },
    ...
    {
        "trade_count_stackable",
        &battle_config.trade_count_stackable,
        1,
        0,
        1,
    },
    {
        "enable_bonus_map_drops",
        &battle_config.enable_bonus_map_drops,
        1,
        0,
        1,
    },
    {
        "hide_cloaked_units",
        &battle_config.hide_cloaked_units,
        0,
        0,
        BL_ALL,
    },
    {
        "oridecon_research_fix",
