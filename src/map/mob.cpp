...
bool mob_ksprotected(block_list* src, block_list* target) {
	...
	do {
		...
		if (t_sd && ((sce->val2 == 1 && sce->val1 != t_sd->id) || (sce->val2 == 2 && sce->val3 && sce->val3 != t_sd->status.party_id) || (sce->val2 == 3 && sce->val4 && sce->val4 != t_sd->status.guild_id)))
			break;
...
static TIMER_FUNC(mob_spawn_guardian_sub) {
	...
	if (g == nullptr) { //Liberate castle, if the guild is not found this is an error! [Skotlex]
...
int32 mob_linksearch(block_list* bl, va_list ap) {
	...
	if (md->mob_id == mob_id && status_has_mode(&md->status, MD_ASSIST) && DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME && md->target_id == 0) {
...
int32 mob_spawn(mob_data* md) {
	...
	if (md->spawn) { //Respawn data
		...
		if ((!md->spawn->state.boss || (md->x == 0 && md->y == 0) || md->spawn->xs != 1 || md->spawn->ys != 1) && (md->spawn->xs + md->spawn->ys < 1 || !rnd_chance(1, md->spawn->xs * md->spawn->ys) || !map_getcell(md->m, md->x, md->y, CELL_CHKREACH))) {
		...
		if (battle_config.no_spawn_on_player > 99 && map_foreachinallrange(mob_count_sub, md, AREA_SIZE, BL_PC)) { // retry again later (players on sight)
			if (md->spawn_timer != INVALID_TIMER)
				delete_timer(md->spawn_timer, mob_delayspawn);
			md->spawn_timer = add_timer(tick + battle_config.mob_respawn_time, mob_delayspawn, md->id, 0);
			return 1;
...
static int32 mob_ai_sub_hard_activesearch(block_list* bl, va_list ap) {
	...
	if (
	    ...
	    battle_check_range(md, bl, md->db->range2)) { //Pick closest target?
		...
		if ((md->ud.walktimer == INVALID_TIMER && wpd.path_len > md->db->range2) || (md->ud.walktimer != INVALID_TIMER && wpd.path_len > md->db->range3))
			return 0;
#endif
		(*target) = bl;
		md->target_id = bl->id;
...
static int32 mob_ai_sub_hard_lootsearch(block_list* bl, va_list ap) {
	...
	dist = distance_bl(md, bl);
	if (mob_can_reach(md, bl, battle_config.loot_range) && ((*target) == nullptr || (battle_config.monster_loot_search_type && md->target_id > bl->id) || (!battle_config.monster_loot_search_type && !check_distance_bl(md, *target, dist)) // New target closer than previous one.
	                                                           )) {
		(*target) = bl;
		md->target_id = bl->id;
	}
...
int32 mob_unlocktarget(mob_data* md, t_tick tick) {
	...
	if (!md->ud.state.ignore_cell_stack_limit && battle_config.official_cell_stack_limit > 0 && (battle_config.mob_ai & 0x8) && map_count_oncell(md->m, md->x, md->y, BL_CHAR | BL_NPC, 1) > battle_config.official_cell_stack_limit) {
...
static bool mob_ai_sub_hard(mob_data* md, t_tick tick) {
		...
		if (!tbl || tbl->m != md->m ||
		    ...
		    (tbl->type == BL_PC &&
		        ((((TBL_PC*)tbl)->state.gangsterparadise && !(mode & MD_STATUSIMMUNE)) ||
		            ((TBL_PC*)tbl)->invincible_timer != INVALID_TIMER))) { //No valid target
	...
	if (md->attacked_id && mode & MD_CANATTACK) {
		if (md->attacked_id == md->target_id) {                                                                                                                                                                                                                                                                            //Rude attacked check.
			if (!battle_check_range(md, tbl, md->status.rhw.range) && (                                                                                                                                                                                                                                                    //Can't attack back and can't reach back.
			                                                              (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0 && (battle_config.mob_ai & 0x2 || md->sc.getSCE(SC_SPIDERWEB) || md->sc.getSCE(SC_BITE) || md->sc.getSCE(SC_VACUUM_EXTREME) || md->sc.getSCE(SC_THORNSTRAP) || md->sc.getSCE(SC__MANHOLE) // Not yet confirmed if boss will teleport once it can't reach target.
			...
			int32 dist;
			if (md->m != abl->m || abl->prev == nullptr || (dist = distance_bl(md, abl)) > AREA_SIZE                                                                                                                                                                           // Attacker longer than visual area
			    ...
			    || (!battle_check_range(md, abl, md->status.rhw.range)                                                                                                                                                                                                         // Not on Melee Range and ...
			           && (                                                                                                                                                                                                                                                    // Reach check
			                  (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0 && (battle_config.mob_ai & 0x2 || md->sc.getSCE(SC_SPIDERWEB) || md->sc.getSCE(SC_BITE) || md->sc.getSCE(SC_VACUUM_EXTREME) || md->sc.getSCE(SC_THORNSTRAP) || md->sc.getSCE(SC__MANHOLE) // Not yet confirmed if boss will teleport once it can't reach target.
			                                                                                || md->walktoxy_fail_count > 0)) ||
			                  !mob_can_reach(md, abl, dist + md->db->range3)))) { // Rude attacked
				if (abl->id != md->id                                             //Self damage does not cause rude attack
				    && ++md->state.attacked_count > RUDE_ATTACKED_COUNT) {
					mobskill_use(md, tick, MSC_RUDEATTACKED);
				}
			} else if (!(battle_config.mob_ai & 0x2) && !status_check_skilluse(md, abl, 0, 0)) {
	...
	if (tbl->type == BL_ITEM) { //Loot time.
		...
		if (md->lootitems == nullptr) { //Can't loot...
			mob_unlocktarget(md, tick);
			return true;
		}
		if (!check_distance_bl(md, tbl, loot_range)) { //Still not within loot range.
		...
		if (pcdb_checkid(md->vd->look[LOOK_BASE])) { //Give them walk act/delay to properly mimic players. [Skotlex]
	...
	if (md->ud.walktimer != INVALID_TIMER && (!can_move || md->ud.walkpath.path_pos <= battle_config.mob_chase_refresh) && mob_is_chasing(md->state.skillstate))
		return true;

	// Out of range
	if (!(mode & MD_CANMOVE) || (!can_move && (md->sc.cant.move || DIFF_TICK(tick, md->ud.canmove_tick) > 0))) { // Can't chase. Immobile and trapped mobs will use idle skills and unlock their target after a while
		if (md->ud.attacktimer == INVALID_TIMER) {                                                               // Only switch mode if no more attack delay left
...
static void mob_item_drop(mob_data* md, std::shared_ptr<s_item_drop_list>& dlist, std::shared_ptr<s_item_drop>& ditem, int32 loot, int32 drop_rate, bool flag) {
	...
	test_autoloot = sd && (drop_rate <= sd->state.autoloot || pc_isautolooting(sd, ditem->item_data.nameid)) && (flag ? ((battle_config.homunculus_autoloot ? (battle_config.hom_idle_no_share == 0 || !pc_isidle_hom(sd)) : 0) || (battle_config.mercenary_autoloot ? (battle_config.mer_idle_no_share == 0 || !pc_isidle_mer(sd)) : 0)) : (battle_config.idle_no_autoloot == 0 || DIFF_TICK(last_tick, sd->idletime) < battle_config.idle_no_autoloot));
#ifdef AUTOLOOT_DISTANCE
	test_autoloot = test_autoloot && sd->m == md->m && check_distance_blxy(sd, dlist->x, dlist->y, AUTOLOOT_DISTANCE);
#endif
...
int32 mob_dead(mob_data* md, block_list* src, int32 type) {
	...
	if (!(type & 1) && !map_getmapflag(m, MF_NOMOBLOOT) && !md->state.rebirth && (!md->special_state.ai ||                                                             //Non special mob
...
#ifdef RENEWAL_DROP
		drop_modifier = pc_level_penalty_mod(first_sd != nullptr ? first_sd : second_sd != nullptr ? second_sd
		                                                                                           : third_sd,
		    PENALTY_DROP, nullptr, md);
#endif
	...
	if (!rebirth) { // Only trigger event on final kill
		...
		if (sd) {
			...
			if ((sd->mission_mobid == md->mob_id) || (mission_mdb != nullptr && ((battle_config.taekwon_mission_mobname == 1 && util::vector_exists(status_get_race2(md), RC2_GOBLIN) && util::vector_exists(mission_mdb->race2, RC2_GOBLIN)) || (battle_config.taekwon_mission_mobname == 2 && mob->jname.compare(mission_mdb->jname) == 0)))) { //TK_MISSION [Skotlex]
...
int32 mob_guardian_guildchange(mob_data* md) {
	...
	if (g == nullptr) { //Properly remove guardian info from Castle data.
...
void mob_add_spawn(uint16 mob_id, const struct spawn_info& new_spawn) {
	...
	std::sort(spawns.begin(), spawns.end(),
	    [](const spawn_info& a, const spawn_info& b) -> bool { return a.qty > b.qty; });
	/** Note
...
bool mobskill_use(mob_data* md, t_tick tick, int32 event, int64 damage) {
	...
	for (n = 0; n < ms.size(); i++, n++) {
		...
		if (ms[i]->state != md->state.skillstate) {
			if (md->state.skillstate != MSS_DEAD && (ms[i]->state == MSS_ANY || (ms[i]->state == MSS_ANYTARGET && md->target_id && md->state.skillstate != MSS_LOOT))) //ANYTARGET works with any state as long as there's a target. [Skotlex]
				;
			else
				continue;
		}
		...
		if (ms[i]->cond1 == event)
			...
			switch (ms[i]->cond1) {
				...
				case MSC_FRIENDHPLTMAXRATE: // friend HP < maxhp%
					flag = ((fbl = mob_getfriendhprate(md, 0, ms[i]->cond2)) != nullptr);
					break;
				case MSC_FRIENDHPINRATE:
					flag = ((fbl = mob_getfriendhprate(md, ms[i]->cond2, ms[i]->val[0])) != nullptr);
					break;
				case MSC_FRIENDSTATUSON:  // friend status[num] on
				case MSC_FRIENDSTATUSOFF: // friend status[num] off
					flag = ((fmd = mob_getfriendstatus(md, ms[i]->cond1, ms[i]->cond2)) != nullptr);
					break;
				case MSC_SLAVELT: // slave < num
					flag = (mob_countslave(md) < c2);
					break;
				case MSC_ATTACKPCGT: // attack pc > num
					flag = (unit_counttargeted(md) > c2);
					break;
				...
				case MSC_RUDEATTACKED:
					...
					if (flag)
						md->state.attacked_count = 0; //Rude attacked count should be reset after the skill condition is met. Thanks to Komurka [Skotlex]
					break;
				case MSC_MASTERHPLTMAXRATE:
					flag = ((fbl = mob_getmasterhpltmaxrate(md, ms[i]->cond2)) != nullptr);
					break;
				case MSC_MASTERATTACKED:
					flag = (md->master_id > 0 && (fbl = map_id2bl(md->master_id)) && unit_counttargeted(fbl) > 0);
					break;
				case MSC_ALCHEMIST:
					flag = (md->special_state.ai != AI_NONE && md->trickcasting == 0 && md->status.hp < md->status.max_hp);
					break;
				case MSC_MOBNEARBYGT:
					flag = (map_foreachinallrange(mob_count_sub, md, AREA_SIZE, BL_MOB) > c2);
					break;
				case MSC_TRICKCASTING:
					flag = (md->trickcasting > 0);
					break;
			}
		}
		...
		if (skill_get_casttype(ms[i]->skill_id) == CAST_GROUND) { //Ground skill.
			...
			if (ms[i]->target >= MST_AROUND5) {
				j = ms[i]->target >= MST_AROUND1 ? (ms[i]->target - MST_AROUND1) + 1 : (ms[i]->target - MST_AROUND5) + 1;
