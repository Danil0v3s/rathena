...
std::unordered_map<size_t, struct duel> duel_get_list() {
	return duel_list;
} //this could be exposed but not really necessarily, (hiden impl)
bool duel_exist(size_t did) {
	return duel_list.find(did) != duel_list.end();
}
duel& duel_get_duelid(size_t did) {
	return duel_list.at(did);
}
...
size_t duel_countactives() {
	...
	for (const auto& lcur : duel_list)
		if (lcur.second.members_count > 1)
			++count;
	return count;
}
