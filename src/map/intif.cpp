...
static const int32 packet_len_table[] = {
    ...
    0,
    0,
    0, //0x3850  Auctions [Zephyrus] itembound[Akinari]
    -1,
    7,
    -1,
    7,
    ...
    0,
    0,
    0, //0x3860  Quests [Kevin] [Inkfish] / Achievements [Aleos]
    -1,
    3,
    3,
    0,
    ...
    3,
    3,
    0, //0x3870  Mercenaries [Zephyrus] / Elemental [pakpil]
    12,
    -1,
    7,
    3,
...
int32 intif_parse_WisMessage(int32 fd) {
	...
	if (sd == nullptr || strcmp(sd->status.name, name) != 0) { //Not found
	...
	if (i < MAX_IGNORE_LIST && sd->ignore[i].name[0] != '\0') { //Ignored
		intif_wis_reply(id, (pc_has_permission(sd, PC_PERM_HIDE_SESSION)) ? 1 : 2);
		return 0;
	}
...
int32 intif_parse_GuildMemberInfoChanged(int32 fd) {
	...
	switch (type) {
		case GMI_POSITION:
			g->guild.member[idx].position = RFIFOW(fd, 18);
			guild_memberposition_changed(g->guild, idx, RFIFOW(fd, 18));
			break;
