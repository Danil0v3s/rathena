...
static void clif_weather_check(map_session_data* sd) {
	...
	if (map_getmapflag(m, MF_SNOW) || map_getmapflag(m, MF_CLOUDS) || map_getmapflag(m, MF_FOG) || map_getmapflag(m, MF_FIREWORKS) || map_getmapflag(m, MF_SAKURA) || map_getmapflag(m, MF_LEAVES) || map_getmapflag(m, MF_CLOUDS2)) {
...
void clif_scriptmenu(map_session_data& sd, uint32 npcid, const char* mes) {
	...
	if (!sd.state.using_fake_npc && (npcid == fake_nd->id || ((bl = map_id2bl(npcid)) && (bl->m != sd.m || bl->x < sd.x - AREA_SIZE - 1 || bl->x > sd.x + AREA_SIZE + 1 || bl->y < sd.y - AREA_SIZE - 1 || bl->y > sd.y + AREA_SIZE + 1))))
...
void clif_additem(const map_session_data* sd, int32 n, int32 amount, unsigned char fail) {
	...
	if (fail) {
		 ...
		 */
		p.bindOnEquipType = sd->inventory.u.items_inventory[n].bound && !itemdb_isstackable2(sd->inventory_data[n]) ? 2 : sd->inventory_data[n]->flag.bindOnEquip ? 1
		                                                                                                                                                          : 0;
...
#if PACKETVER >= 20080102
	p->bindOnEquipType = it->bound ? 2 : id->flag.bindOnEquip ? 1
	                                                          : 0;
#endif
...
void clif_parse_guild_castle_teleport_request(int32 fd, map_session_data* sd) {
	...
	if (map_getmapflag(sd->m, MF_GVG_CASTLE) || map_getmapflag(sd->m, MF_NOTELEPORT) || map_getmapflag(sd->m, MF_NOWARP))
		return;
...
void clif_use_card(const map_session_data* sd, int32 idx) {
	...
	if (!c)
		return; // no item is available for card insertion
...
void clif_parse_LoadEndAck(int32 fd, map_session_data* sd) {
	...
	if (sd->state.connect_new) {
		...
		if ((lv = pc_checkskill(sd, SG_KNOWLEDGE)) > 0) {
			if (sd->m == sd->feel_map[0].m || sd->m == sd->feel_map[1].m || sd->m == sd->feel_map[2].m)
	...
	if (sd->state.changemap) { // restore information that gets lost on map-change
		...
		if (night_flag && mapdata->getMapFlag(MF_NIGHTENABLED)) { //Display night.
			if (!sd->state.night) {
				sd->state.night = 1;
				clif_status_load(sd, EFST_SKE, 1);
			}
...
void clif_parse_WalkToXY(int32 fd, map_session_data* sd) {
	...
	if (battle_config.official_cell_stack_limit > 0 && sd->x == x + 1 && sd->y == y && map_count_oncell(sd->m, x, y, BL_CHAR | BL_NPC, 1))
		return;
...
void clif_parse_ActionRequest_sub(map_session_data& sd, uint8 action_type, int32 target_id, t_tick tick) {
	...
	switch (action_type) {
		...
		case DMG_SIT_DOWN: // sitdown
			...
			if (!sd.sc.empty() && (sd.sc.getSCE(SC_DANCING) || (sd.sc.getSCE(SC_GRAVITATION) && sd.sc.getSCE(SC_GRAVITATION)->val3 == BCT_SELF))) //No sitting during these states either.
				break;
...
void clif_parse_Restart(int32 fd, map_session_data* sd) {
	switch (RFIFOB(fd, packet_db[RFIFOW(fd, 0)].pos[0])) {
		...
		case 0x01:
			/*	Rovert's Prevent logout option - Fixed [Valaris]	*/
			if (!sd->sc.getSCE(SC_CLOAKING) && !sd->sc.getSCE(SC_HIDING) && !sd->sc.getSCE(SC_CHASEWALK) && !sd->sc.getSCE(SC_CLOAKINGEXCEED) && !sd->sc.getSCE(SC_SUHIDE) && !sd->sc.getSCE(SC_NEWMOON) &&
			    (!battle_config.prevent_logout || sd->canlog_tick == 0 || DIFF_TICK(gettick(), sd->canlog_tick) > battle_config.prevent_logout)) { //Send to char-server for character selection.
...
/// 01af <num>.W
void clif_parse_ChangeCart(int32 fd, map_session_data* sd) { // TODO: State tracking?
	int32 type;
...
/// 01bb <char name>.24B
void clif_parse_GMShift(int32 fd, map_session_data* sd) { // FIXME: remove is supposed to receive account name for clients prior 20100803RE
	char* player_name;
...
/// 01bd <char name>.24B
void clif_parse_GMRecall(int32 fd, map_session_data* sd) { // FIXME: recall is supposed to receive account name for clients prior 20100803RE
	char* player_name;
	char command[NAME_LENGTH + 8];
...
void clif_parse_FriendsListRemove(int32 fd, map_session_data* sd) {
	...
	for (i = 0; i < MAX_FRIENDS &&
	    (sd->status.friends[i].char_id != char_id || sd->status.friends[i].account_id != account_id);
	    i++)
		;
	...
	if ((f_sd = map_id2sd(account_id)) && f_sd->status.char_id == char_id) {
		for (i = 0; i < MAX_FRIENDS &&
		    (f_sd->status.friends[i].char_id != sd->status.char_id || f_sd->status.friends[i].account_id != sd->status.account_id);
		    i++)
			;
...
///     1 = feed
///     2 = delete
void clif_parse_HomMenu(int32 fd, map_session_data* sd) { //[orn]
	int32 cmd = RFIFOW(fd, 0);
	...
	if (i == MAIL_MAX_INBOX) {
		...
		for (int32 j = 0; j < MAIL_MAX_ITEM; j++) {
			...
			if (item->nameid > 0 && item->amount > 0 && data != nullptr) {
				...
				mailitem->bindOnEquip = item->bound ? 2 : data->flag.bindOnEquip ? 1
				                                                                 : 0;
...
void clif_parse_Mail_getattach(int32 fd, map_session_data* sd) {
	...
	int32 attachment = packet_id == 0x9f1 ? MAIL_ATT_ZENY : packet_id == 0x9f3 ? MAIL_ATT_ITEM
	                                                                           : MAIL_ATT_NONE;
#endif
...
void clif_parse_Auction_setitem(int32 fd, map_session_data* sd) {
	...
	if (id != nullptr && !(id->type == IT_ARMOR || id->type == IT_PETARMOR || id->type == IT_WEAPON || id->type == IT_CARD || id->type == IT_ETC || id->type == IT_SHADOWGEAR)) { // Consumable or pets are not allowed
