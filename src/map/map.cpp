	...
	if (bl->type & BL_CHAR) {
		...
		if (sc != nullptr && !sc->empty()) {
			if (sc->getSCE(SC_DANCING))
				...
				if (sc->getSCE(SC_PROPERTYWALK) && sc->getSCE(SC_PROPERTYWALK)->val3 < skill_get_maxcount(sc->getSCE(SC_PROPERTYWALK)->val1, sc->getSCE(SC_PROPERTYWALK)->val2) && map_find_skill_unit_oncell(bl, bl->x, bl->y, SO_ELECTRICWALK, nullptr, 0) == nullptr && map_find_skill_unit_oncell(bl, bl->x, bl->y, NPC_ELECTRICWALK, nullptr, 0) == nullptr && map_find_skill_unit_oncell(bl, bl->x, bl->y, SO_FIREWALK, nullptr, 0) == nullptr && map_find_skill_unit_oncell(bl, bl->x, bl->y, NPC_FIREWALK, nullptr, 0) == nullptr && skill_unitsetting(bl, sc->getSCE(SC_PROPERTYWALK)->val1, sc->getSCE(SC_PROPERTYWALK)->val2, x0, y0, 0)) {
...
#endif
					    && (!wall_check || path_search_long(nullptr, center->m, center->x, center->y, bl->x, bl->y, CELL_CHKWALL)) && bl_list_count < BL_LIST_MAX)
...
int32 map_foreachinareaV(int32 (*func)(block_list*, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int32 type, va_list ap, bool wall_check) {
	...
	if (type & ~BL_MOB) {
		for (by = y0 / BLOCK_SIZE; by <= y1 / BLOCK_SIZE; by++) {
			for (bx = x0 / BLOCK_SIZE; bx <= x1 / BLOCK_SIZE; bx++) {
				for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != nullptr; bl = bl->next) {
					if (bl->type & type && bl->x >= x0 && bl->x <= x1 && bl->y >= y0 && bl->y <= y1 && (!wall_check || path_search_long(nullptr, m, cx, cy, bl->x, bl->y, CELL_CHKWALL)) && bl_list_count < BL_LIST_MAX)
						bl_list[bl_list_count++] = bl;
				}
			}
		}
	...
	if (type & BL_MOB) {
		for (by = y0 / BLOCK_SIZE; by <= y1 / BLOCK_SIZE; by++) {
			for (bx = x0 / BLOCK_SIZE; bx <= x1 / BLOCK_SIZE; bx++) {
				for (bl = mapdata->block_mob[bx + by * mapdata->bxs]; bl != nullptr; bl = bl->next) {
					if (bl->x >= x0 && bl->x <= x1 && bl->y >= y0 && bl->y <= y1 && (!wall_check || path_search_long(nullptr, m, cx, cy, bl->x, bl->y, CELL_CHKWALL)) && bl_list_count < BL_LIST_MAX)
						bl_list[bl_list_count++] = bl;
				}
			}
		}
...
int32 map_quit(map_session_data* sd) {
	...
	if (sd->state.permanent_speed == 1)
		sd->state.permanent_speed = 0; // Remove lock so speed is set back to normal at login.
...
map_session_data* map_nick2sd(const char* nick, bool allow_partial) {
	...
	for (sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter)) {
		if (allow_partial && battle_config.partial_name_scan) { // partial name search
			if (strnicmp(sd->status.name, nick, nicklen) == 0) {
				found_sd = sd;

				if (strcmp(sd->status.name, nick) == 0) { // Perfect Match
					qty = 1;
					break;
				}
		...
		} else if (strcasecmp(sd->status.name, nick) == 0) { // exact search only
			found_sd = sd;
			qty = 1;
			break;
		}
...
struct s_mapiterator* mapit_alloc(enum e_mapitflags flags, enum bl_type types) {
	...
	mapit->types = types;
	if (types == BL_PC)
		mapit->dbi = db_iterator(pc_db);
	else if (types == BL_MOB)
		mapit->dbi = db_iterator(mobid_db);
	else
		mapit->dbi = db_iterator(id_db);
	return mapit;
}
...
void map_spawnmobs(int16 m) {
	...
	if (mapdata->mob_delete_timer != INVALID_TIMER) { //Mobs have not been removed yet [Skotlex]
		delete_timer(mapdata->mob_delete_timer, map_removemobs_timer);
		mapdata->mob_delete_timer = INVALID_TIMER;
		return;
	}
...
TIMER_FUNC(map_removemobs_timer) {
	...
	if (mapdata->mob_delete_timer != tid) { //Incorrect timer call!
...
int32 map_check_dir(int32 s_dir, int32 t_dir) {
	...
	switch (s_dir) {
		case DIR_NORTH:
			if (t_dir == DIR_NORTHEAST || t_dir == DIR_NORTHWEST || t_dir == DIR_NORTH)
				return 0;
			break;
		case DIR_NORTHWEST:
			if (t_dir == DIR_NORTH || t_dir == DIR_WEST || t_dir == DIR_NORTHWEST)
				return 0;
			break;
		case DIR_WEST:
			if (t_dir == DIR_NORTHWEST || t_dir == DIR_SOUTHWEST || t_dir == DIR_WEST)
				return 0;
			break;
		case DIR_SOUTHWEST:
			if (t_dir == DIR_WEST || t_dir == DIR_SOUTH || t_dir == DIR_SOUTHWEST)
				return 0;
			break;
		case DIR_SOUTH:
			if (t_dir == DIR_SOUTHWEST || t_dir == DIR_SOUTHEAST || t_dir == DIR_SOUTH)
				return 0;
			break;
		case DIR_SOUTHEAST:
			if (t_dir == DIR_SOUTH || t_dir == DIR_EAST || t_dir == DIR_SOUTHEAST)
				return 0;
			break;
		case DIR_EAST:
			if (t_dir == DIR_SOUTHEAST || t_dir == DIR_NORTHEAST || t_dir == DIR_EAST)
				return 0;
			break;
		case DIR_NORTHEAST:
			if (t_dir == DIR_EAST || t_dir == DIR_NORTH || t_dir == DIR_NORTHEAST)
				return 0;
			break;
	}
	return 1;
...
int32 map_random_dir(const block_list* bl, int16* x, int16* y) {
	...
	do {
	    ...
	    (map_getcell(bl->m, xi, yi, CELL_CHKNOPASS) || !path_search(nullptr, bl->m, bl->x, bl->y, xi, yi, 1, CELL_CHKNOREACH)) && (++i) < 100);
...
#undef RENEWALPREFIX
		...
		if (strcmpi(w1, "barter_table") == 0)
			...
			if (strcmpi(w1, "map_server_ip") == 0)
				...
				log_db_db = w2;
			else if (strcmpi(w1, "start_status_points") == 0)
				inter_config.start_status_points = atoi(w2);
			else if (strcmpi(w1, "emblem_woe_change") == 0)
				inter_config.emblem_woe_change = config_switch(w2) == 1;
			else if (strcmpi(w1, "emblem_transparency_limit") == 0) {
				auto val = atoi(w2);
