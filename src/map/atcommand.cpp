...
ACMD_FUNC(send) {
	...
	if (!message || !*message ||
	    !((sscanf(message, "len %8x", &type) == 1 && (len = 1)) || sscanf(message, "%8x", &type) == 1)) {
		int32 i;
		for (i = 900; i <= 903; ++i)
...
ACMD_FUNC(mapmove) {
	...
	if ((x || y) && map_getcell(m, x, y, CELL_CHKNOPASS)) { //This is to prevent the pc_setpos call from printing an error.
...
ACMD_FUNC(jump) {
	...
	if ((x || y) && map_getcell(sd->m, x, y, CELL_CHKNOPASS)) { //This is to prevent the pc_setpos call from printing an error.
		clif_displaymessage(fd, msg_txt(sd, 2));                // Invalid coordinates, using random target cell.
...
ACMD_FUNC(option) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d %11d", &param1, &param2, &param3) < 1 || param1 < 0 || param2 < 0 || param3 < 0) { // failed to match the parameters so inform the user of the options
		const char* text;
...
ACMD_FUNC(hide) {
	...
	if (pc_isinvisible(sd)) {
		...
		if (!battle_config.pk_mode && map_getmapflag(sd->m, MF_PVP) && !map_getmapflag(sd->m, MF_PVP_NOCALCRANK)) { // register the player for ranking calculations
		...
		if (map_getmapflag(sd->m, MF_PVP) && !map_getmapflag(sd->m, MF_PVP_NOCALCRANK) && sd->pvp_timer != INVALID_TIMER) { // unregister the player for ranking
...
ACMD_FUNC(item) {
	...
	if (!strcmpi(parent_cmd, "itembound")) {
		if (!message || !*message || (sscanf(message, "\"%99[^\"]\" %11d %11d", item_name, &number, &bound) < 3 && sscanf(message, "%99s %11d %11d", item_name, &number, &bound) < 3)) {
	...
	} else if (!message || !*message || (sscanf(message, "\"%99[^\"]\" %11d", item_name, &number) < 1 && sscanf(message, "%99s %11d", item_name, &number) < 1)) {
...
ACMD_FUNC(item2) {
	...
	if (!strcmpi(parent_cmd + 1, "itembound2")) {
		if (!message || !*message || (sscanf(message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &bound) < 10 && sscanf(message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &bound) < 10)) {
	...
	} else if (!message || !*message || (sscanf(message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4) < 9 && sscanf(message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4) < 9)) {
...
ACMD_FUNC(go) {
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) { // no value matched so send the list of locations
		const char* text;
...
ACMD_FUNC(produce) {
	...
	if (!message || !*message || (sscanf(message, "\"%99[^\"]\" %11d %11d", item_name, &attribute, &star) < 1 && sscanf(message, "%99s %11d %11d", item_name, &attribute, &star) < 1)) {
...
ACMD_FUNC(zeny) {
	...
	if (!ret)
		clif_displaymessage(fd, msg_txt(sd, 176)); //ret=0 mean cmd success
	return 0;
}
	...
	if ((sd->class_ & MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT && pc_checkskill(sd, RK_DRAGONTRAINING) > 0) {
		if (!(sd->sc.option & OPTION_DRAGON)) {
			...
			if (message[0]) {
				...
				option = (color == 2 ? OPTION_DRAGON2 : color == 3 ? OPTION_DRAGON3
...
ACMD_FUNC(npcmove) {
	...
	if (npc_movenpc(nd, x, y)) {                    //actually failed to move
...
ACMD_FUNC(npctalk) {
	...
	if (ifcolor)
		clif_messagecolor(nd, color, temp, true, AREA_CHAT_WOC);
	else
		clif_disp_overhead(nd, temp);
...
ACMD_FUNC(pettalk) {
	...
	if (message[0] == '/') { // pet emotion processing
		...
		if (i == ET_DICE1)
			i = rnd_value<int32>(ET_DICE1, ET_DICE6); // randomize /dice
...
ACMD_FUNC(clone) {
	...
	if (strcmpi(parent_cmd, "clone") == 0)
		...
		master = sd->id;
		if (battle_config.atc_slave_clone_limit && mob_countslave(sd) >= battle_config.atc_slave_clone_limit) {
...
ACMD_FUNC(delitem) {
	...
	while (amount && (idx = pc_search_inventory(sd, nameid)) != -1) {
		...
		if (sd->inventory_data[idx]->type == IT_PETEGG && sd->inventory.u.items_inventory[idx].card[0] == CARD0_PET) { // delete pet
...
ACMD_FUNC(accinfo) {
	...
	if (!message || !*message || strlen(message) > NAME_LENGTH || (sscanf(message, "%23s", query) < 1)) {
...
bool is_atcommand(const int32 fd, map_session_data* sd, const char* message, int32 type) {
	...
	if (info->restriction) {
		...
		if (info->restriction & ATCMD_NOSCRIPT && (type == 0 || type == 3)) //scripts prevent
			return true;
		if (info->restriction & ATCMD_NOAUTOTRADE && (type == 0 || type == 3) && ((is_atcommand && sd && sd->state.autotrade) || (ssd && ssd->state.autotrade)))
			return true;
	}
