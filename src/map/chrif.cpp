...
static const int32 packet_len_table[0x3d] = {
    // U - used, F - free
    60,
    3,
    -1,
    ...
    -1,
    6,
    -1, // 2af8-2aff: U->2af8, U->2af9, U->2afa, U->2afb, U->2afc, U->2afd, U->2afe, U->2aff
    6,
    -1,
    18,
    7,
    -1,
    -1,
    28 + MAP_NAME_LENGTH_EXT,
    10, // 2b00-2b07: U->2b00, U->2b01, U->2b02, U->2b03, U->2b04, U->2b05, U->2b06, U->2b07
    6,
    30,
    10,
    -1,
    ...
    7,
    44,
    34, // 2b08-2b0f: U->2b08, U->2b09, U->2b0a, U->2b0b, U->2b0c, U->2b0d, U->2b0e, U->2b0f
    11,
    10,
    10,
    0,
    ...
    -1,
    0,
    10, // 2b10-2b17: U->2b10, U->2b11, U->2b12, F->2b13, U->2b14, U->2b15, F->2b16, U->2b17
    2,
    10,
    2,
    -1,
    ...
    14,
    19,
    19, // 2b20-2b27: U->2b20, U->2b21, U->2b22, U->2b23, U->2b24, U->2b25, U->2b26, U->2b27
    -1,
    0,
    6,
    15,
    ...
    6,
    -1,
    -1, // 2b28-2b2f: U->2b28, F->2b29, U->2b2a, U->2b2b, F->2b2c, U->2b2d, U->2b2e, U->2b2f
};
...
void chrif_authok(int32 fd) {
	...
	if (global_core->is_running() &&
	    ...
	    node->login_id1 == login_id1) { //Auth Ok
...
void chrif_authfail(int32 fd) { /* HELLO WORLD. ip in RFIFOL 15 is not being used (but is available) */
	...
	if (node != nullptr &&
	    ...
	    node->sex == sex &&
	    node->state == ST_LOGIN) { // found a match
		clif_authfail_fd(node->fd, 0);
		chrif_auth_delete(account_id, char_id, ST_LOGIN);
	}
}
...
int32 chrif_parse(int32 fd) {
	...
	while (RFIFOREST(fd) >= 2) {
		...
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) || packet_len_table[cmd - 0x2af8] == 0) {
			...
			if (r == 1)
				continue; // Treated in intif
			if (r == 2)
				return 0; // Didn't have enough data (len==-1)
