...
int32 npc_touch_areanpc2(mob_data* md) {
	...
	for (i = 0; i < mapdata->npc_num_area; i++) {
		...
		if (x >= mapdata->npc[i]->x - xs && x <= mapdata->npc[i]->x + xs && y >= mapdata->npc[i]->y - ys && y <= mapdata->npc[i]->y + ys) { // In the npc touch area
			switch (mapdata->npc[i]->subtype) {
				case NPCTYPE_WARP: {
...
int32 npc_check_areanpc(int32 flag, int16 m, int16 x, int16 y, int16 range) {
	...
	for (i = 0; i < mapdata->npc_num_area; i++) {
		...
		if (x1 >= mapdata->npc[i]->x - xs && x0 <= mapdata->npc[i]->x + xs && y1 >= mapdata->npc[i]->y - ys && y0 <= mapdata->npc[i]->y + ys)
			break; // found a npc
	}
...
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if (strcmp(w1, "-") == 0) { // 'floating' shop?
		...
		if (sscanf(w1, "%15[^,],%6hd,%6hd,%4hd", mapname, &x, &y, &dir) != 4 || strchr(w4, ',') == nullptr) {
	...
	while (p) {
		...
		if (value < 0) {
			if (type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP)
				value = id->value_buy;
			else
...
static const char* npc_parse_script(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if (sscanf(w4, "%*[^,],%6hd,%6hd", &xs, &ys) == 2) { // OnTouch area defined
		nd->u.scr.xs = xs;
		nd->u.scr.ys = ys;
...
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath, map_session_data* owner = nullptr) {
	...
	if (w2[length - 1] != ')' || length <= 11 || length - 11 >= sizeof(srcname)) { // does not match 'duplicate(%127s)', name is empty or too long
...
void npc_market_tosql(const char* exname, struct npc_item_list* list) {
	...
	if (SQL_ERROR == stmt.Prepare("REPLACE INTO `%s` (`name`,`nameid`,`price`,`amount`,`flag`) VALUES ('%s','%u','%d','%d','%" PRIu8 "')", market_table, exname, list->nameid, list->value, list->qty, list->flag) ||
...
static void npc_market_fromsql(void) {
	...
	while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
		...
		list.nameid = strtoul(data, nullptr, 10);
		Sql_GetData(mmysql_handle, 2, &data, nullptr);
		list.value = atoi(data);
...
static const char* npc_parse_mob(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if ((w1count = sscanf(w1, "%15[^,],%6hd,%6hd,%6hd,%6hd", mapname, &x, &y, &xs, &ys)) < 1 || sscanf(w3, "%23[^,],%11d", mobname, &mob_lv) < 1 || (w4count = sscanf(w4, "%23[^,],%11d,%11u,%11u,%77[^,],%11d,%11d[^\t\r\n]", sprite, &num, &delay, &mob.delay2, mob.eventname, &size, &ai)) < 2) {
	...
	if (battle_config.force_random_spawn || (mob.x == 0 && mob.y == 0) || (mob.xs == 1 && mob.ys == 1 && !map_getcell(mob.m, mob.x, mob.y, CELL_CHKREACH))) { //Force a random spawn anywhere on the map.
	...
	if (battle_config.dynamic_mobs) {
		for (int32 i = 0; i < map_num; i++) {
			for (int16 j = 0; j < MAX_MOB_LIST_PER_MAP; j++) {
				...
				if (mapdata->mob_delete_timer != INVALID_TIMER) { // Mobs were removed anyway,so delete the timer [Inkfish]
