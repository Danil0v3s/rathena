...
struct Script_Config script_config = {
    ...
    1,
    65535,
    2048, //warn_func_mismatch_paramnum/check_cmdcount/check_gotocount
    0,
    INT_MAX, // input_min_value/input_max_value
...
static void script_reportfunc(struct script_state* st) {
	...
	if (!data_isreference(data) || str_data[reference_getid(data)].type != C_FUNC) { // script currently not executing a built-in function or corrupt stack
		return;
	}
...
int32 add_str(const char* p) {
	...
	if (str_hash[h] == 0) { // empty bucket, add new node here
		str_hash[h] = str_num;
	} else { // scan for end of list, or occurrence of identical string
		int32 i;
...
void set_label(int32 l, int32 pos, const char* script_pos_cur) {
	...
	if (str_data[l].type == C_INT || str_data[l].type == C_PARAM || str_data[l].type == C_FUNC) { //Prevent overwriting constants values, parameters and built-in functions [Skotlex]
...
static const char* parse_callfunc(const char* p, int32 require_paren, int32 is_custom) {
	const char* p2;
...
const char* parse_variable(const char* p) {
	...
	if (type == C_ADD_POST || type == C_SUB_POST) { // post ++ / --
		...
		if (type != C_EQ) { // push the type of modifier onto the stack
			add_scriptc(type);
		}
	}
...
const char* parse_line(const char* p) {
	...
	if (p2 != nullptr) { // variable assignment processed so leave the method
	...
	switch (*p) {
		...
		case 'F':
			if (p2 - p == 3 && !strncasecmp(p, "for", 3)) {
				...
				set_label(l, script_pos, p);
				return p;
			} else if (p2 - p == 8 && strncasecmp(p, "function", 8) == 0) { // internal script function
				...
				if (*p2 == ';') { // function <name> ;
					...
					return p;
				} else if (*p2 == '{') { // function <name> <line/block of code>
					char label[256];
					int32 l;
...
static void add_buildin_func(void) {
	...
	for (i = 0; buildin_func[i].func; i++) {
		...
		if (*p != 0) {
			...
			if (!strcmp(buildin_func[i].name, "setr"))
				buildin_set_ref = n;
			else if (!strcmp(buildin_func[i].name, "callsub"))
				buildin_callsub_ref = n;
			else if (!strcmp(buildin_func[i].name, "callfunc"))
				buildin_callfunc_ref = n;
			else if (!strcmp(buildin_func[i].name, "getelementofarray"))
				buildin_getelementofarray_ref = n;
		}
	}
}
...
struct script_code* parse_script_(const char* src, const char* file, int32 line, int32 options, const char* src_file, int32 src_line, const char* src_func) {
	...
	if (options & SCRIPT_IGNORE_EXTERNAL_BRACKETS) {                 // does not require brackets around the script
		if (*p == '\0' && !(options & SCRIPT_RETURN_EMPTY_SCRIPT)) { // empty script and can return nullptr
			aFree(script_buf);
			script_pos = 0;
			script_size = 0;
			script_buf = nullptr;
		...
		p = skip_space(p + 1);
		if (*p == '}' && !(options & SCRIPT_RETURN_EMPTY_SCRIPT)) { // empty script and can return nullptr
			aFree(script_buf);
			script_pos = 0;
			script_size = 0;
			script_buf = nullptr;
	...
	if (postfix == '$') { // string variable
		...
		switch (prefix) {
			...
			case '#':
				...
				break;
			case '.': {
				struct DBMap* n = data->ref ? data->ref->vars : name[1] == '@' ? st->stack->scope.vars
...
void script_array_ensure_zero(struct script_state* st, map_session_data* sd, int64 uid, struct reg_db* ref) {
	...
	struct reg_db* src = script_array_src(st, sd ? sd : st->rid ? map_id2sd(st->rid)
	                                                            : nullptr,
	    name, ref);
	bool insert = false;
...
bool set_reg_str(struct script_state* st, map_session_data* sd, int64 num, const char* name, const char* value, struct reg_db* ref) {
	...
	switch (prefix) {
		...
		case '.': {
			struct reg_db* n = (ref) ? ref : (name[1] == '@') ? &st->stack->scope
			                                                  : &st->script->local;
...
const char* conv_str_(struct script_state* st, struct script_data* data, map_session_data* sd) {
	...
	} else if (data_isint(data)) { // int32 -> string
...
int64 conv_num_(struct script_state* st, struct script_data* data, map_session_data* sd) {
	...
	} else if (data_isstring(data)) { // string -> int32
...
int64 get_num(unsigned char* script, int32* pos) {
	...
	for (i = 0; script[*pos + i] >= 0xc0; i++)
		; // do nothing just find it
	...
	switch (op) {
		case C_ADD:
			if (data_isint(left) && data_isstring(right)) { // convert int-string to string-string
				conv_str(st, left);
			} else if (data_isstring(left) && data_isint(right)) { // convert string-int32 to string-string
				conv_str(st, right);
			}
			break;
	}
...
static void script_check_buildin_argtype(struct script_state* st, int32 func) {
	...
	for (idx = 2; script_hasdata(st, idx); idx++) {
		...
		if (type == '?' || type == '*') { // optional argument or unknown number of optional parameters ( no types are after this )
			...
			if (data_isreference(data)) { // get name for variables to determine the type they refer to
				name = reference_getname(data);
			}
			...
			switch (type) {
				...
				case 's':
					...
					break;
				case 'i':
					if (!data_isint(data) && !(data_isreference(data) && (reference_toparam(data) || reference_toconstant(data) || !is_string_variable(name)))) { // int32 ( params and constants are always int32 )
...
void run_script_main(struct script_state* st) {
	...
	if (st->sleep.tick > 0) {
		...
		//Dispose of script.
		if ((sd = map_id2sd(st->rid)) != nullptr) { //Restore previous stack and save char.
			if (sd->state.using_fake_npc) {
				clif_clearunit_single(sd->npc_id, CLR_OUTSIGHT, *sd);
				sd->state.using_fake_npc = 0;
			}
...
BUILDIN_FUNC(menu) {
	...
	if (sd->state.menu_or_input == 0) {
		...
		if (sd->npc_menu >= 0xff) { // client supports only up to 254 entries; 0 is not used and 255 is reserved for cancel; excess entries are displayed but cause 'uint8' overflow
		...
		if (sd->npc_menu > 0) { // Invalid selection
			ShowDebug("buildin_menu: Selection is out of range (%d pairs are missing?) - please report this\n", sd->npc_menu);
			st->state = END;
			return SCRIPT_CMD_FAILURE;
		}
		if (!data_islabel(script_getdata(st, i + 1))) { // TODO remove this temporary crash-prevention code (fallback for multiple scripts requesting user input)
...
BUILDIN_FUNC(warpguild) {
	...
	if (type == 2 && !script_rid2sd(sd)) { // "SavePoint" uses save point of the currently attached player
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(countinarray) {
	...
	if (i > array_size1 - 1 || j > array_size2 - 1) { //To prevent unintended behavior
...
BUILDIN_FUNC(getnameditem) {
	...
	if (!script_rid2sd(sd)) { //Player not attached!
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	if (!item_db.exists(nameid) /* || itemdb_isstackable(nameid)*/) { //Even though named stackable items "could" be risky, they are required for certain quests.
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
static bool buildin_delitem_search(map_session_data* sd, struct item* it, uint8 exact_match, uint8 loc) {
	...
	for (;;) {
		...
		for (i = 0; amount && i < size; i++) {
			...
			if (exact_match) {
				if ((exact_match & 0x1) && (itm->identify != it->identify || itm->attribute != it->attribute || itm->enchantgrade != it->enchantgrade || memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching exact attributes
					continue;
				}
				if (exact_match & 0x2) {
					uint8 j;
		...
		if (amount == 0 || important == 0) { // either everything was already consumed or no items were skipped
			...
			for (i = 0; amount && i < size; i++) {
				...
				if (itemdb_type(itm->nameid) == IT_PETEGG && itm->card[0] == CARD0_PET && CheckForCharServer()) { // pet which cannot be deleted
					continue;
				}

				if (exact_match) {
					if ((exact_match & 0x1) && (itm->refine != it->refine || itm->identify != it->identify || itm->attribute != it->attribute || itm->enchantgrade != it->enchantgrade || memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching attributes
						continue;
					}
					if (exact_match & 0x2) {
						uint8 j;
...
BUILDIN_FUNC(successrefitem) {
	...
	if (i >= 0) {
		...
		if (sd->inventory.u.items_inventory[i].refine == battle_config.blacksmith_fame_refine_threshold &&
		    ...
		    sd->inventory_data[i]->type == IT_WEAPON) { // Fame point system [DracoRPG]
 ...
 *------------------------------------------*/
static int32 buildin_killmonster_sub_strip(block_list* bl, va_list ap) { //same fix but with killmonster instead - stripping events from mobs.
...
static int32 buildin_killmonsterall_sub_strip(block_list* bl, va_list ap) { //Strips the event from the mob if it's killed the old method.
...
BUILDIN_FUNC(sc_start) {
	...
	if (tick == 0 && val1 > 0 && type > SC_NONE && type < SC_MAX && (skill_id = status_db.getSkill(type)) > 0) { // When there isn't a duration specified, try to get it from the skill_db
...
BUILDIN_FUNC(warpwaitingpc) {
	...
	for (i = 0; i < n && cd->users > 0; i++) {
		...
		if (strcmp(map_name, "SavePoint") == 0 && map_getmapflag(sd->m, MF_NOTELEPORT)) { // can't teleport on this map
			break;
		}
...
BUILDIN_FUNC(guardian) {
	...
	if (script_hasdata(st, 8)) { // "<event label>",<guardian index>
		...
		if (script_isstring(st, 7)) { // "<event label>"
			evt = script_getstr(st, 7);
		} else { // <guardian index>
...
BUILDIN_FUNC(guardianinfo) {
	...
	if (type == 0)
		...
		script_pushint(st, -1);
	else if ((gd = map_id2md(gc->guardian[id].id)) == nullptr)
		script_pushint(st, -1);
	else {
		if (type == 1)
...
BUILDIN_FUNC(petloot) {
	...
	pd = sd->pd;
	if (pd->loot != nullptr) { //Release whatever was there already and reallocate memory
	...
	switch (type) {
		...
		case 1: {
			...
			for (i = 0; i < MAX_PARTY; i++) {
				...
				if ((!(pl_sd = p->data[i].sd) || pl_sd->status.party_id != p_id) || (map_idx && pl_sd->m != map_idx))
					continue;
		...
		case 2: {
			...
			for (i = 0; i < MAX_GUILD; i++) {
				map_session_data* pl_sd;
				if ((!(pl_sd = g->guild.member[i].sd) || pl_sd->status.guild_id != g_id) || (map_idx && pl_sd->m != map_idx))
					continue;
				recovery_sub(pl_sd, revive);
			}
			break;
	...
	for (i = 0; id != 0; i++) {
		...
		for (j = 0; j < EQI_MAX; j++) {
			...
			if (itemdb_type(id) != IT_CARD) {
				...
				for (k = 0; k < MAX_SLOTS; k++) { //New hash system which should support up to 4 slots on any equipment. [Skotlex]
					uint32 hash = 0;
					if (sd->inventory.u.items_inventory[index].card[k] != id)
						continue;
...
BUILDIN_FUNC(setbattleflag) {
	...
	if (battle_set_value(flag, value) == 0)
		...
		if (script_hasdata(st, 4) && script_getnum(st, 4)) { // Only attempt to reload monster data if told to
			const char* expdrop_flags[] = {
			    // Only certain flags require a reload, check for those types
			    "base_exp_rate",
			    "job_exp_rate",
			    "mvp_exp_rate",
			    "quest_exp_rate",
			    ...
			    "item_rate_mvp_mode",
			    "item_rate_treasure",
			    "item_rate_treasure_min",
			    "item_rate_treasure_max",
	...
	switch (bl->type) {
		case BL_MOB: {
			...
			switch (type) {
				...
				case UMOB_LEVEL:
					md->level = (uint16)value;
					clif_name_area(md);
					break;
				case UMOB_HP:
					md->base_status->hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					clif_name_area(md);
					break;
				case UMOB_MAXHP:
					md->base_status->hp = md->base_status->max_hp = (uint32)value;
					status_set_maxhp(bl, (uint32)value, 0);
					clif_name_area(md);
					break;
				case UMOB_MASTERAID: md->master_id = value; break;
				case UMOB_MAPID:
					if (mapname)
						value = map_mapname2mapid(mapname);
					unit_warp(bl, (int16)value, 0, 0, CLR_TELEPORT);
					break;
				case UMOB_X:
					if (!unit_walktoxy(bl, (int16)value, md->y, 2))
						unit_movepos(bl, (int16)value, md->y, 0, 0);
					break;
				case UMOB_Y:
					if (!unit_walktoxy(bl, md->x, (int16)value, 2))
						unit_movepos(bl, md->x, (int16)value, 0, 0);
					break;
				case UMOB_SPEED:
					md->base_status->speed = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_MODE:
				...
				case UMOB_SEX:
					...
					break;
				case UMOB_CLASS:
					status_set_viewdata(bl, (uint16)value);
					unit_refresh(bl);
					break;
				...
				case UMOB_STR:
					md->base_status->str = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_AGI:
					md->base_status->agi = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_VIT:
					md->base_status->vit = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_INT:
					md->base_status->int_ = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_DEX:
					md->base_status->dex = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				case UMOB_LUK:
					md->base_status->luk = (uint16)value;
					status_calc_misc(bl, &md->status, md->level);
					calc_status = true;
					break;
				...
				case UMOB_ATKRANGE:
					md->base_status->rhw.range = (uint16)value;
					calc_status = true;
					break;
				case UMOB_ATKMIN:
					md->base_status->rhw.atk = (uint16)value;
					calc_status = true;
					break;
				case UMOB_ATKMAX:
					md->base_status->rhw.atk2 = (uint16)value;
					calc_status = true;
					break;
				case UMOB_MATKMIN:
					md->base_status->matk_min = (uint16)value;
					calc_status = true;
					break;
				case UMOB_MATKMAX:
					md->base_status->matk_max = (uint16)value;
					calc_status = true;
					break;
				case UMOB_DEF:
					md->base_status->def = (defType)value;
					calc_status = true;
					break;
				case UMOB_MDEF:
					md->base_status->mdef = (defType)value;
					calc_status = true;
					break;
				case UMOB_HIT:
					md->base_status->hit = (int16)value;
					calc_status = true;
					break;
				case UMOB_FLEE:
					md->base_status->flee = (int16)value;
					calc_status = true;
					break;
				case UMOB_PDODGE:
					md->base_status->flee2 = (int16)value;
					calc_status = true;
					break;
				case UMOB_CRIT:
					md->base_status->cri = (int16)value;
					calc_status = true;
					break;
				case UMOB_RACE: md->status.race = md->base_status->race = (unsigned char)value; break;
				case UMOB_ELETYPE:
					md->base_status->def_ele = (unsigned char)value;
					calc_status = true;
					break;
				case UMOB_ELELEVEL:
					md->base_status->ele_lv = (unsigned char)value;
					calc_status = true;
					break;
				case UMOB_AMOTION:
					md->base_status->amotion = (int16)value;
					calc_status = true;
					break;
				case UMOB_ADELAY:
					md->base_status->adelay = (int16)value;
					calc_status = true;
					break;
				case UMOB_DMOTION:
					md->base_status->dmotion = (int16)value;
					calc_status = true;
					break;
				case UMOB_TARGETID: {
					if (value == 0) {
		...
		case BL_HOM: {
			...
			switch (type) {
				...
				case UHOM_HP:
					hd->base_status.hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					break;
				case UHOM_MAXHP:
					hd->base_status.hp = hd->base_status.max_hp = (uint32)value;
					status_set_maxhp(bl, (uint32)value, 0);
					break;
				case UHOM_SP:
					hd->base_status.sp = (uint32)value;
					status_set_sp(bl, (uint32)value, 0);
					break;
				case UHOM_MAXSP:
					hd->base_status.sp = hd->base_status.max_sp = (uint32)value;
					status_set_maxsp(bl, (uint32)value, 0);
					break;
				...
				case UHOM_MAPID:
					...
					break;
				case UHOM_X:
					if (!unit_walktoxy(bl, (int16)value, hd->y, 2))
						unit_movepos(bl, (int16)value, hd->y, 0, 0);
					break;
				case UHOM_Y:
					if (!unit_walktoxy(bl, hd->x, (int16)value, 2))
						unit_movepos(bl, hd->x, (int16)value, 0, 0);
					break;
				case UHOM_HUNGER:
					hd->homunculus.hunger = (int16)value;
				...
				case UHOM_INTIMACY:
					...
					break;
				case UHOM_SPEED:
					hd->base_status.speed = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_LOOKDIR: unit_setdir(bl, (uint8)value); break;
				case UHOM_CANMOVETICK: hd->ud.canmove_tick = value > 0 ? (uint32)value : 0; break;
				case UHOM_STR:
					hd->base_status.str = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_AGI:
					hd->base_status.agi = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_VIT:
					hd->base_status.vit = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_INT:
					hd->base_status.int_ = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_DEX:
					hd->base_status.dex = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_LUK:
					hd->base_status.luk = (uint16)value;
					status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
					calc_status = true;
					break;
				case UHOM_DMGIMMUNE: hd->ud.immune_attack = value > 0; break;
				case UHOM_ATKRANGE:
					hd->base_status.rhw.range = (uint16)value;
					calc_status = true;
					break;
				case UHOM_ATKMIN:
					hd->base_status.rhw.atk = (uint16)value;
					calc_status = true;
					break;
				case UHOM_ATKMAX:
					hd->base_status.rhw.atk2 = (uint16)value;
					calc_status = true;
					break;
				case UHOM_MATKMIN:
					hd->base_status.matk_min = (uint16)value;
					calc_status = true;
					break;
				case UHOM_MATKMAX:
					hd->base_status.matk_max = (uint16)value;
					calc_status = true;
					break;
				case UHOM_DEF:
					hd->base_status.def = (defType)value;
					calc_status = true;
					break;
				case UHOM_MDEF:
					hd->base_status.mdef = (defType)value;
					calc_status = true;
					break;
				case UHOM_HIT:
					hd->base_status.hit = (int16)value;
					calc_status = true;
					break;
				case UHOM_FLEE:
					hd->base_status.flee = (int16)value;
					calc_status = true;
					break;
				case UHOM_PDODGE:
					hd->base_status.flee2 = (int16)value;
					calc_status = true;
					break;
				case UHOM_CRIT:
					hd->base_status.cri = (int16)value;
					calc_status = true;
					break;
				case UHOM_RACE: hd->battle_status.race = hd->base_status.race = (unsigned char)value; break;
				case UHOM_ELETYPE:
					hd->base_status.def_ele = (unsigned char)value;
					calc_status = true;
					break;
				case UHOM_ELELEVEL:
					hd->base_status.ele_lv = (unsigned char)value;
					calc_status = true;
					break;
				case UHOM_AMOTION:
					hd->base_status.amotion = (int16)value;
					calc_status = true;
					break;
				case UHOM_ADELAY:
					hd->base_status.adelay = (int16)value;
					calc_status = true;
					break;
				case UHOM_DMOTION:
					hd->base_status.dmotion = (int16)value;
					calc_status = true;
					break;
				case UHOM_TARGETID: {
					if (value == 0) {
		...
		case BL_PET: {
			...
			switch (type) {
				...
				case UPET_HP:
					pd->status.hp = pd->status.max_hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					break;
				case UPET_MAXHP:
					pd->status.hp = pd->status.max_hp = (uint32)value;
				...
				case UPET_MAPID:
					...
					break;
				case UPET_X:
					if (!unit_walktoxy(bl, (int16)value, pd->y, 2))
						unit_movepos(bl, (int16)value, pd->y, 0, 0);
					break;
				case UPET_Y:
					if (!unit_walktoxy(bl, pd->x, (int16)value, 2))
						unit_movepos(bl, pd->x, (int16)value, 0, 0);
					break;
				case UPET_HUNGER:
					pd->pet.hungry = cap_value((int16)value, 0, 100);
					clif_send_petdata(map_id2sd(pd->pet.account_id), *pd, CHANGESTATEPET_HUNGER);
					break;
				case UPET_INTIMACY:
					pet_set_intimate(pd, (uint32)value);
					clif_send_petdata(map_id2sd(pd->pet.account_id), *pd, CHANGESTATEPET_INTIMACY);
					break;
				case UPET_SPEED:
					pd->status.speed = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_LOOKDIR: unit_setdir(bl, (uint8)value); break;
				case UPET_CANMOVETICK: pd->ud.canmove_tick = value > 0 ? (uint32)value : 0; break;
				case UPET_STR:
					pd->status.str = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_AGI:
					pd->status.agi = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_VIT:
					pd->status.vit = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_INT:
					pd->status.int_ = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_DEX:
					pd->status.dex = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
				case UPET_LUK:
					pd->status.luk = (uint16)value;
					status_calc_misc(bl, &pd->status, pd->pet.level);
					break;
		...
		case BL_MER: {
			...
			switch (type) {
				...
				case UMER_HP:
					mc->base_status.hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					break;
				case UMER_MAXHP:
					mc->base_status.hp = mc->base_status.max_hp = (uint32)value;
					status_set_maxhp(bl, (uint32)value, 0);
					break;
				...
				case UMER_MAPID:
					...
					break;
				case UMER_X:
					if (!unit_walktoxy(bl, (int16)value, mc->y, 2))
						unit_movepos(bl, (int16)value, mc->y, 0, 0);
					break;
				case UMER_Y:
					if (!unit_walktoxy(bl, mc->x, (int16)value, 2))
						unit_movepos(bl, mc->x, (int16)value, 0, 0);
					break;
				...
				case UMER_SPEED:
					mc->base_status.speed = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_LOOKDIR: unit_setdir(bl, (uint8)value); break;
				case UMER_CANMOVETICK: mc->ud.canmove_tick = value > 0 ? (uint32)value : 0; break;
				case UMER_STR:
					mc->base_status.str = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_AGI:
					mc->base_status.agi = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_VIT:
					mc->base_status.vit = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_INT:
					mc->base_status.int_ = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_DEX:
					mc->base_status.dex = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_LUK:
					mc->base_status.luk = (uint16)value;
					status_calc_misc(bl, &mc->base_status, mc->db->lv);
					calc_status = true;
					break;
				case UMER_DMGIMMUNE: mc->ud.immune_attack = value > 0; break;
				case UMER_ATKRANGE:
					mc->base_status.rhw.range = (uint16)value;
					calc_status = true;
					break;
				case UMER_ATKMIN:
					mc->base_status.rhw.atk = (uint16)value;
					calc_status = true;
					break;
				case UMER_ATKMAX:
					mc->base_status.rhw.atk2 = (uint16)value;
					calc_status = true;
					break;
				case UMER_MATKMIN:
					mc->base_status.matk_min = (uint16)value;
					calc_status = true;
					break;
				case UMER_MATKMAX:
					mc->base_status.matk_max = (uint16)value;
					calc_status = true;
					break;
				case UMER_DEF:
					mc->base_status.def = (defType)value;
					calc_status = true;
					break;
				case UMER_MDEF:
					mc->base_status.mdef = (defType)value;
					calc_status = true;
					break;
				case UMER_HIT:
					mc->base_status.hit = (int16)value;
					calc_status = true;
					break;
				case UMER_FLEE:
					mc->base_status.flee = (int16)value;
					calc_status = true;
					break;
				case UMER_PDODGE:
					mc->base_status.flee2 = (int16)value;
					calc_status = true;
					break;
				case UMER_CRIT:
					mc->base_status.cri = (int16)value;
					calc_status = true;
					break;
				case UMER_RACE: mc->battle_status.race = mc->base_status.race = (unsigned char)value; break;
				case UMER_ELETYPE:
					mc->base_status.def_ele = (unsigned char)value;
					calc_status = true;
					break;
				case UMER_ELELEVEL:
					mc->base_status.ele_lv = (unsigned char)value;
					calc_status = true;
					break;
				case UMER_AMOTION:
					mc->base_status.amotion = (int16)value;
					calc_status = true;
					break;
				case UMER_ADELAY:
					mc->base_status.adelay = (int16)value;
					calc_status = true;
					break;
				case UMER_DMOTION:
					mc->base_status.dmotion = (int16)value;
					calc_status = true;
					break;
				case UMER_TARGETID: {
					if (value == 0) {
		...
		case BL_ELEM: {
			...
			switch (type) {
				...
				case UELE_HP:
					ed->base_status.hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					break;
				case UELE_MAXHP:
					ed->base_status.hp = ed->base_status.max_hp = (uint32)value;
					status_set_maxhp(bl, (uint32)value, 0);
					break;
				case UELE_SP:
					ed->base_status.sp = (uint32)value;
					status_set_sp(bl, (uint32)value, 0);
					break;
				case UELE_MAXSP:
					ed->base_status.sp = ed->base_status.max_sp = (uint32)value;
					status_set_maxsp(bl, (uint32)value, 0);
					break;
				...
				case UELE_MAPID:
					...
					break;
				case UELE_X:
					if (!unit_walktoxy(bl, (int16)value, ed->y, 2))
						unit_movepos(bl, (int16)value, ed->y, 0, 0);
					break;
				case UELE_Y:
					if (!unit_walktoxy(bl, ed->x, (int16)value, 2))
						unit_movepos(bl, ed->x, (int16)value, 0, 0);
					break;
				case UELE_LIFETIME: ed->elemental.life_time = (uint32)value; break;
				case UELE_MODE:
					ed->elemental.mode = (enum e_mode)value;
					calc_status = true;
					break;
				case UELE_SPEED:
					ed->base_status.speed = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_LOOKDIR: unit_setdir(bl, (uint8)value); break;
				case UELE_CANMOVETICK: ed->ud.canmove_tick = value > 0 ? (uint32)value : 0; break;
				case UELE_STR:
					ed->base_status.str = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_AGI:
					ed->base_status.agi = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_VIT:
					ed->base_status.vit = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_INT:
					ed->base_status.int_ = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_DEX:
					ed->base_status.dex = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_LUK:
					ed->base_status.luk = (uint16)value;
					status_calc_misc(bl, &ed->base_status, ed->db->lv);
					calc_status = true;
					break;
				case UELE_DMGIMMUNE: ed->ud.immune_attack = value > 0; break;
				case UELE_ATKRANGE:
					ed->base_status.rhw.range = (uint16)value;
					calc_status = true;
					break;
				case UELE_ATKMIN:
					ed->base_status.rhw.atk = (uint16)value;
					calc_status = true;
					break;
				case UELE_ATKMAX:
					ed->base_status.rhw.atk2 = (uint16)value;
					calc_status = true;
					break;
				case UELE_MATKMIN:
					ed->base_status.matk_min = (uint16)value;
					calc_status = true;
					break;
				case UELE_MATKMAX:
					ed->base_status.matk_max = (uint16)value;
					calc_status = true;
					break;
				case UELE_DEF:
					ed->base_status.def = (defType)value;
					calc_status = true;
					break;
				case UELE_MDEF:
					ed->base_status.mdef = (defType)value;
					calc_status = true;
					break;
				case UELE_HIT:
					ed->base_status.hit = (int16)value;
					calc_status = true;
					break;
				case UELE_FLEE:
					ed->base_status.flee = (int16)value;
					calc_status = true;
					break;
				case UELE_PDODGE:
					ed->base_status.flee2 = (int16)value;
					calc_status = true;
					break;
				case UELE_CRIT:
					ed->base_status.cri = (int16)value;
					calc_status = true;
					break;
				case UELE_RACE: ed->battle_status.race = ed->base_status.race = (unsigned char)value; break;
				case UELE_ELETYPE:
					ed->base_status.def_ele = (unsigned char)value;
					calc_status = true;
					break;
				case UELE_ELELEVEL:
					ed->base_status.ele_lv = (unsigned char)value;
					calc_status = true;
					break;
				case UELE_AMOTION:
					ed->base_status.amotion = (int16)value;
					calc_status = true;
					break;
				case UELE_ADELAY:
					ed->base_status.adelay = (int16)value;
					calc_status = true;
					break;
				case UELE_DMOTION:
					ed->base_status.dmotion = (int16)value;
					calc_status = true;
					break;
				case UELE_TARGETID: {
					if (value == 0) {
		...
		case BL_NPC: {
			...
			switch (type) {
				...
				case UNPC_HP:
					nd->status.hp = (uint32)value;
					status_set_hp(bl, (uint32)value, 0);
					break;
				case UNPC_MAXHP:
					nd->status.hp = nd->status.max_hp = (uint32)value;
					status_set_maxhp(bl, (uint32)value, 0);
					break;
				case UNPC_MAPID:
					if (mapname)
					...
					break;
				case UNPC_X:
					if (!unit_walktoxy(bl, (int16)value, nd->y, 2))
						unit_movepos(bl, (int16)value, nd->x, 0, 0);
					break;
				case UNPC_Y:
					if (!unit_walktoxy(bl, nd->x, (int16)value, 2))
						unit_movepos(bl, nd->x, (int16)value, 0, 0);
					break;
				case UNPC_LOOKDIR: unit_setdir(bl, (uint8)value); break;
				case UNPC_STR:
					nd->params.str = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				case UNPC_AGI:
					nd->params.agi = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				case UNPC_VIT:
					nd->params.vit = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				case UNPC_INT:
					nd->params.int_ = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				case UNPC_DEX:
					nd->params.dex = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				case UNPC_LUK:
					nd->params.luk = (uint16)value;
					status_calc_misc(bl, &nd->status, nd->level);
					break;
				...
				case UNPC_DEADSIT:
					nd->vd.dead_sit = (char)value;
					unit_refresh(bl);
					break;
				case UNPC_GROUP_ID:
					nd->ud.group_id = value;
					unit_refresh(bl);
					break;
				default:
...
BUILDIN_FUNC(getvariableofnpc) {
	...
	nd = npc_name2id(script_getstr(st, 3));
	if (nd == nullptr || nd->subtype != NPCTYPE_SCRIPT || nd->u.scr.script == nullptr) { // NPC not found or has no script
...
BUILDIN_FUNC(waitingroom2bg) {
	...
	if ((bg_id = bg_create(mapindex, &team)) == 0) { // Creation failed
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(setdragon) {
	...
	if (!pc_checkskill(sd, RK_DRAGONTRAINING) || (sd->class_ & MAPID_THIRDMASK) != MAPID_RUNE_KNIGHT)
		...
		if (color) {
			option = (color == 1 ? OPTION_DRAGON1 : color == 2 ? OPTION_DRAGON2
...
BUILDIN_FUNC(countbound) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid > 0 && ((!type && sd->inventory.u.items_inventory[i].bound) || (type && sd->inventory.u.items_inventory[i].bound == type))) {
...
BUILDIN_FUNC(reputationui) {
	...
	} else if (!script_rid2sd(sd)) { //Player not attached!
		st->state = END;
		return SCRIPT_CMD_FAILURE;
	}
