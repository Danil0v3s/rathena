...
#define WH_CB(name) \
	static void name##_cb(const v8::FunctionCallbackInfo<v8::Value>& info)
		...
		WH_CB(guardianInfo);
		WH_CB(killMonster);
		WH_CB(killMonsterAll);
		WH_CB(mobCount);
		WH_CB(respawnGuildOwned);
		WH_CB(getRandomMobId);
		WH_CB(getMonsterInfo);
		WH_CB(getMobDrops);
		WH_CB(mobInfo);
		WH_CB(unitWalk);
		WH_CB(unitWalkToTarget);
		WH_CB(unitAttack);
		WH_CB(unitKill);
		WH_CB(unitWarp);
		WH_CB(unitTalk);
		WH_CB(unitSkillUseId);
		WH_CB(unitSkillUsePos);
		WH_CB(unitStopAttack);
		WH_CB(unitStopWalk);
		WH_CB(unitExists);
		WH_CB(getUnitType);
		WH_CB(getUnitName);
		...
		WH_CB(getMapUsers);
		WH_CB(getAreaUsers);
		WH_CB(getServerUsers);
		WH_CB(isLoggedIn);
		WH_CB(ridToName);
		WH_CB(getAreaDropItem);
		WH_CB(mapIdToName);
		WH_CB(getMapXY);
		WH_CB(distance);
		WH_CB(setCell);
		...
		WH_CB(warpParty);
		WH_CB(warpGuild);
		WH_CB(areaPercentHeal);
		WH_CB(attachRid);
		WH_CB(addRid);
		WH_CB(playerAttached);
		WH_CB(getAttachedRid);
		WH_CB(setMapFlag);
		WH_CB(removeMapFlag);
		WH_CB(getMapFlag);
		WH_CB(setMapFlagNoSave);
		WH_CB(day);
		WH_CB(night);
		WH_CB(isDay);
		WH_CB(isNight);
