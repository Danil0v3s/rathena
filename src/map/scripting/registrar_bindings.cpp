...
namespace rathena::scripting {
	...
	namespace {
		...
		void install_hooks(v8::Isolate* iso, NpcRegistration& reg, const HookSnapshot& h) {
			if (!h.on_click.IsEmpty())
				reg.on_click.Reset(iso, h.on_click);
		}
		...
		read_npc_with_duplicates(v8::Isolate* iso, v8::Local<v8::Context> ctx,
			...
			for (uint32_t i = 0; i < own_names->Length(); ++i) {
				...
				if (!own_names->Get(ctx, i).ToLocal(&key_v) || !key_v->IsString())
					continue;
				v8::String::Utf8Value key_utf8(iso, key_v);
				if (*key_utf8 == nullptr)
					continue;
				if (strncmp(*key_utf8, "On", 2) != 0)
					continue;
				v8::Local<v8::Value> val_v;
				if (!obj->Get(ctx, key_v).ToLocal(&val_v) || !val_v->IsFunction())
					continue;
			...
			if (obj->Get(ctx, dups_key).ToLocal(&dups_val) && !dups_val.IsEmpty() && !dups_val->IsUndefined() && !dups_val->IsNull()) {
				if (!dups_val->IsArray()) {
					throw ScriptRegistrationError(
				...
				for (uint32_t i = 0; i < dups->Length(); ++i) {
					...
					if (!dups->Get(ctx, i).ToLocal(&dup_val))
						continue;
