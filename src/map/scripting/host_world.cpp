...
#define UNWRAP_OPT                              \
	auto* self = args::unwrap<WorldHost>(info); \
	if (!self)                                  \
		return;
#define UNWRAP_REQ         \
	UNWRAP_OPT;            \
	...
	void WorldHost::spawnGuardian_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		ret_int(info, mob_spawn_guardian(mn.c_str(), static_cast<int16>(x), static_cast<int16>(y), display.c_str(), mob, evt.c_str(), guard_idx, guard_idx >= 0));
	}
	...
	namespace {
		...
		int32 mobcount_sub(struct block_list* bl, va_list ap) {
			auto* md = BL_CAST(BL_MOB, bl);
			const char* event = va_arg(ap, const char*);
			if (!md || md->status.hp < 1)
				return 0;
			if (event && strcmp(event, md->npc_event) != 0)
				return 0;
			return 1;
		}
	} // namespace
	...
	void WorldHost::respawnGuildOwned_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		if (flag & 4)
			map_foreachinmap(respawnguild_mob_sub, m, BL_MOB);
	}
	...
	void WorldHost::unitWarp_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		auto* bl = map_id2bl(gid);
		if (bl)
			unit_warp(bl, map_mapname2mapid(mname.c_str()), x, y, CLR_TELEPORT);
	}
	void WorldHost::unitTalk_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		auto* bl = map_id2bl(gid);
		if (bl)
			clif_disp_overhead(bl, text.c_str());
	}
	void WorldHost::unitSkillUseId_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		auto* src = map_id2bl(gid);
		if (src)
			unit_skilluse_id(src, gid, sid, sl);
	}
	void WorldHost::unitSkillUsePos_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		auto* src = map_id2bl(gid);
		if (src)
			unit_skilluse_pos(src, x, y, sid, sl);
	}
	...
	void WorldHost::setUnitName_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		switch (bl->type) {
			case BL_MOB: {
				auto* md = map_id2md(gid);
				if (md)
					safestrncpy(md->name, name.c_str(), NAME_LENGTH);
				break;
			}
			case BL_HOM: {
				auto* hd = map_id2hd(gid);
				if (hd)
					safestrncpy(hd->homunculus.name, name.c_str(), NAME_LENGTH);
				break;
			}
			case BL_PET: {
				auto* pd = map_id2pd(gid);
				if (pd)
					safestrncpy(pd->pet.name, name.c_str(), NAME_LENGTH);
				break;
			}
			default: return;
		}
	...
	void WorldHost::getUnitTitle_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		ret_str(info, "");
	}
	...
	void WorldHost::getUnitData_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		ret_null(info);
	}
	void WorldHost::setUnitData_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		switch (type) {
			...
			case UMOB_LEVEL:
				md->level = static_cast<uint16>(value);
				clif_name_area(md);
				break;
			case UMOB_HP:
				md->base_status->hp = value;
				status_set_hp(bl, value, 0);
				clif_name_area(md);
				break;
			case UMOB_MAXHP:
				md->base_status->hp = md->base_status->max_hp = value;
				status_set_maxhp(bl, value, 0);
				clif_name_area(md);
				break;
			...
			case UMOB_SPEED:
				md->base_status->speed = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_MODE:
				md->base_status->mode = static_cast<e_mode>(value);
				unit_refresh(bl);
				break;
			case UMOB_CLASS:
				status_set_viewdata(bl, static_cast<uint16>(value));
				unit_refresh(bl);
				break;
			...
			case UMOB_STR:
				md->base_status->str = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_AGI:
				md->base_status->agi = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_VIT:
				md->base_status->vit = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_INT:
				md->base_status->int_ = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_DEX:
				md->base_status->dex = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
			case UMOB_LUK:
				md->base_status->luk = static_cast<uint16>(value);
				status_calc_misc(bl, &md->status, md->level);
				break;
	...
	void WorldHost::warpParty_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		for (int i = 0; i < MAX_PARTY; ++i) {
			auto* pl = p->data[i].sd;
			if (pl)
				pc_setpos(pl, static_cast<uint16>(idx), x, y, CLR_TELEPORT);
		}
	}
	...
	void WorldHost::attachRid_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		(void)info;
	}
	void WorldHost::addRid_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		(void)info;
	}
	...
	void WorldHost::setMapFlag_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		int16 m = map_mapname2mapid(mname.c_str());
		if (m >= 0)
			map_setmapflag(m, static_cast<e_mapflag>(flag), true);
	}
	void WorldHost::removeMapFlag_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		int16 m = map_mapname2mapid(mname.c_str());
		if (m >= 0)
			map_setmapflag(m, static_cast<e_mapflag>(flag), false);
	}
	...
	void WorldHost::day_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		(void)info;
		map_night_timer(0, 0, 0, 1);
	}
	void WorldHost::night_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		(void)info;
		map_day_timer(0, 0, 0, 1);
	}
	void WorldHost::isDay_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		ret_bool(info, !night_flag);
	}
	void WorldHost::isNight_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		ret_bool(info, night_flag);
	}
	...
	static inline void toggle_mapflag(const v8::FunctionCallbackInfo<v8::Value>& info,
		...
		if (m < 0)
			return;
		if (map_getmapflag(m, flag) != (on ? 1 : 0))
			map_setmapflag(m, flag, on);
	}
	void WorldHost::pvpOn_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_PVP, true);
	}
	void WorldHost::pvpOff_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_PVP, false);
	}
	void WorldHost::gvgOn_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_GVG, true);
	}
	void WorldHost::gvgOff_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_GVG, false);
	}
	void WorldHost::gvgOn3_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_GVG_TE, true);
	}
	void WorldHost::gvgOff3_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		toggle_mapflag(info, MF_GVG_TE, false);
	}
	...
	void WorldHost::flagEmblem_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		if (!nd || nd->subtype != NPCTYPE_SCRIPT || gid < 0)
			return;
	...
	void WorldHost::castleData_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		switch (type) {
			...
			default:
				if (type >= 10 && type <= 17)
					v = castle->guardian[type - 10].visible;
				break;
		}
		ret_int(info, v);
	}
	...
	void WorldHost::charCommand_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		atCommand_cb(info);
	}
	void WorldHost::useAtCommand_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		atCommand_cb(info);
	}
	void WorldHost::bindAtCommand_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		if (*c == atcommand_symbol || *c == charcommand_symbol)
			++c;
		// Look up existing
		int i = 0;
		for (; i < atcmd_binding_count; ++i)
			if (strcmp(atcmd_binding[i]->command, c) == 0)
				break;
	...
	void WorldHost::unbindAtCommand_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		...
		for (int j = 0; j < atcmd_binding_count; ++j) {
			if (!atcmd_binding[j])
				continue;
			if (cursor != j)
				atcmd_binding[cursor] = atcmd_binding[j];
			++cursor;
		}
	...
	void WorldHost::groupId_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
		gmLevel_cb(info);
	}
...
#define BIND(name) args::bind_method<WorldHost>(iso, ctx, obj, this, #name, &WorldHost::name##_cb)
		...
		BIND(killMonsterAll);
		BIND(mobCount);
		BIND(respawnGuildOwned);
		BIND(getRandomMobId);
		BIND(getMonsterInfo);
		BIND(getMobDrops);
		BIND(mobInfo);
		BIND(unitWalk);
		BIND(unitWalkToTarget);
		BIND(unitAttack);
		BIND(unitKill);
		BIND(unitWarp);
		BIND(unitTalk);
		BIND(unitSkillUseId);
		BIND(unitSkillUsePos);
		BIND(unitStopAttack);
		BIND(unitStopWalk);
		BIND(unitExists);
		BIND(getUnitType);
