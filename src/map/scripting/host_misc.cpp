...
namespace rathena::scripting {
	...
	namespace {
		...
		void void_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			(void)info;
		}

		void int0_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_int(info, 0);
		}
		void int1_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_int(info, 1);
		}

		void str_empty_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_str(info, "");
		}
		void false_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_bool(info, false);
		}
		void true_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_bool(info, true);
		}
		void null_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			ret_null(info);
		}
	...
	namespace {
		void npc_setDisplay_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (args::has(info, 1))
				npc_setclass(&self->nd(), args::int_arg(info, 1));
		}
		...
		bool nd_is_shop(npc_data* nd) {
			return nd && (nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_CASHSHOP || nd->subtype == NPCTYPE_ITEMSHOP || nd->subtype == NPCTYPE_POINTSHOP || nd->subtype == NPCTYPE_MARKETSHOP);
		}
		...
		std::vector<shop_row> parse_shop_items(const v8::FunctionCallbackInfo<v8::Value>& info, int arg_index) {
			...
			if (info.Length() <= arg_index || !info[arg_index]->IsArray())
				return out;
			auto iso_ = args::iso(info);
			auto cx = args::ctx(info);
			...
			for (uint32 i = 0; i < len; ++i) {
				v8::Local<v8::Value> v;
				if (!arr->Get(cx, i).ToLocal(&v) || !v->IsObject())
					continue;
				auto obj = v8::Local<v8::Object>::Cast(v);
				v8::Local<v8::Value> id_v, price_v;
				if (!obj->Get(cx, key_id).ToLocal(&id_v))
					continue;
				if (!obj->Get(cx, key_price).ToLocal(&price_v))
					continue;
				out.push_back({
		...
		void npc_shopUpdate_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = 0; i < nd.u.shop.count; ++i) {
				if (nd.u.shop.shop_item[i].nameid != id)
					continue;
				if (price != 0) {
					if (price < 0) {
		...
		void npc_warpWaitingPc_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (info.Length() >= 4)
				n = args::int_arg(info, 3);
			for (int i = 0; i < n && cd->users > 0; ++i) {
				auto* sd = cd->usersd[0];
				if (mn == "SavePoint" && map_getmapflag(sd->m, MF_NOTELEPORT))
					break;
				if (cd->zeny) {
					if (static_cast<uint32>(sd->status.zeny) < cd->zeny)
						break;
	...
	namespace {
		...
		void ach_exists_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (uint16 i = 0; i < ad.count; ++i) {
				if (ad.achievements[i].achievement_id == id) {
		...
		void ach_complete_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			int id = args::int_arg(info, 0);
			if (!achievement_db.exists(id)) {
				args::ret_bool(info, false);
				return;
			}
			if (!self->sd().state.pc_loaded) {
				args::ret_bool(info, false);
				return;
			}
			auto& ad = self->sd().achievement_data;
	...
	namespace {
		void storage_openExtra_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (num > 0)
				storage_premiumStorage_load(&self->sd(), static_cast<uint8>(num), static_cast<uint8>(mode));
			else
				storage_premiumStorage_open(&self->sd());
		}
		...
		void storage_countGuildItem_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = 0; i < MAX_GUILD_STORAGE; ++i) {
				if (gstor->u.items_guild[i].nameid == id)
					total += gstor->u.items_guild[i].amount;
			}
			args::ret_int(info, total);
		}
		...
		void storage_guildLog_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (self)
				storage_guild_log_read(&self->sd());
	...
	namespace {
		...
		void hom_addIntimacy_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (value == 0)
				return;
			if (value > 0)
				hom_increase_intimacy(self->sd().hd, static_cast<uint32>(value));
			else
				hom_decrease_intimacy(self->sd().hd, static_cast<uint32>(std::abs(value)));
	...
	namespace {
		...
		void party_changeLeader_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (p)
				party_changeleader(self->sd(), t_sd, p);
		}
	...
	namespace {
		...
		void guild_getAlliance_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			auto g = guild_search(gid);
			if (!g || idx < 0 || idx >= MAX_GUILDALLIANCE) {
				args::ret_int(info, 0);
				return;
			}
		...
		void guild_getMapUsers_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = 0; i < MAX_GUILD; ++i) {
				...
				if (pl && pl->m == m && !pl->state.autotrade)
					++c;
			}
			args::ret_int(info, c);
		}
	...
	namespace {
		...
		int32 instance_warpall_sub(struct block_list* bl, va_list ap) {
			...
			switch (idata->mode) {
				case IM_NONE: break;
				case IM_CHAR:
					if ((int)sd->status.char_id != owner)
						return 0;
					break;
				case IM_PARTY:
					if ((int)sd->status.party_id != owner)
						return 0;
					break;
				case IM_GUILD:
					if ((int)sd->status.guild_id != owner)
						return 0;
					break;
				case IM_CLAN:
					if ((int)sd->status.clan_id != owner)
						return 0;
					break;
				default: break;
			}
		...
		void instance_checkGuild_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = 0; i < MAX_GUILD; ++i) {
				...
				if (!pl_sd || !map_id2bl(pl_sd->id) || pl_sd->state.autotrade)
					continue;
		...
		void instance_setVar_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (is_string_variable(name.c_str())) {
				...
				if (value != 0)
					i64db_i64put(idata->regs.vars, uid, value);
				else
					i64db_remove(idata->regs.vars, uid);
			}
		}
		...
		void instance_checkClan_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = 0; i < MAX_CLAN; ++i) {
				...
				if (!pl || !map_id2bl(pl->id) || pl->state.autotrade)
					continue;
		...
		void instance_list_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			for (int i = instance_start; i < map_num; ++i) {
				auto* md = &map[i];
				if (md->instance_src_map != src_id)
					continue;
				auto idata = util::umap_find(instances, md->instance_id);
				if (!idata)
					continue;
				if (mode != IM_MAX && idata->mode != mode)
					continue;
	...
	namespace {
		...
		void bg_join_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			map_session_data* sd = char_id ? map_charid2sd(char_id) : self ? self->sd()
			                                                               : nullptr;
			if (!sd) {
				args::ret_int(info, 0);
				return;
		...
		void bg_spawnMonster_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			args::ret_int(info, mob_spawn_bg(mapn.c_str(), static_cast<int16>(x), static_cast<int16>(y), disp.c_str(), cls, evt.c_str(), static_cast<uint32>(bg_id)));
		}
	...
	namespace {
		...
		void channel_chat_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			auto* c = channel_name2channel(const_cast<char*>(name.c_str()), self->sd(), 0);
			if (c)
				channel_send(c, self->sd(), msg.c_str());
		}
	} // namespace
	...
	namespace {
		...
		void channel_setColor_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			auto* c = channel_name2channel(const_cast<char*>(name.c_str()), self->sd(), 0);
			if (c)
				c->color = static_cast<uint32>(args::int_arg(info, 1));
		}
		void channel_setPassword_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			auto* c = channel_name2channel(const_cast<char*>(name.c_str()), self->sd(), 0);
			if (c)
				safestrncpy(c->pass, pass.c_str(), sizeof(c->pass));
		}
		void channel_setOption_cb(const v8::FunctionCallbackInfo<v8::Value>& info) {
			...
			if (!c)
				return;
			if (on)
				c->opt |= static_cast<unsigned char>(opt);
			else
				c->opt &= ~static_cast<unsigned char>(opt);
		}
