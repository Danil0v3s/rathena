...
namespace rathena::scripting {
	...
	void ScriptHostImpl::dispatch_sleep_resume(int account_id) {
		...
		if (!session || session->pending_kind != PendingKind::Sleep)
			return;
		...
		isolate_->PerformMicrotaskCheckpoint();
		if (try_catch.HasCaught())
			log_exception(isolate_, try_catch, "sleep_resume");
	}
	...
	void ScriptHost::load_entry_point(const std::string& p) {
		impl_->load_entry_point(p);
	}
	...
	void ScriptHost::dispatch_sleep_resume(int aid) {
		impl_->dispatch_sleep_resume(aid);
	}
