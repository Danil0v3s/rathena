...
int32 status_damage(block_list* src, block_list* target, int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int32 flag, uint16 skill_id) {
	...
	if (flag & 16) {
		...
		status->sp = 1;
		if (!sp)
			sp = 1; // To make sure the status bar is updated
	}
...
bool status_check_skilluse(const block_list* src, const block_list* target, uint16 skill_id, int32 flag) {
	...
	switch (skill_id) {
		...
		case ALL_ODINS_POWER:
			...
			if (src && map_getcell(src->m, src->x, src->y, CELL_CHKLANDPROTECTOR) && !status_has_mode(status, MD_STATUSIMMUNE) && (src->type != BL_PC || ((TBL_PC*)src)->skillitem != skill_id))
				return false;
			break;
		case SC_MANHOLE:
	...
	if (sc != nullptr && !sc->empty()) {
		...
		if (
		    (sc->getSCE(SC_TRICKDEAD) && skill_id != NV_TRICKDEAD) || (sc->getSCE(SC_AUTOCOUNTER) && !flag && skill_id) || (sc->getSCE(SC_GOSPEL) && sc->getSCE(SC_GOSPEL)->val4 == BCT_SELF && skill_id != PA_GOSPEL) || (sc->getSCE(SC_SUHIDE) && skill_id != SU_HIDE))
			return false;
		...
		if (sc->getSCE(SC_DANCING) != nullptr && flag != 2 && skill_id != 0) {
			...
			if (src->type == BL_PC && ((skill_id >= WA_SWING_DANCE && skill_id <= WM_UNLIMITED_HUMMING_VOICE) || skill_id == WM_FRIGG_SONG)) { // Lvl 5 Lesson or higher allow you use 3rd job skills while dancing.
		...
		if (skill_id &&                                                      // Do not block item-casted skills.
		    (src->type != BL_PC || ((TBL_PC*)src)->skillitem != skill_id)) { // Skills blocked through status changes...
			if (!flag && (                                                   // Blocked only from using the skill (stuff like autospell may still go through
			                 ...
			                 (sc->getSCE(SC_ANKLE) && skill_block_check(src, SC_ANKLE, skill_id)) || (sc->getSCE(SC_STASIS) && skill_block_check(src, SC_STASIS, skill_id)) || (sc->getSCE(SC_BITE) && skill_block_check(src, SC_BITE, skill_id)) || (sc->getSCE(SC_NOVAEXPLOSING) && skill_block_check(src, SC_NOVAEXPLOSING, skill_id)) || (sc->getSCE(SC_GRAVITYCONTROL) && skill_block_check(src, SC_GRAVITYCONTROL, skill_id)) || (sc->getSCE(SC_KAGEHUMI) && skill_block_check(src, SC_KAGEHUMI, skill_id))
#ifdef RENEWAL
	...
	if (tsc != nullptr && !tsc->empty()) {
		if (!skill_id && tsc->getSCE(SC_TRICKDEAD))
			return false;
		if ((skill_id == WZ_STORMGUST || skill_id == WZ_FROSTNOVA || skill_id == NJ_HYOUSYOURAKU || skill_id == NPC_STORMGUST2) && tsc->getSCE(SC_FREEZE))
			return false;
...
void status_calc_misc(block_list* bl, struct status_data* status, int32 level) {
	...
	if (bl->type == BL_HOM) {
		...
		// Flee
		stat = status->flee;
		stat += level + status->agi + (bl->type == BL_MER ? 0 : bl->type == BL_PC ? status->luk / 5
		                                                                          : 0) +
		    100; //base level + ( every 1 agi = +1 flee ) + (every 5 luk = +1 flee) + 100
		stat += 2 * status->con;
		status->flee = cap_value(stat, 1, SHRT_MAX);
		// Def2
		if (bl->type == BL_MER)
...
int32 status_calc_pc_sub(map_session_data* sd, uint8 opt) {
	...
	// Zero up structures...
	memset(&sd->def_set_race, 0, sizeof(sd->def_set_race) + sizeof(sd->mdef_set_race) + sizeof(sd->norecover_state_race) + sizeof(sd->hp_vanish_race) + sizeof(sd->sp_vanish_race));
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (opt & SCO_FIRST && sd->inventory_data[index]->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index], sd->m))) { // Execute equip-script on login
	...
	if (base_status->rhw.range < 1)
		base_status->rhw.range = 1;
	if (base_status->lhw.range < 1)
		base_status->lhw.range = 1;
	...
	if ((pc_isriding(sd) || pc_isridingdragon(sd)) &&
	    (sd->status.weapon == W_1HSPEAR || sd->status.weapon == W_2HSPEAR)) { // When Riding with spear, damage modifier to mid-class becomes
	...
	if (sd->special_state.restart_full_recover) {
		...
		if ((sd->class_ & MAPID_FIRSTMASK) == MAPID_NOVICE && !(sd->class_ & JOBL_2) && battle_config.restart_hp_rate < 50)
...
void status_calc_regen_rate(block_list* bl, struct regen_data* regen, status_change* sc) {
	...
	// No HP or SP regen
	if ((sc->getSCE(SC_POISON) && !sc->getSCE(SC_SLOWPOISON)) || (sc->getSCE(SC_DPOISON) && !sc->getSCE(SC_SLOWPOISON)) || sc->getSCE(SC_BERSERK) || sc->getSCE(SC_TRICKDEAD) || sc->getSCE(SC_BLEEDING) || (sc->getSCE(SC_MAGICMUSHROOM) && sc->getSCE(SC_MAGICMUSHROOM)->val3 == 1) || sc->getSCE(SC_SATURDAYNIGHTFEVER) || sc->getSCE(SC_REBOUND) || sc->getSCE(SC_NORECOVER_STATE))
		regen->flag = RGN_NONE;
...
static uint16 status_calc_watk(block_list* bl, status_change* sc, int32 watk) {
	...
	if (sc->getSCE(SC_FIRE_INSIGNIA) && sc->getSCE(SC_FIRE_INSIGNIA)->val1 == 2)
		watk += 50;
	if ((sc->getSCE(SC_FIRE_INSIGNIA) && sc->getSCE(SC_FIRE_INSIGNIA)->val1 == 2) || (sc->getSCE(SC_WATER_INSIGNIA) && sc->getSCE(SC_WATER_INSIGNIA)->val1 == 2) || (sc->getSCE(SC_WIND_INSIGNIA) && sc->getSCE(SC_WIND_INSIGNIA)->val1 == 2) || (sc->getSCE(SC_EARTH_INSIGNIA) && sc->getSCE(SC_EARTH_INSIGNIA)->val1 == 2))
		watk += watk * 10 / 100;
...
static int16 status_calc_def2(block_list* bl, status_change* sc, int32 def2) {
	...
	if (sc->getSCE(SC_JOINTBEAT))
		def2 -= def2 * (sc->getSCE(SC_JOINTBEAT)->val2 & BREAK_SHOULDER ? 50 : 0) / 100 + def2 * (sc->getSCE(SC_JOINTBEAT)->val2 & BREAK_WAIST ? 25 : 0) / 100;
	if (sc->getSCE(SC_FLING))
...
#endif
			else if (sd && sc->getSCE(SC_DANCING))
...
unsigned char status_calc_attack_element(const block_list* bl, const status_change* sc, int32 element) {
	...
	if (sc->getSCE(SC_WATERWEAPON) || (sc->getSCE(SC_WATER_INSIGNIA) && sc->getSCE(SC_WATER_INSIGNIA)->val1 == 2))
		return ELE_WATER;
	if (sc->getSCE(SC_EARTHWEAPON) || (sc->getSCE(SC_EARTH_INSIGNIA) && sc->getSCE(SC_EARTH_INSIGNIA)->val1 == 2))
		return ELE_EARTH;
	if (sc->getSCE(SC_FIREWEAPON) || (sc->getSCE(SC_FIRE_INSIGNIA) && sc->getSCE(SC_FIRE_INSIGNIA)->val1 == 2))
		return ELE_FIRE;
	if (sc->getSCE(SC_WINDWEAPON) || (sc->getSCE(SC_WIND_INSIGNIA) && sc->getSCE(SC_WIND_INSIGNIA)->val1 == 2))
		return ELE_WIND;
	...
	switch (type) {
		...
		case SC_MODECHANGE: {
			...
			if (sc->getSCE(type)) { // Pile up with previous values.
				if (!val2)
					val2 = sc->getSCE(type)->val2;
				val3 |= sc->getSCE(type)->val3;
				val4 |= sc->getSCE(type)->val4;
			}
			mode = val2 ? ((val2 & ~MD_MASK) | val2) : bstatus->mode; // Base mode
			if (val4)
				mode = static_cast<e_mode>(mode & ~val4); // Del mode
			if (val3)
				mode = static_cast<e_mode>(mode | val3); // Add mode
		...
		case SC_STRIPWEAPON:
			...
			if (tick == 1)
				return true; // Minimal duration: Only strip without causing the SC
			break;
		case SC_STRIPSHIELD:
			if (val2 == 1)
				val2 = 0; // GX effect. Do not take shield off..
			else if (sd && !(flag & SCSTART_LOADED)) {
				int16 i;
	...
	if (!(flag & SCSTART_LOADED)) // &4 - Do not parse val settings when loading SCs
		switch (type) {
			...
			case SC_BERSERK:
				...
				if (!val4)
					val4 = skill_get_time2(scdb->skill_id, val1);
				if (!val4)
			...
			case SC_MARIONETTE2: {
				...
				stat = min(stat, max_stat - status2->str);
				val3 |= cap_value(stat, 0, 0xFF) << 16;
				stat = (psce->val3 >> 8) & 0xFF;
				stat = min(stat, max_stat - status2->agi);
				val3 |= cap_value(stat, 0, 0xFF) << 8;
				stat = (psce->val3 >> 0) & 0xFF;
				stat = min(stat, max_stat - status2->vit);
				val3 |= cap_value(stat, 0, 0xFF);
				stat = (psce->val4 >> 16) & 0xFF;
				stat = min(stat, max_stat - status2->int_);
				val4 |= cap_value(stat, 0, 0xFF) << 16;
				stat = (psce->val4 >> 8) & 0xFF;
				stat = min(stat, max_stat - status2->dex);
				val4 |= cap_value(stat, 0, 0xFF) << 8;
				stat = (psce->val4 >> 0) & 0xFF;
				stat = min(stat, max_stat - status2->luk);
				val4 |= cap_value(stat, 0, 0xFF);
				break;
			}
			case SC_SPIRIT:
				...
				if (sd && val2 == SL_HIGH) {
					...
					stat = max(0, max_stat - status2->str);
					val3 |= cap_value(stat, 0, 0xFF) << 16;
					stat = max(0, max_stat - status2->agi);
					val3 |= cap_value(stat, 0, 0xFF) << 8;
					stat = max(0, max_stat - status2->vit);
					val3 |= cap_value(stat, 0, 0xFF);
					stat = max(0, max_stat - status2->int_);
					val4 |= cap_value(stat, 0, 0xFF) << 16;
					stat = max(0, max_stat - status2->dex);
					val4 |= cap_value(stat, 0, 0xFF) << 8;
					stat = max(0, max_stat - status2->luk);
					val4 |= cap_value(stat, 0, 0xFF);
				}
				break;
			...
			case SC_SIRCLEOFNATURE:
				...
				break;
			case SC_SONGOFMANA:
				status_heal(bl, 0, status->max_sp * (val1 <= 2 ? 10 : val1 <= 4 ? 15
				                                                                : 20) /
				        100,
				    1);
				val3 = 50 * val1;
	...
	switch (type) {
		...
		case SC_CLOSECONFINE:
			if (val2 > 0) {
				...
				int32 range = 1 + skill_get_range2(bl, scdb->skill_id, val1, true) + skill_get_range2(bl, TF_BACKSLIDING, 1, true); // Since most people use this to escape the hold....
	...
	switch (type) {
		...
		case SC_KNOWLEDGE:
			...
			if (bl->m == sd->feel_map[0].m ||
			    ...
			    bl->m == sd->feel_map[2].m) { // Timeout will be handled by pc_setpos
...
int32 status_change_timer_sub(block_list* bl, va_list ap) {
	...
	switch (type) {
		...
		case SC_RUWACH: // Reveal hidden target and deal little dammages if enemy
			if (tsc && (tsc->getSCE(SC_HIDING) || tsc->getSCE(SC_CLOAKING) || tsc->getSCE(SC_CAMOUFLAGE) || tsc->getSCE(SC_NEWMOON) || tsc->getSCE(SC_CLOAKINGEXCEED))) {
				status_change_end(bl, SC_HIDING);
				status_change_end(bl, SC_CLOAKING);
				status_change_end(bl, SC_CAMOUFLAGE);
				status_change_end(bl, SC_CLOAKINGEXCEED);
		...
		case SC_SIGHTBLASTER:
			if (battle_check_target(src, bl, BCT_ENEMY) > 0 &&
				...
				if (sce) {
					...
					if (bl->type == BL_SKILL)
						su = (skill_unit*)bl;
					if (skill_attack(BF_MAGIC, src, src, bl, WZ_SIGHTBLASTER, sce->val1, tick, 0x1000000) && (!su || !su->group || !skill_get_inf2(su->group->skill_id, INF2_ISTRAP))) { // The hit is not counted if it's against a trap
...
static int32 status_natural_heal(block_list* bl, va_list args) {
	...
	if (flag && (status_isdead(*bl) || (sc && (sc->option & (OPTION_HIDE | OPTION_CLOAK | OPTION_CHASEWALK) || sc->getSCE(SC__INVISIBILITY)))))
		flag = RGN_NONE;
