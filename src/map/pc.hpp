...
#define pc_setsit(sd)                                             \
	{                                                             \
		unit_stop_walking((sd), USW_FIXPOS | USW_MOVE_FULL_CELL); \
		unit_stop_attack((sd));                                   \
...
static bool pc_cant_act2(map_session_data* sd) {
	return sd->state.vending || sd->state.buyingstore || (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING) || sd->state.trading || sd->state.storage_flag || sd->state.prevend || sd->state.refineui_open || sd->state.stylist_open || sd->state.inventory_expansion_confirmation || sd->npc_shopid || sd->state.barter_open || sd->state.barter_extended_open || sd->state.laphine_synthesis || sd->state.laphine_upgrade || sd->state.roulette_open || sd->state.enchantgrade_open || sd->state.item_reform || sd->state.item_enchant_index;
}
...
#define pc_check_weapontype(sd, type) ((type) & ((sd)->status.weapon < MAX_WEAPON_TYPE ? 1 << (sd)->status.weapon : (1 << (sd)->weapontype1) | (1 << (sd)->weapontype2) | (1 << (sd)->status.weapon)))
