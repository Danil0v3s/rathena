...
bool navi_path_search(struct navi_walkpath_data* wpd, const struct navi_pos* from, const struct navi_pos* dest, cell_chk cell) {
	...
	for (;;) {
		...
		if (y < ys && !map_getcellp(mapdata, x, y + 1, cell))
			allowed_dirs |= PATH_DIR_NORTH;
		if (y > 0 && !map_getcellp(mapdata, x, y - 1, cell))
			allowed_dirs |= PATH_DIR_SOUTH;
		if (x < xs && !map_getcellp(mapdata, x + 1, y, cell))
			allowed_dirs |= PATH_DIR_EAST;
		if (x > 0 && !map_getcellp(mapdata, x - 1, y, cell))
			allowed_dirs |= PATH_DIR_WEST;
...
void write_spawn(std::ostream& os, const struct map_data* m, const std::shared_ptr<s_mob_db> mobinfo, int32 amount, int32 idx) {
	...
	os << "" << mobinfo->lv << ", ";
	os << "" << (((mobinfo->status.ele_lv * 20 + mobinfo->status.def_ele) << 16) | (mobinfo->status.size << 8) | mobinfo->status.race);
	os << "},\n";
}
	...
	for (int32 mapid = 0; mapid < map_num; mapid++) {
		...
		for (int32 npcidx = 0; npcidx < m->npc_num; npcidx++) {
			...
			if (nd->subtype == NPCTYPE_WARP) {
				...
				if (nd->class_ == -1 || nd->class_ == JT_HIDDEN_NPC || nd->class_ == JT_HIDDEN_WARP_NPC || nd->class_ == JT_GUILD_FLAG || nd->navi.hidden)
					continue;
