...
static void grf_decode(unsigned char* buf, size_t len, char entry_type, int32 entry_len) {
	if (entry_type & FILELIST_TYPE_ENCRYPT_MIXED) { // fully encrypted
		int32 digits;
		int32 cycle;
		int32 i;
...
static int32 grfio_entryread(const char* grfname, int32 gentry) {
	...
	if (grf_version == 0x01) { // ****** Grf version 01xx ******
		...
		if (fread(eheader, 1, 8, fp) != 8)
			ShowError("An error occurred in fread while reading eheader buffer\n");
...
static bool grfio_parse_restable_row(const char* row) {
	...
	if (entry != nullptr) { // alias for GRF resource
		FILELIST fentry;
		memcpy(&fentry, entry, sizeof(FILELIST));
	...
	if (exists(local)) { // alias for local resource
		FILELIST fentry;
