 ...
 *--------------------------------------*/
int32 null_recv(int32 fd) {
	return 0;
}
int32 null_send(int32 fd) {
	return 0;
}
int32 null_parse(int32 fd) {
	return 0;
}
...
int32 recv_to_fifo(int32 fd) {
	...
	if (len == SOCKET_ERROR) { //An exception has occurred
...
int32 _realloc_writefifo(int32 fd, size_t addition, const char* file, int32 line, const char* func) {
	...
	if (session[fd]->wdata_size + addition > session[fd]->max_wdata) { // grow rule; grow in multiples of WFIFO_SIZE
		newsize = WFIFO_SIZE;
		while (session[fd]->wdata_size + addition > newsize)
			newsize += WFIFO_SIZE;
	} else if (session[fd]->max_wdata >= (size_t)2 * (session[fd]->flag.server ? FIFOSIZE_SERVERLINK : WFIFO_SIZE) && (session[fd]->wdata_size + addition) * 4 < session[fd]->max_wdata) { // shrink rule, shrink by 2 when only a quarter of the fifo is used, don't shrink below nominal size.
