 ...
 */
	template <typename K, typename V>
	bool umap_exists(std::unordered_map<K, V>& map, K key) {
		return map.find(key) != map.end();
	}
 ...
 */
	template <typename K, typename V>
	bool vector_erase_if_exists(std::vector<K>& vector, V value) {
...
**/
	...
	template <typename InstanceClass, typename InterfaceClass = InstanceClass>
	class Singleton {
	protected:
		Singleton() = default;
		~Singleton() = default;
