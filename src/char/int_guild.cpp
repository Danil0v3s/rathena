	...
	*t_info = '\0';
		...
		for (i = 0; i < g.max_member; i++) {
			...
			if (m->account_id) {
				...
				if (m->modified & GS_MEMBER_NEW || new_guild == 1) {
					if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `guild_id` = '%d' WHERE `char_id` = '%d'", schema_config.char_db, g.guild_id, m->char_id))
						Sql_ShowDebug(sql_handle);
				}
	...
	if (flag & GS_POSITION) {
		...
		for (i = 0; i < MAX_GUILDPOSITION; i++) {
			...
			if (SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` (`guild_id`,`position`,`name`,`mode`,`exp_mode`) VALUES ('%d','%d','%s','%d','%d')", schema_config.guild_position_db, g.guild_id, i, esc_name, p->mode, p->exp_mode))
				Sql_ShowDebug(sql_handle);
		...
		for (i = 0; i < MAX_GUILDEXPULSION; i++) {
			...
			if (e->account_id > 0) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` (`guild_id`,`account_id`,`name`,`mes`,`char_id`) "
				                                       "VALUES ('%u','%u','%s','%s','%u')",
				                     schema_config.guild_expulsion_db, g.guild_id, e->account_id, esc_name, esc_mes, e->char_id))
					Sql_ShowDebug(sql_handle);
			}
		}
	}
	...
	if (flag & GS_SKILL) {
		...
		for (i = 0; i < MAX_GUILDSKILL; i++) {
			if (g.skill[i].id > 0 && g.skill[i].lv > 0) {
				if (SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `%s` (`guild_id`,`id`,`lv`) VALUES ('%d','%d','%d')", schema_config.guild_skill_db, g.guild_id, g.skill[i].id, g.skill[i].lv))
					Sql_ShowDebug(sql_handle);
			}
		}
	}
...
std::shared_ptr<CharGuild> inter_guild_fromsql(int32 guild_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT g.`name`,c.`name`,g.`guild_lv`,g.`connect_member`,g.`max_member`,g.`average_lv`,g.`exp`,g.`next_exp`,g.`skill_point`,g.`mes1`,g.`mes2`,g.`emblem_len`,g.`emblem_id`,COALESCE(UNIX_TIMESTAMP(g.`last_master_change`),0), g.`emblem_data` "
	                                       "FROM `%s` g LEFT JOIN `%s` c ON c.`char_id` = g.`char_id` WHERE g.`guild_id`='%d'",
	                     schema_config.guild_db, schema_config.char_db, guild_id)) {
		Sql_ShowDebug(sql_handle);
		return nullptr;
	}
	...
	memcpy(g->guild.name, data, zmin(len, NAME_LENGTH));
	Sql_GetData(sql_handle, 1, &data, &len);
	memcpy(g->guild.master, data, zmin(len, NAME_LENGTH));
	Sql_GetData(sql_handle, 2, &data, nullptr);
	g->guild.guild_lv = atoi(data);
	Sql_GetData(sql_handle, 3, &data, nullptr);
	g->guild.connect_member = atoi(data);
	Sql_GetData(sql_handle, 4, &data, nullptr);
	g->guild.max_member = atoi(data);
	if (g->guild.max_member > MAX_GUILD) { // Fix reduction of MAX_GUILD [PoW]
	...
	g->guild.average_lv = atoi(data);
	Sql_GetData(sql_handle, 6, &data, nullptr);
	g->guild.exp = strtoull(data, nullptr, 10);
	Sql_GetData(sql_handle, 7, &data, nullptr);
	g->guild.next_exp = strtoull(data, nullptr, 10);
	Sql_GetData(sql_handle, 8, &data, nullptr);
	g->guild.skill_point = atoi(data);
	Sql_GetData(sql_handle, 9, &data, &len);
	memcpy(g->guild.mes1, data, zmin(len, sizeof(g->guild.mes1)));
	Sql_GetData(sql_handle, 10, &data, &len);
	memcpy(g->guild.mes2, data, zmin(len, sizeof(g->guild.mes2)));
	Sql_GetData(sql_handle, 11, &data, &len);
	g->guild.emblem_len = atoi(data);
	Sql_GetData(sql_handle, 12, &data, &len);
	g->guild.emblem_id = atoi(data);
	Sql_GetData(sql_handle, 13, &data, nullptr);
	g->guild.last_leader_change = atoi(data);
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `c`.`account_id`,`m`.`char_id`,`c`.`hair`,`c`.`hair_color`,`c`.`sex`,`c`.`class`,`c`.`base_level`,`m`.`exp`,`c`.`online`,`m`.`position`,`c`.`name`,coalesce(UNIX_TIMESTAMP(`c`.`last_login`),0) "
	                                       "FROM `%s` `m` INNER JOIN `%s` `c` on `c`.`char_id`=`m`.`char_id` WHERE `m`.`guild_id`='%d' ORDER BY `position`",
	                     schema_config.guild_member_db, schema_config.char_db, guild_id)) {
		Sql_ShowDebug(sql_handle);
		return nullptr;
	}
	for (i = 0; i < g->guild.max_member && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i) {
		...
		m->lv = atoi(data);
		Sql_GetData(sql_handle, 7, &data, nullptr);
		m->exp = strtoull(data, nullptr, 10);
		Sql_GetData(sql_handle, 8, &data, nullptr);
		m->online = atoi(data);
		...
		Sql_GetData(sql_handle, 10, &data, &len);
		memcpy(m->name, data, zmin(len, NAME_LENGTH));
	...
	while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		...
		Sql_GetData(sql_handle, 1, &data, &len);
		memcpy(gpos->name, data, zmin(len, NAME_LENGTH));
		Sql_GetData(sql_handle, 2, &data, nullptr);
		gpos->mode = atoi(data);
		Sql_GetData(sql_handle, 3, &data, nullptr);
		gpos->exp_mode = atoi(data);
	...
	for (i = 0; i < MAX_GUILDALLIANCE && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i) {
		...
		memcpy(a->name, data, zmin(len, NAME_LENGTH));
	}
	...
	for (i = 0; i < MAX_GUILDEXPULSION && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i) {
		...
		Sql_GetData(sql_handle, 1, &data, &len);
		memcpy(e->name, data, zmin(len, NAME_LENGTH));
		Sql_GetData(sql_handle, 2, &data, &len);
		memcpy(e->mes, data, zmin(len, sizeof(e->mes)));
		Sql_GetData(sql_handle, 3, &data, nullptr);
		e->char_id = strtoul(data, nullptr, 10);
	}
	...
	for (i = 0; i < MAX_GUILDSKILL; i++) { //Skill IDs must always be initialized. [Skotlex]
...
std::shared_ptr<struct guild_castle> inter_guildcastle_fromsql(int32 castle_id) {
	...
	if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		Sql_GetData(sql_handle, CD_GUILD_ID, &data, nullptr);
		gc->guild_id = atoi(data);
		Sql_GetData(sql_handle, CD_CURRENT_ECONOMY, &data, nullptr);
		gc->economy = atoi(data);
		Sql_GetData(sql_handle, CD_CURRENT_DEFENSE, &data, nullptr);
		gc->defense = atoi(data);
		Sql_GetData(sql_handle, CD_INVESTED_ECONOMY, &data, nullptr);
		gc->triggerE = atoi(data);
		Sql_GetData(sql_handle, CD_INVESTED_DEFENSE, &data, nullptr);
		gc->triggerD = atoi(data);
		Sql_GetData(sql_handle, CD_NEXT_TIME, &data, nullptr);
		gc->nextTime = atoi(data);
		Sql_GetData(sql_handle, CD_PAY_TIME, &data, nullptr);
		gc->payTime = atoi(data);
		Sql_GetData(sql_handle, CD_CREATE_TIME, &data, nullptr);
		gc->createTime = atoi(data);
		Sql_GetData(sql_handle, CD_ENABLED_KAFRA, &data, nullptr);
		gc->visibleC = atoi(data);
		for (i = CD_ENABLED_GUARDIAN00; i < CD_MAX; i++) {
			Sql_GetData(sql_handle, i, &data, nullptr);
			gc->guardian[i - CD_ENABLED_GUARDIAN00].visible = atoi(data);
		}
	}
	Sql_FreeResult(sql_handle);
...
int32 mapif_parse_GuildLeave(int32 fd, int32 guild_id, uint32 account_id, uint32 char_id, int32 flag, const char* mes) {
	...
	if (flag) { // Write expulsion reason
		// Find an empty slot
		int32 j;
