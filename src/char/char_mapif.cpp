...
int32 chmapif_parse_askscdata(int32 fd) {
	if (RFIFOREST(fd) < 10)
		...
		aid = RFIFOL(fd, 2);
		cid = RFIFOL(fd, 6);
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT type, tick, val1, val2, val3, val4 from `%s` WHERE `account_id` = '%d' AND `char_id`='%d'", schema_config.scdata_db, aid, cid)) {
		...
		if (Sql_NumRows(sql_handle) > 0) {
			...
			for (count = 0; count < 50 && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
				...
				scdata.tick = strtoll(data, nullptr, 10);
...
int32 chmapif_parse_req_skillcooldown(int32 fd) {
	if (RFIFOREST(fd) < 10)
		...
		cid = RFIFOL(fd, 6);
		RFIFOSKIP(fd, 10);
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT skill, tick FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'", schema_config.skillcooldown_db, aid, cid)) {
			Sql_ShowDebug(sql_handle);
			return 1;
		}
		if (Sql_NumRows(sql_handle) > 0) {
			...
			for (count = 0; count < MAX_SKILLCOOLDOWN && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
				Sql_GetData(sql_handle, 0, &data, nullptr);
				scd.skill_id = atoi(data);
				Sql_GetData(sql_handle, 1, &data, nullptr);
				scd.tick = strtoll(data, nullptr, 10);
...
int32 chmapif_parse_askrmfriend(int32 fd) {
	if (RFIFOREST(fd) < 10)
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `friend_id`='%d' LIMIT 1", schema_config.friend_db, char_id, friend_id)) {
			Sql_ShowDebug(sql_handle);
			return 1;
		}
		RFIFOSKIP(fd, 10);
...
int32 chmapif_parse_reqcharban(int32 fd) {
	if (RFIFOREST(fd) < 10 + NAME_LENGTH)
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id`,`char_id`,`unban_time` FROM `%s` WHERE `name` = '%s'", schema_config.char_db, esc_name))
			...
			else if (unban_time < now)
				unban_time = now;   //new entry
			unban_time += timediff; //alterate the time
			if (unban_time < now)
				unban_time = 0; //we have totally reduce the time

			if (SQL_SUCCESS != stmt.Prepare("UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1", schema_config.char_db) || SQL_SUCCESS != stmt.BindParam(0, SQLDT_LONG, (void*)&unban_time, sizeof(unban_time)) || SQL_SUCCESS != stmt.BindParam(1, SQLDT_INT32, (void*)&t_cid, sizeof(t_cid)) || SQL_SUCCESS != stmt.Execute()
...
int32 chmapif_bonus_script_get(int32 fd) {
	if (RFIFOREST(fd) < 6)
		...
		if (SQL_ERROR == stmt.Prepare("SELECT `script`, `tick`, `flag`, `type`, `icon` FROM `%s` WHERE `char_id` = '%d' LIMIT %d", schema_config.bonus_script_db, cid, MAX_PC_BONUS_SCRIPT) ||
...
int32 chmapif_parse(int32 fd) {
	...
	if (id == ARRAYLENGTH(map_server)) { // not a map server
	...
	while (RFIFOREST(fd) >= 2) {
		...
		switch (RFIFOW(fd, 0)) {
			...
			case 0x2b2a:
				next = chmapif_parse_reqcharunban(fd);
				break; //charunban
