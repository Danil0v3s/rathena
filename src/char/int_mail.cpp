...
int32 mail_savemessage(struct mail_message* msg) {
	...
	if (SQL_SUCCESS != stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_SUCCESS != stmt.BindParam(0, SQLDT_STRING, msg->send_name, strnlen(msg->send_name, NAME_LENGTH)) || SQL_SUCCESS != stmt.BindParam(1, SQLDT_STRING, msg->dest_name, strnlen(msg->dest_name, NAME_LENGTH)) || SQL_SUCCESS != stmt.BindParam(2, SQLDT_STRING, msg->title, strnlen(msg->title, MAIL_TITLE_LENGTH)) || SQL_SUCCESS != stmt.BindParam(3, SQLDT_STRING, msg->body, strnlen(msg->body, MAIL_BODY_LENGTH)) || SQL_SUCCESS != stmt.Execute()) {
...
bool mail_loadmessage(int32 mail_id, struct mail_message* msg) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,`time`,`status`,`zeny`,`type` FROM `%s` WHERE `id` = '%d'", schema_config.mail_db, mail_id) || SQL_SUCCESS != Sql_NextRow(sql_handle)) {
		Sql_ShowDebug(sql_handle);
		Sql_FreeResult(sql_handle);
		return false;
	} else {
		...
		msg->type = (mail_inbox_type)atoi(data);
	...
	for (i = 0; i < MAIL_MAX_ITEM && SQL_SUCCESS == Sql_NextRow(sql_handle); i++) {
		Sql_GetData(sql_handle, 0, &data, nullptr);
		msg->item[i].amount = (int16)atoi(data);
		Sql_GetData(sql_handle, 1, &data, nullptr);
		msg->item[i].nameid = strtoul(data, nullptr, 10);
		Sql_GetData(sql_handle, 2, &data, nullptr);
		msg->item[i].refine = atoi(data);
		Sql_GetData(sql_handle, 3, &data, nullptr);
		msg->item[i].attribute = atoi(data);
		Sql_GetData(sql_handle, 4, &data, nullptr);
		msg->item[i].identify = atoi(data);
		Sql_GetData(sql_handle, 5, &data, nullptr);
		msg->item[i].unique_id = strtoull(data, nullptr, 10);
		Sql_GetData(sql_handle, 6, &data, nullptr);
		msg->item[i].bound = atoi(data);
		Sql_GetData(sql_handle, 7, &data, nullptr);
		msg->item[i].enchantgrade = atoi(data);
		msg->item[i].expire_time = 0;

		for (j = 0; j < MAX_SLOTS; j++) {
			Sql_GetData(sql_handle, 8 + j, &data, nullptr);
			msg->item[i].card[j] = strtoul(data, nullptr, 10);
		}

		for (j = 0; j < MAX_ITEM_RDM_OPT; j++) {
			Sql_GetData(sql_handle, 8 + MAX_SLOTS + j * 3, &data, nullptr);
			msg->item[i].option[j].id = atoi(data);
			Sql_GetData(sql_handle, 9 + MAX_SLOTS + j * 3, &data, nullptr);
			msg->item[i].option[j].value = atoi(data);
			Sql_GetData(sql_handle, 10 + MAX_SLOTS + j * 3, &data, nullptr);
			msg->item[i].option[j].param = atoi(data);
		}
	}
...
int32 mail_timer_sub(int32 limit, enum mail_inbox_type type) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `m`.`id`, `c`.`char_id`, `c`.`account_id`, `c2`.`account_id` "
	                                       "FROM `%s` `m` "
	...
	for (int32 i = 0; i < MAIL_ITERATION_SIZE && SQL_SUCCESS == Sql_NextRow(sql_handle); i++) {
		...
		Sql_GetData(sql_handle, 0, &data, nullptr);
		mails[i].mail_id = atoi(data);
		Sql_GetData(sql_handle, 1, &data, nullptr);
		mails[i].char_id = atoi(data);
		Sql_GetData(sql_handle, 2, &data, nullptr);
		mails[i].account_id = atoi(data);
		Sql_GetData(sql_handle, 3, &data, nullptr);
		mails[i].account_id_sender = atoi(data);
	}
...
void mapif_parse_Mail_send(int32 fd) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id`, `char_id` FROM `%s` WHERE `name` = '%s'", schema_config.char_db, esc_name)) {
		...
		Sql_GetData(sql_handle, 0, &data, nullptr);
		if (atoi(data) != account_id) { // Cannot send mail to char in the same account
			Sql_GetData(sql_handle, 1, &data, nullptr);
			msg.dest_id = atoi(data);
		}
#else
