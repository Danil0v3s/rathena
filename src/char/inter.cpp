...
int32 inter_recv_packet_length[] = {
    ...
    0,
    0,
    0, // 3050-  Auction System [Zephyrus]
    6,
    -1,
    6,
    -1,
    16 + NAME_LENGTH + ACHIEVEMENT_NAME_LENGTH,
    0,
    0,
    0,
    0,
    ...
    0,
    0,
    0, // 3060-  Quest system [Kevin] [Inkfish] / Achievements [Aleos]
    -1,
    10,
    6,
    -1,
    ...
    10,
    6,
    -1, // 3070-  Mercenary packets [Zephyrus], Elemental packets [pakpil]
    52,
    14,
    -1,
    6,
    ...
    0,
    0,
    0, // 3090-  Homunculus packets [albator]
    2,
    -1,
    6,
    6,
...
void mapif_parse_accinfo(int32 fd) {
	...
	if (account_id < START_ACCOUNT_NUM) { // is string
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` FROM `%s` WHERE `name` LIKE '%s' LIMIT 10", schema_config.char_db, query_esq) || Sql_NumRows(sql_handle) == 0) {
...
void mapif_accinfo_ack(bool success, int32 map_fd, int32 u_fd, int32 u_aid, int32 account_id, int32 group_id, int32 logincount, int32 state, const char* email, const char* last_ip, const char* lastlogin, const char* birthdate, const char* userid) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, `online` FROM `%s` WHERE `account_id` = '%d' ORDER BY `char_num` LIMIT %d", schema_config.char_db, account_id, MAX_CHARS) || Sql_NumRows(sql_handle) == 0) {
		if (Sql_NumRows(sql_handle) == 0)
...
int32 mapif_wis_message(std::shared_ptr<struct WisData> wd) {
	...
	if (wd->len > 2047 - headersize)
		wd->len = 2047 - headersize; //Force it to fit to avoid crashes. [Skotlex]
...
int32 mapif_parse_RegistryRequest(int32 fd) {
	...
	if (RFIFOB(fd, 10))
		chlogif_request_accreg2(RFIFOL(fd, 2), RFIFOL(fd, 6));
	return 1;
}
...
int32 inter_parse_frommap(int32 fd) {
	...
	switch (cmd) {
		...
		default:
			if (inter_party_parse_frommap(fd) || inter_guild_parse_frommap(fd) || inter_storage_parse_frommap(fd) || inter_pet_parse_frommap(fd) || inter_homunculus_parse_frommap(fd) || inter_mercenary_parse_frommap(fd) || inter_elemental_parse_frommap(fd) || inter_mail_parse_frommap(fd) || inter_auction_parse_frommap(fd) || inter_quest_parse_frommap(fd) || inter_clan_parse_frommap(fd) || inter_achievement_parse_frommap(fd))
				break;
			else
				return 0;
	}
