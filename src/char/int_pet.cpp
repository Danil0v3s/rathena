...
int32 inter_pet_tosql(int32 pet_id, struct s_pet* p) {
	...
	if (pet_id == -1) { // New pet.
		if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` "
		                     ...
		                     schema_config.pet_db, p->class_, esc_name, p->account_id, p->char_id, p->level, p->egg_id, p->equip, p->intimate, p->hungry, p->rename_flag, p->incubate, p->autofeed)) {
...
int32 inter_pet_fromsql(int32 pet_id, struct s_pet* p) {
	...
	if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		...
		memcpy(p->name, data, zmin(len, NAME_LENGTH));
		...
		p->egg_id = strtoul(data, nullptr, 10);
		Sql_GetData(sql_handle, 7, &data, nullptr);
		p->equip = strtoul(data, nullptr, 10);
		...
		p->incubate = atoi(data);
		Sql_GetData(sql_handle, 12, &data, nullptr);
		p->autofeed = atoi(data) != 0;
