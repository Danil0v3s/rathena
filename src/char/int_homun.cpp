...
bool mapif_homunculus_save(struct s_homunculus* hd) {
	...
	if (hd->hom_id == 0) { // new homunculus
		if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` "
		                     ...
		                     schema_config.homunculus_db, hd->char_id, hd->class_, hd->prev_class, esc_name, hd->level, hd->exp, hd->intimacy, hd->hunger, hd->str, hd->agi, hd->vit, hd->int_, hd->dex, hd->luk, hd->hp, hd->max_hp, hd->sp, hd->max_sp, hd->skillpts, hd->rename_flag, hd->vaporize, hd->autofeed)) {
	...
	// Save skills
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%d'", schema_config.skill_homunculus_db, hd->hom_id) || SQL_ERROR == stmt.Prepare("INSERT INTO `%s` (`homun_id`, `id`, `lv`) VALUES (%d, ?, ?)", schema_config.skill_homunculus_db, hd->hom_id)) {
	...
	for (uint16 i = 0; i < MAX_HOMUNSKILL; ++i) {
		if (hd->hskill[i].id > 0 && hd->hskill[i].lv != 0) {
			if (SQL_ERROR == stmt.BindParam(0, SQLDT_UINT16, &hd->hskill[i].id, 0) || SQL_ERROR == stmt.BindParam(1, SQLDT_UINT16, &hd->hskill[i].lv, 0) || SQL_ERROR == stmt.Execute()) {
				SqlStmt_ShowDebug(stmt);
				return false;
			}
		}
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%d'", schema_config.skillcooldown_homunculus_db, hd->hom_id) || SQL_ERROR == stmt.Prepare("INSERT INTO `%s` (`homun_id`, `skill`, `tick`) VALUES (%d, ?, ?)", schema_config.skillcooldown_homunculus_db, hd->hom_id)) {
		SqlStmt_ShowDebug(stmt);
		return false;
	}
	...
	for (uint16 i = 0; i < MAX_SKILLCOOLDOWN; ++i) {
		...
		if (SQL_ERROR == stmt.BindParam(0, SQLDT_UINT16, &hd->scd[i].skill_id, 0) || SQL_ERROR == stmt.BindParam(1, SQLDT_LONGLONG, &hd->scd[i].tick, 0) || SQL_ERROR == stmt.Execute()) {
			SqlStmt_ShowDebug(stmt);
			return false;
		}
	}
...
bool mapif_homunculus_load(int32 homun_id, struct s_homunculus* hd) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`rename_flag`, `vaporize`, `autofeed` FROM `%s` WHERE `homun_id`='%u'", schema_config.homunculus_db, homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}

	if (!Sql_NumRows(sql_handle)) { //No homunculus found.
	...
	hd->exp = strtoull(data, nullptr, 10);
	Sql_GetData(sql_handle, 7, &data, nullptr);
	hd->intimacy = (uint32)strtoul(data, nullptr, 10);
	...
	hd->hp = strtoul(data, nullptr, 10);
	Sql_GetData(sql_handle, 16, &data, nullptr);
	hd->max_hp = strtoul(data, nullptr, 10);
	Sql_GetData(sql_handle, 17, &data, nullptr);
	hd->sp = strtoul(data, nullptr, 10);
	Sql_GetData(sql_handle, 18, &data, nullptr);
	hd->max_sp = strtoul(data, nullptr, 10);
	Sql_GetData(sql_handle, 19, &data, nullptr);
	hd->skillpts = atoi(data);
	Sql_GetData(sql_handle, 20, &data, nullptr);
	hd->rename_flag = atoi(data);
	Sql_GetData(sql_handle, 21, &data, nullptr);
	hd->vaporize = atoi(data);
	Sql_GetData(sql_handle, 22, &data, nullptr);
	hd->autofeed = atoi(data) != 0;
	Sql_FreeResult(sql_handle);
...
bool mapif_homunculus_delete(int32 homun_id) {
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%u'", schema_config.skillcooldown_homunculus_db, homun_id) || SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%u'", schema_config.skill_homunculus_db, homun_id) || SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%u'", schema_config.homunculus_db, homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
