...
int32 char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p) {
	...
	if (
	    ...
	    (p->spl != cp->spl) || (p->con != cp->con) || (p->crt != cp->crt)) { //Save status
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
		                     ...
		                     schema_config.char_db, p->base_level, p->job_level, p->base_exp, p->job_exp, p->zeny, p->max_hp, p->hp, p->max_sp, p->sp, p->status_point, p->skill_point, p->str, p->agi, p->vit, p->int_, p->dex, p->luk, p->option, p->party_id, p->guild_id, p->pet_id, p->hom_id, p->ele_id, p->weapon, p->shield, p->head_top, p->head_mid, p->head_bottom, p->last_point.map, p->last_point.x, p->last_point.y, p->last_point_instanceid, p->save_point.map, p->save_point.x, p->save_point.y, p->rename,
		                     (unsigned long)p->delete_date, // FIXME: platform-dependent size
		                     p->robe, p->character_moves, p->font, p->uniqueitem_counter, p->hotkey_rowshift, p->clan_id, p->title_id, p->show_equip, p->hotkey_rowshift2, p->max_ap, p->ap, p->trait_point, p->pow, p->sta, p->wis, p->spl, p->con, p->crt, p->account_id, p->char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		} else
	...
	if (
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `class`='%d',"
		                                       ...
		                                       " WHERE  `account_id`='%d' AND `char_id` = '%d'",
		                     schema_config.char_db, p->class_, p->hair, p->hair_color, p->clothes_color, p->body, p->partner_id, p->father, p->mother, p->child, p->karma, p->manner, p->fame, p->inventory_slots, p->body_direction, p->disable_call, p->disable_partyinvite, p->disable_showcostumes, p->account_id, p->char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		} else
...
int32 char_memitemdata_to_sql(const struct item items[], int32 max, int32 id, enum storage_type tableswitch, uint8 stor_id) {
	...
	if (SQL_ERROR == stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_ERROR == stmt.Execute()) {
	...
	while (SQL_SUCCESS == stmt.NextRow()) {
		...
		for (i = 0; i < max; ++i) {
			...
			if (items[i].nameid == item.nameid && items[i].card[0] == item.card[0] && items[i].card[2] == item.card[2] && items[i].card[3] == item.card[3] && items[i].unique_id == item.unique_id) { //They are the same item.
				int32 k;
...
bool char_memitemdata_from_sql(struct s_storage* p, int32 max, int32 id, enum storage_type tableswitch, uint8 stor_id) {
	...
	if (SQL_ERROR == stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &id, 0) || SQL_ERROR == stmt.Execute()) {
		SqlStmt_ShowDebug(stmt);
		return false;
	}
...
int32 char_mmo_chars_fromsql(char_session_data& sd, CHARACTER_INFO chars[], uint8* count) {
	...
	if (SQL_ERROR == stmt.Prepare("SELECT "
	                     ...
	                     schema_config.char_db, sd.account_id, MAX_CHARS) ||
	    SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT32, &p.char_id) || SQL_ERROR == stmt.BindColumn(1, SQLDT_UCHAR, &p.slot) || SQL_ERROR == stmt.BindColumn(2, SQLDT_STRING, &p.name, sizeof(p.name)) || SQL_ERROR == stmt.BindColumn(3, SQLDT_INT16, &p.class_) || SQL_ERROR == stmt.BindColumn(4, SQLDT_UINT32, &p.base_level) || SQL_ERROR == stmt.BindColumn(5, SQLDT_UINT32, &p.job_level) || SQL_ERROR == stmt.BindColumn(6, SQLDT_UINT64, &p.base_exp) || SQL_ERROR == stmt.BindColumn(7, SQLDT_UINT64, &p.job_exp) || SQL_ERROR == stmt.BindColumn(8, SQLDT_INT32, &p.zeny) || SQL_ERROR == stmt.BindColumn(9, SQLDT_INT16, &p.str) || SQL_ERROR == stmt.BindColumn(10, SQLDT_INT16, &p.agi) || SQL_ERROR == stmt.BindColumn(11, SQLDT_INT16, &p.vit) || SQL_ERROR == stmt.BindColumn(12, SQLDT_INT16, &p.int_) || SQL_ERROR == stmt.BindColumn(13, SQLDT_INT16, &p.dex) || SQL_ERROR == stmt.BindColumn(14, SQLDT_INT16, &p.luk) || SQL_ERROR == stmt.BindColumn(15, SQLDT_UINT32, &p.max_hp) || SQL_ERROR == stmt.BindColumn(16, SQLDT_UINT32, &p.hp) || SQL_ERROR == stmt.BindColumn(17, SQLDT_UINT32, &p.max_sp) || SQL_ERROR == stmt.BindColumn(18, SQLDT_UINT32, &p.sp) || SQL_ERROR == stmt.BindColumn(19, SQLDT_UINT32, &p.status_point) || SQL_ERROR == stmt.BindColumn(20, SQLDT_UINT32, &p.skill_point) || SQL_ERROR == stmt.BindColumn(21, SQLDT_UINT32, &p.option) || SQL_ERROR == stmt.BindColumn(22, SQLDT_UCHAR, &p.karma) || SQL_ERROR == stmt.BindColumn(23, SQLDT_INT16, &p.manner) || SQL_ERROR == stmt.BindColumn(24, SQLDT_INT16, &p.hair) || SQL_ERROR == stmt.BindColumn(25, SQLDT_INT16, &p.hair_color) || SQL_ERROR == stmt.BindColumn(26, SQLDT_INT16, &p.clothes_color) || SQL_ERROR == stmt.BindColumn(27, SQLDT_INT16, &p.body) || SQL_ERROR == stmt.BindColumn(28, SQLDT_INT16, &p.weapon) || SQL_ERROR == stmt.BindColumn(29, SQLDT_INT16, &p.shield) || SQL_ERROR == stmt.BindColumn(30, SQLDT_INT16, &p.head_top) || SQL_ERROR == stmt.BindColumn(31, SQLDT_INT16, &p.head_mid) || SQL_ERROR == stmt.BindColumn(32, SQLDT_INT16, &p.head_bottom) || SQL_ERROR == stmt.BindColumn(33, SQLDT_STRING, &p.last_point.map, sizeof(p.last_point.map)) || SQL_ERROR == stmt.BindColumn(34, SQLDT_INT16, &p.rename) || SQL_ERROR == stmt.BindColumn(35, SQLDT_UINT32, &p.delete_date) || SQL_ERROR == stmt.BindColumn(36, SQLDT_INT16, &p.robe) || SQL_ERROR == stmt.BindColumn(37, SQLDT_UINT32, &p.character_moves) || SQL_ERROR == stmt.BindColumn(38, SQLDT_LONG, &p.unban_time) || SQL_ERROR == stmt.BindColumn(39, SQLDT_UCHAR, &p.font) || SQL_ERROR == stmt.BindColumn(40, SQLDT_UINT32, &p.uniqueitem_counter) || SQL_ERROR == stmt.BindColumn(41, SQLDT_ENUM, &sex, sizeof(sex)) || SQL_ERROR == stmt.BindColumn(42, SQLDT_UCHAR, &p.hotkey_rowshift) || SQL_ERROR == stmt.BindColumn(43, SQLDT_ULONG, &p.title_id) || SQL_ERROR == stmt.BindColumn(44, SQLDT_UINT16, &p.show_equip) || SQL_ERROR == stmt.BindColumn(45, SQLDT_UCHAR, &p.hotkey_rowshift2) || SQL_ERROR == stmt.BindColumn(46, SQLDT_UINT32, &p.max_ap) || SQL_ERROR == stmt.BindColumn(47, SQLDT_UINT32, &p.ap) || SQL_ERROR == stmt.BindColumn(48, SQLDT_UINT32, &p.trait_point) || SQL_ERROR == stmt.BindColumn(49, SQLDT_INT16, &p.pow) || SQL_ERROR == stmt.BindColumn(50, SQLDT_INT16, &p.sta) || SQL_ERROR == stmt.BindColumn(51, SQLDT_INT16, &p.wis) || SQL_ERROR == stmt.BindColumn(52, SQLDT_INT16, &p.spl) || SQL_ERROR == stmt.BindColumn(53, SQLDT_INT16, &p.con) || SQL_ERROR == stmt.BindColumn(54, SQLDT_INT16, &p.crt) || SQL_ERROR == stmt.BindColumn(55, SQLDT_UINT16, &p.inventory_slots) || SQL_ERROR == stmt.BindColumn(56, SQLDT_UINT8, &p.body_direction) || SQL_ERROR == stmt.BindColumn(57, SQLDT_UINT16, &p.disable_call) || SQL_ERROR == stmt.BindColumn(58, SQLDT_UINT8, &p.disable_partyinvite) || SQL_ERROR == stmt.BindColumn(59, SQLDT_UINT8, &p.disable_showcostumes)) {
		SqlStmt_ShowDebug(stmt);
		return 0;
	}
...
int32 char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if (SQL_ERROR == stmt.Prepare("SELECT "
	    ...
	    SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT32, &p->char_id) || SQL_ERROR == stmt.BindColumn(1, SQLDT_INT32, &p->account_id) || SQL_ERROR == stmt.BindColumn(2, SQLDT_UCHAR, &p->slot) || SQL_ERROR == stmt.BindColumn(3, SQLDT_STRING, &p->name, sizeof(p->name)) || SQL_ERROR == stmt.BindColumn(4, SQLDT_INT16, &p->class_) || SQL_ERROR == stmt.BindColumn(5, SQLDT_UINT32, &p->base_level) || SQL_ERROR == stmt.BindColumn(6, SQLDT_UINT32, &p->job_level) || SQL_ERROR == stmt.BindColumn(7, SQLDT_UINT64, &p->base_exp) || SQL_ERROR == stmt.BindColumn(8, SQLDT_UINT64, &p->job_exp) || SQL_ERROR == stmt.BindColumn(9, SQLDT_INT32, &p->zeny) || SQL_ERROR == stmt.BindColumn(10, SQLDT_INT16, &p->str) || SQL_ERROR == stmt.BindColumn(11, SQLDT_INT16, &p->agi) || SQL_ERROR == stmt.BindColumn(12, SQLDT_INT16, &p->vit) || SQL_ERROR == stmt.BindColumn(13, SQLDT_INT16, &p->int_) || SQL_ERROR == stmt.BindColumn(14, SQLDT_INT16, &p->dex) || SQL_ERROR == stmt.BindColumn(15, SQLDT_INT16, &p->luk) || SQL_ERROR == stmt.BindColumn(16, SQLDT_UINT32, &p->max_hp) || SQL_ERROR == stmt.BindColumn(17, SQLDT_UINT32, &p->hp) || SQL_ERROR == stmt.BindColumn(18, SQLDT_UINT32, &p->max_sp) || SQL_ERROR == stmt.BindColumn(19, SQLDT_UINT32, &p->sp) || SQL_ERROR == stmt.BindColumn(20, SQLDT_UINT32, &p->status_point) || SQL_ERROR == stmt.BindColumn(21, SQLDT_UINT32, &p->skill_point) || SQL_ERROR == stmt.BindColumn(22, SQLDT_UINT32, &p->option) || SQL_ERROR == stmt.BindColumn(23, SQLDT_UCHAR, &p->karma) || SQL_ERROR == stmt.BindColumn(24, SQLDT_INT16, &p->manner) || SQL_ERROR == stmt.BindColumn(25, SQLDT_INT32, &p->party_id) || SQL_ERROR == stmt.BindColumn(26, SQLDT_INT32, &p->guild_id) || SQL_ERROR == stmt.BindColumn(27, SQLDT_INT32, &p->pet_id) || SQL_ERROR == stmt.BindColumn(28, SQLDT_INT32, &p->hom_id) || SQL_ERROR == stmt.BindColumn(29, SQLDT_INT32, &p->ele_id) || SQL_ERROR == stmt.BindColumn(30, SQLDT_INT16, &p->hair) || SQL_ERROR == stmt.BindColumn(31, SQLDT_INT16, &p->hair_color) || SQL_ERROR == stmt.BindColumn(32, SQLDT_INT16, &p->clothes_color) || SQL_ERROR == stmt.BindColumn(33, SQLDT_INT16, &p->body) || SQL_ERROR == stmt.BindColumn(34, SQLDT_INT16, &p->weapon) || SQL_ERROR == stmt.BindColumn(35, SQLDT_INT16, &p->shield) || SQL_ERROR == stmt.BindColumn(36, SQLDT_INT16, &p->head_top) || SQL_ERROR == stmt.BindColumn(37, SQLDT_INT16, &p->head_mid) || SQL_ERROR == stmt.BindColumn(38, SQLDT_INT16, &p->head_bottom) || SQL_ERROR == stmt.BindColumn(39, SQLDT_STRING, &p->last_point.map, sizeof(p->last_point.map)) || SQL_ERROR == stmt.BindColumn(40, SQLDT_INT16, &p->last_point.x) || SQL_ERROR == stmt.BindColumn(41, SQLDT_INT16, &p->last_point.y) || SQL_ERROR == stmt.BindColumn(42, SQLDT_STRING, &p->save_point.map, sizeof(p->save_point.map)) || SQL_ERROR == stmt.BindColumn(43, SQLDT_INT16, &p->save_point.x) || SQL_ERROR == stmt.BindColumn(44, SQLDT_INT16, &p->save_point.y) || SQL_ERROR == stmt.BindColumn(45, SQLDT_UINT32, &p->partner_id) || SQL_ERROR == stmt.BindColumn(46, SQLDT_UINT32, &p->father) || SQL_ERROR == stmt.BindColumn(47, SQLDT_UINT32, &p->mother) || SQL_ERROR == stmt.BindColumn(48, SQLDT_UINT32, &p->child) || SQL_ERROR == stmt.BindColumn(49, SQLDT_INT32, &p->fame) || SQL_ERROR == stmt.BindColumn(50, SQLDT_INT16, &p->rename) || SQL_ERROR == stmt.BindColumn(51, SQLDT_UINT32, &p->delete_date) || SQL_ERROR == stmt.BindColumn(52, SQLDT_INT16, &p->robe) || SQL_ERROR == stmt.BindColumn(53, SQLDT_UINT32, &p->character_moves) || SQL_ERROR == stmt.BindColumn(54, SQLDT_LONG, &p->unban_time) || SQL_ERROR == stmt.BindColumn(55, SQLDT_UCHAR, &p->font) || SQL_ERROR == stmt.BindColumn(56, SQLDT_UINT32, &p->uniqueitem_counter) || SQL_ERROR == stmt.BindColumn(57, SQLDT_ENUM, &sex, sizeof(sex)) || SQL_ERROR == stmt.BindColumn(58, SQLDT_UCHAR, &p->hotkey_rowshift) || SQL_ERROR == stmt.BindColumn(59, SQLDT_INT32, &p->clan_id) || SQL_ERROR == stmt.BindColumn(60, SQLDT_ULONG, &p->title_id) || SQL_ERROR == stmt.BindColumn(61, SQLDT_UINT16, &p->show_equip) || SQL_ERROR == stmt.BindColumn(62, SQLDT_UCHAR, &p->hotkey_rowshift2) || SQL_ERROR == stmt.BindColumn(63, SQLDT_UINT32, &p->max_ap) || SQL_ERROR == stmt.BindColumn(64, SQLDT_UINT32, &p->ap) || SQL_ERROR == stmt.BindColumn(65, SQLDT_UINT32, &p->trait_point) || SQL_ERROR == stmt.BindColumn(66, SQLDT_INT16, &p->pow) || SQL_ERROR == stmt.BindColumn(67, SQLDT_INT16, &p->sta) || SQL_ERROR == stmt.BindColumn(68, SQLDT_INT16, &p->wis) || SQL_ERROR == stmt.BindColumn(69, SQLDT_INT16, &p->spl) || SQL_ERROR == stmt.BindColumn(70, SQLDT_INT16, &p->con) || SQL_ERROR == stmt.BindColumn(71, SQLDT_INT16, &p->crt) || SQL_ERROR == stmt.BindColumn(72, SQLDT_UINT16, &p->inventory_slots) || SQL_ERROR == stmt.BindColumn(73, SQLDT_UINT8, &p->body_direction) || SQL_ERROR == stmt.BindColumn(74, SQLDT_UINT8, &p->disable_call) || SQL_ERROR == stmt.BindColumn(75, SQLDT_INT32, &p->last_point_instanceid) || SQL_ERROR == stmt.BindColumn(76, SQLDT_UINT8, &p->disable_partyinvite) || SQL_ERROR == stmt.BindColumn(77, SQLDT_UINT8, &p->disable_showcostumes)) {
		SqlStmt_ShowDebug(stmt);
		return 0;
	}
	...
	//read memo data
	//`memo` (`memo_id`,`char_id`,`map`,`x`,`y`)
	if (SQL_ERROR == stmt.Prepare("SELECT `map`,`x`,`y` FROM `%s` WHERE `char_id`=? ORDER by `memo_id` LIMIT %d", schema_config.memo_db, MAX_MEMOPOINTS) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_STRING, &tmp_point.map, sizeof(tmp_point.map), nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(1, SQLDT_INT16, &tmp_point.x, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(2, SQLDT_INT16, &tmp_point.y, 0, nullptr, nullptr))
		SqlStmt_ShowDebug(stmt);
	...
	//read skill
	//`skill` (`char_id`, `id`, `lv`)
	if (SQL_ERROR == stmt.Prepare("SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d", schema_config.skill_db, MAX_SKILL) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_UINT16, &tmp_skill.id, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(1, SQLDT_UINT8, &tmp_skill.lv, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(2, SQLDT_UINT8, &tmp_skill.flag, 0, nullptr, nullptr))
		SqlStmt_ShowDebug(stmt);
	...
	if (SQL_ERROR == stmt.Prepare("SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN `%s` f ON f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d", schema_config.char_db, schema_config.friend_db, MAX_FRIENDS) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT32, &tmp_friend.account_id, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(1, SQLDT_INT32, &tmp_friend.char_id, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(2, SQLDT_STRING, &tmp_friend.name, sizeof(tmp_friend.name), nullptr, nullptr))
		SqlStmt_ShowDebug(stmt);
	...
	if (SQL_ERROR == stmt.Prepare("SELECT `hotkey`, `type`, `itemskill_id`, `skill_lvl` FROM `%s` WHERE `char_id`=?", schema_config.hotkey_db) || SQL_ERROR == stmt.BindParam(0, SQLDT_INT32, &char_id, 0) || SQL_ERROR == stmt.Execute() || SQL_ERROR == stmt.BindColumn(0, SQLDT_INT32, &hotkey_num, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(1, SQLDT_UCHAR, &tmp_hotkey.type, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(2, SQLDT_UINT32, &tmp_hotkey.id, 0, nullptr, nullptr) || SQL_ERROR == stmt.BindColumn(3, SQLDT_UINT16, &tmp_hotkey.lv, 0, nullptr, nullptr))
		SqlStmt_ShowDebug(stmt);
...
int32 char_make_new_char(struct char_session_data* sd, char* name_, int32 str, int32 agi, int32 vit, int32 int_, int32 dex, int32 luk, int32 slot, int32 hair_color, int32 hair_style, int16 start_job, int32 sex) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
	                     ...
	                     schema_config.char_db, sd->account_id, slot, esc_name, start_job, charserv_config.start_zeny, status_points, str, agi, vit, int_, dex, luk, (40 * (100 + vit) / 100), (40 * (100 + vit) / 100), (11 * (100 + int_) / 100), (11 * (100 + int_) / 100), hair_style, hair_color, tmp_start_point[start_point_idx].map, tmp_start_point[start_point_idx].x, tmp_start_point[start_point_idx].y, tmp_start_point[start_point_idx].map, tmp_start_point[start_point_idx].x, tmp_start_point[start_point_idx].y, sex, start_job)) {
		Sql_ShowDebug(sql_handle);
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id) {
	...
	party_id = atoi(data);
	Sql_GetData(sql_handle, 3, &data, nullptr);
	guild_id = atoi(data);
	Sql_GetData(sql_handle, 4, &data, nullptr);
	base_level = atoi(data);
	Sql_GetData(sql_handle, 5, &data, nullptr);
	hom_id = atoi(data);
	Sql_GetData(sql_handle, 6, &data, nullptr);
	partner_id = atoi(data);
	Sql_GetData(sql_handle, 7, &data, nullptr);
	father_id = atoi(data);
	Sql_GetData(sql_handle, 8, &data, nullptr);
	mother_id = atoi(data);
	Sql_GetData(sql_handle, 9, &data, nullptr);
	elemental_id = atoi(data);
	Sql_GetData(sql_handle, 10, &data, nullptr);
	...
	if ((charserv_config.char_config.char_del_level > 0 && base_level >= charserv_config.char_config.char_del_level) || (charserv_config.char_config.char_del_level < 0 && base_level <= -charserv_config.char_config.char_del_level)) {
	...
	if (charserv_config.log_char) {
		if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`char_msg`,`name`) VALUES (NOW(), '%d', '%d', 'Deleted char (CID %d)', '%s')", schema_config.charlog_db, account_id, 0, char_id, esc_name))
			Sql_ShowDebug(sql_handle);
	}
...
void char_auth_ok(int32 fd, struct char_session_data* sd) {
	...
	if (character != nullptr) {
		...
		if (session_isValid(character->fd) && character->fd != fd) { //There's already a connection from this account that hasn't picked a char yet.
			chclif_send_auth_result(fd, 8);
			return;
		}
		character->fd = fd;
...
int32 char_search_mapserver(const std::string& map, uint32 ip, uint16 port) {
	for (int32 i = 0; i < ARRAYLENGTH(map_server); i++) {
		if (session_isValid(map_server[i].fd) && (ip == (uint32)-1 || map_server[i].ip == ip) && (port == (uint16)-1 || map_server[i].port == port)) {
...
bool char_checkdb(void) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `char_id`,`merc_id`,`arch_calls`,`arch_faith`,"
	                                       ...
	                                       " FROM `%s` LIMIT 1;",
	                     schema_config.mercenary_owner_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`guild_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
	                                       "`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`option_val1`,`option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_id4`,`option_val4`,`option_parm4`,`expire_time`,`bound`,`unique_id`,`enchantgrade`"
	                                       " FROM `%s` LIMIT 1;",
	                     schema_config.guild_storage_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	//checking clan table
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `clan_id`,`opposition`,`alliance_id`,`name`"
	                                       " FROM `%s` LIMIT 1;",
	                     schema_config.clan_alliance_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `char_id`,`id`,`count1`,`count2`,`count3`,`count4`,`count5`,`count6`,`count7`,`count8`,`count9`,`count10`,`completed`,`rewarded`"
	                                       " FROM `%s` LIMIT 1;",
	                     schema_config.achievement_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	Sql_FreeResult(sql_handle);
