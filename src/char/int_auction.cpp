...
void auction_save(std::shared_ptr<struct auction_data> auction) {
	...
	if (SQL_SUCCESS != stmt.PrepareStr(StringBuf_Value(&buf)) || SQL_SUCCESS != stmt.BindParam(0, SQLDT_STRING, auction->seller_name, strnlen(auction->seller_name, NAME_LENGTH)) || SQL_SUCCESS != stmt.BindParam(1, SQLDT_STRING, auction->buyer_name, strnlen(auction->buyer_name, NAME_LENGTH)) || SQL_SUCCESS != stmt.BindParam(2, SQLDT_STRING, auction->item_name, strnlen(auction->item_name, ITEM_NAME_LENGTH)) || SQL_SUCCESS != stmt.Execute()) {
...
void inter_auctions_fromsql(void) {
	...
	while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		...
		item->nameid = strtoul(data, nullptr, 10);
		...
		item->unique_id = strtoull(data, nullptr, 10);
		Sql_GetData(sql_handle, 15, &data, nullptr);
		item->enchantgrade = atoi(data);
