...
int32 inter_party_tosql(struct party* p, int32 flag, int32 index) {
	...
	if (flag & PS_BASIC) { // Update party info.
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `name`='%s', `exp`='%d', `item`='%d' WHERE `party_id`='%d'", schema_config.party_db, esc_name, p->exp, p->item, party_id))
			Sql_ShowDebug(sql_handle);
	}

	if (flag & PS_LEADER) { // Update leader
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s`  SET `leader_id`='%d', `leader_char`='%d' WHERE `party_id`='%d'", schema_config.party_db, p->member[index].account_id, p->member[index].char_id, party_id))
			Sql_ShowDebug(sql_handle);
	}

	if (flag & PS_ADDMEMBER) { // Add one party member.
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `party_id`='%d' WHERE `account_id`='%d' AND `char_id`='%d'", schema_config.char_db, party_id, p->member[index].account_id, p->member[index].char_id))
			Sql_ShowDebug(sql_handle);
	}

	if (flag & PS_DELMEMBER) { // Remove one party member.
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d' AND `account_id`='%d' AND `char_id`='%d'", schema_config.char_db, party_id, p->member[index].account_id, p->member[index].char_id))
			Sql_ShowDebug(sql_handle);
	}
...
std::shared_ptr<struct party_data> inter_party_fromsql(int32 party_id) {
	...
	memcpy(p->party.name, data, zmin(len, NAME_LENGTH));
	...
	for (i = 0; i < MAX_PARTY && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i) {
		...
		memcpy(m->map, data, zmin(len, sizeof(m->map)));
		Sql_GetData(sql_handle, 5, &data, nullptr);
...
int32 inter_party_sql_init(void) {
	...
	if (charserv_config.clear_parties) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` USING `%s` LEFT JOIN `%s` ON `%s`.leader_id =`%s`.account_id AND `%s`.leader_char = `%s`.char_id WHERE `%s`.account_id IS NULL", schema_config.party_db, schema_config.party_db, schema_config.char_db, schema_config.party_db, schema_config.char_db, schema_config.party_db, schema_config.char_db, schema_config.char_db))
			Sql_ShowDebug(sql_handle);
	}
...
int32 inter_party_CharOnline(uint32 char_id, int32 party_id) {
	...
	if (party_id == -1) { // Get party_id from the database
		char* data;
...
int32 inter_party_CharOffline(uint32 char_id, int32 party_id) {
	...
	if (party_id == -1) { // Get guild_id from the database
		char* data;
