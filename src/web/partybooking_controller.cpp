...
std::string s_party_booking_entry::to_json(std::string& world_name) {
	return "{ \"AID\": " + std::to_string(this->account_id) +
...
bool party_booking_read(std::string& world_name, std::vector<s_party_booking_entry>& output, const std::string& condition, const std::string& order) {
	...
	if (SQL_SUCCESS != stmt.Prepare(query.c_str()) || SQL_SUCCESS != stmt.BindParam(0, SQLDT_STRING, (void*)world_name_escaped, strlen(world_name_escaped)) || SQL_SUCCESS != stmt.Execute() || SQL_SUCCESS != stmt.BindColumn(0, SQLDT_UINT32, &entry.account_id) || SQL_SUCCESS != stmt.BindColumn(1, SQLDT_UINT32, &entry.char_id) || SQL_SUCCESS != stmt.BindColumn(2, SQLDT_STRING, (void*)char_name, sizeof(char_name)) || SQL_SUCCESS != stmt.BindColumn(3, SQLDT_UINT16, &entry.purpose) || SQL_SUCCESS != stmt.BindColumn(4, SQLDT_UINT8, &entry.assist) || SQL_SUCCESS != stmt.BindColumn(5, SQLDT_UINT8, &entry.damagedealer) || SQL_SUCCESS != stmt.BindColumn(6, SQLDT_UINT8, &entry.healer) || SQL_SUCCESS != stmt.BindColumn(7, SQLDT_UINT8, &entry.tanker) || SQL_SUCCESS != stmt.BindColumn(8, SQLDT_UINT16, &entry.minimum_level) || SQL_SUCCESS != stmt.BindColumn(9, SQLDT_UINT16, &entry.maximum_level) || SQL_SUCCESS != stmt.BindColumn(10, SQLDT_STRING, (void*)comment, sizeof(comment))) {
...
HANDLER_FUNC(partybooking_add) {
	...
	if (SQL_SUCCESS != stmt.Prepare("SELECT 1 FROM `%s` WHERE `leader_id` = ? AND `leader_char` = ?", party_table, aid, cid) || SQL_SUCCESS != stmt.BindParam(0, SQLDT_UINT32, &aid, sizeof(aid)) || SQL_SUCCESS != stmt.BindParam(1, SQLDT_UINT32, &cid, sizeof(cid)) || SQL_SUCCESS != stmt.Execute()) {
		SqlStmt_ShowDebug(stmt);
		csl.unlock();
		res.status = HTTP_BAD_REQUEST;
		res.set_content("Error", "text/plain");
