...
3. All rAthena development commands can be executed inside the dev container, such as compiling (`cmake --preset make`, `cmake --build --preset make`) and starting the server (`./athena-start`, `gdb map-server`, etc ...)
...
6. Change the value of `BUILDER_CMAKE_ARGS` environment variable of the `builder` service in order to change the parameters sent to the `cmake` configure command (e.g. `-DPACKETVER=20211103 -DENABLE_PRERE=ON`)
